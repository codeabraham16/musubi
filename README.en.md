...
| **Skills** | `musubi_detect_stack` · `musubi_search_skills` · `musubi_list_skills` · `musubi_promote_skill` · `musubi_install_skill` · `musubi_save_skill` · `musubi_resolve_skills` · `musubi_skill_usage` (what happened to each skill when it fired) · `musubi_log_skill_decision` · `musubi_discover_skills` (marketplace) · `musubi_author_skill` |
| **Ingestion** | `musubi_ingest_url` (links/media → brain) |
| **Orchestration** | `musubi_workflow` (DAG) · `musubi_work` (multi-agent) · `musubi_phase` (pipeline) · `musubi_sdd` (guided flow) · `musubi_debate` |
| **Hybrid sync** (central brain) | `musubi_promote` · `musubi_sync_status` · `musubi_sync_requeue` · `musubi_sync_pull` |
| **Telemetry & health** | `musubi_log_error` · `musubi_resolve_telemetry` · `musubi_doctor` · `musubi_maintain` · `musubi_insights` · `musubi_tool_usage` (which tools actually get used; persistent ledger) · `musubi_readiness` (how ready the install is, measured by what it DID; an unobserved signal scores zero) |
| **Memory conflicts** | `musubi_conflicts` · `musubi_judge` |
| **Fleet** (device control) | `musubi_fleet_enroll` (**admin**: registers a machine and hands over its token once; the tier decides what can be asked of it) · `musubi_fleet_list` (inventory with `online` DERIVED from the last sign of life, not stored) · `musubi_fleet_metrics` (host telemetry: CPU, RAM, disk, load, uptime; only the machines your credential was granted `metrics` on) · `musubi_fleet_exec` (runs an ARGV on a machine and waits for the result; requires the `exec` capability ON THAT MACHINE) · `musubi_fleet_log` (the audit trail: who ran what, where, and how it went — permanent, while the output expires) · `musubi_fleet_shell` (INTERACTIVE SHELL over SSH; requires the `shell` capability, which is SEPARATE from `exec` and never implied by it: whoever gets a prompt runs anything, so gating it behind `exec` would turn the per-command allowlist into decoration. Two ceilings enforced by the brain: max lifetime and idle) · `musubi_fleet_shell_log` (who had a prompt, where, and for how long; the CONTENT is never recorded) · `musubi_fleet_screen` (screen session over self-hosted RustDesk: the password is minted per session, short-lived, and **Musubi never stores it**) · `musubi_fleet_sessions` (who asked to view which screen) · `musubi_fleet_cronologia` (WHAT HAPPENED TO a machine within a window: commands, screens and shells CROSSED into a single timeline. The gate is PER FACT, not per list —`exec` for commands, `screen:view` for screens, `shell` for shells—, so two people see two different timelines of the same machine and neither sees more than they should. What the list does NOT contain travels in the response: an empty timeline means «nothing happened OF WHAT I LOOK AT», not «nothing happened») · `musubi_fleet_contexto` (CROSSES what happened to a machine with what the team KNEW: the window's activity alongside the memory notes and the code touched in that same stretch. It is CORRELATION, NOT CAUSE, and every finding carries its `enlace` —`termino` if the text NAMES the machine or one of its services, `ventana` if it merely coincides in time—: merging them would turn any coincidence into a lead. Terms come from the INVENTORY, not from command text, and are declared so you can judge the link) · `musubi_fleet_probe` (goes out to measure what runs no agent: Tier B over SSH, Android over ADB) · `musubi_fleet_services` (what RUNS inside each machine —systemd units, Windows services, containers—; gated by `metrics` on each machine, and `unknown` is NOT `stopped`: a machine that could not enumerate its services is not claiming postgres is down) · `musubi_fleet_service_declare` (**admin**: declares by hand what no machine enumerates on its own —a Tier B, a bot, a bridge—; it inherits the project FROM ITS MACHINE and starts out «declared and not yet measured», a legitimate state. What a person declares is NEVER pruned by the machine's heartbeat —it leaves the inventory when a person removes it—, and re-declaring a decommissioned one brings it back) · `musubi_fleet_consent` (**admin**: what is owed to the person USING the machine when someone asks to come in. A SEPARATE axis from capabilities: `screen` decides who may enter, this decides what happens to whoever is inside. `libre` · `avisa` · `pide` · `prohibido`, and when two sources disagree the MOST RESTRICTIVE wins — a machine hardens what the project said, never loosens it. `pide` on a machine with nobody to ask hardens to `prohibido`, it does not soften to `libre`) · `musubi_fleet_rename` (**admin**: renames a machine KEEPING ITS ID —and with it its audit trail, sessions, services and token—. Renaming is NOT cosmetic: three things index BY NAME and none by id —the grants in `principals.yaml`, the per-command allowlist and policy scope—, so it can silently take `exec` away from someone or hand it over. That is why it does NOT rename on the first call: it reports which credentials and policies name the OLD name AND the NEW one —whose authorization the machine would inherit—, says what to edit, and stops until you pass `confirmar`) · `musubi_fleet_require_approval` (**admin**: turns on FOUR-EYES for a machine — a second principal must approve every `shell` or `screen`. A THIRD axis: capabilities say who may, consent says what is owed to whoever is at the machine, and this says HOW MANY PEOPLE are needed. Off by default, enabled per machine; ⚠ with only one person holding that capability, turning it on is a lock, not a slow control) · `musubi_fleet_approve` (you are the second person: requires THE SAME capability as the requested session — not `admin`: the bar is «you could have done it yourself» — and NOBODY approves their own. Single-use, and a «no» stands until the request expires: if it could be re-asked immediately, the control would be «ask until someone says yes») · `musubi_fleet_approvals` (what is waiting for a second pair of eyes; approval does NOT travel, so you have to come look. Shows only what YOU could approve, and counts what it left out) · `musubi_fleet_revoke` (**admin**: kill switch; the row stays for the audit trail). A device token does **not** authenticate against `/mcp`: it beats against `POST /fleet/heartbeat` and nothing else, so compromising one machine of the fleet does not hand over the team's memory |
| **Identity & access** (central brain) | `musubi_whoami` (who am I? read-only) · `musubi_token_new` · `musubi_token_list` · `musubi_token_revoke` (**admin**: adding/removing members over the network, the counterpart of `musubi token`) |
