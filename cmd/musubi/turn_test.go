...
func TestTurnBrevityManualInjectsOncePerSession(t *testing.T) {
	...
	_, ctx := hookAdditionalContext(t, turnOutput(store, loop, pipeOff(), maOff(), 0, "full", strings.NewReader(in)))
	...
	_, ctx2 := hookAdditionalContext(t, turnOutput(store, loop, pipeOff(), maOff(), 0, "full", strings.NewReader(in)))
...
func TestTurnBrevityAutoFiresOnlyWhenOverBudget(t *testing.T) {
	...
	_, ctx := hookAdditionalContext(t, turnOutput(store, loop, pipeOff(), maOff(), 8000, "auto", strings.NewReader(in)))
...
func TestTurnBrevityAutoSilentUnderOrDisabled(t *testing.T) {
	...
	store.ledger = map[string]int{"startup_cognitive": 9000}
	_, ctx0 := hookAdditionalContext(t, turnOutput(store, loop, pipeOff(), maOff(), 0, "auto", strings.NewReader(in)))
...
func TestTurnBrevityOffSilent(t *testing.T) {
	...
	for _, mode := range []string{"", "off"} {
		_, ctx := hookAdditionalContext(t, turnOutput(store, loop, pipeOff(), maOff(), 0, mode, strings.NewReader(in)))
