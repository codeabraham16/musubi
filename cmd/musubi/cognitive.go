	...
	return []skills.Skill{
		...
		},
		{
			...
			Rules: "Antes de dar por bueno un cambio de riesgo (o en la fase verify de un flujo SDD), sometelo a un DEBATE adversarial con veredicto determinista en vez de una sola lectura complaciente:\n" +
				...
				"3. Abrí el debate con musubi_debate action=open topic=<el cambio a juzgar> rounds=2 quorum=<mayoría de los escépticos, p. ej. 2 de 3>. Guardá el debate_id. (musubi_debate estructura las rondas y CUENTA los votos sin sesgo; la inteligencia de refutar es tuya.)\n" +
				"4. Lanzá N sub-agentes escépticos con el Task tool (mcpServers:[musubi]), cada uno con un LENTE distinto (correctitud, seguridad, ¿reproduce el bug?, contrato de la spec). A CADA uno pasale la evidencia del paso 1 e instruílo a intentar REFUTAR el cambio desde su lente, posteando su hallazgo con musubi_debate action=post id=<debate_id> agent=<lente> stance=<veredicto + evidencia>. Ante la duda, que refute.\n" +
