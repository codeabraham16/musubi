	...
	return []skills.Skill{
		...
		},
		{
			...
			Rules: "Antes de dar por bueno un cambio de riesgo (o en la fase verify de un flujo SDD), sometelo a un DEBATE adversarial con veredicto determinista en vez de una sola lectura complaciente:\n" +
				...
				"2. SABOTEÁ CADA INVARIANTE NUEVO. Rompé a propósito el código que la prueba dice cubrir y comprobá que se ponga en ROJO; si sigue verde, esa prueba no estaba midiendo nada. Dos trampas ya medidas acá: la mutación VACUA (el cambio no altera lo que la prueba observa, así que el verde es correcto y no informa) y el SITIO EQUIVOCADO (mide bien, pero otra cosa). Antes de concluir que 'el sabotaje no rompió nada', verificá que el archivo haya cambiado de verdad.\n" +
				"3. DIMENSIONÁ EL PANEL CON EL CAMBIO, NO A OJO. Corré musubi_detect_changes: su campo `revision` trae el NIVEL y el PANEL (jueces/rondas/quórum) derivados de siete señales del propio diff, con el desglose para rehacer la cuenta. Usá ESE panel en vez del mismo tribunal para todo: un typo y una refactorización con cuarenta llamadores no merecen lo mismo, y revisar de más enseña a saltearse la revisión. Si `revision.radio_ciego` viene true, el grafo no cubre parte de lo tocado y el nivel ya subió por eso: NO lo bajes a mano.\n" +
				...
				"5. Lanzá los sub-agentes escépticos que pida el panel (jueces) con el Task tool (mcpServers:[musubi]), cada uno con un LENTE distinto (correctitud, seguridad, ¿reproduce el bug?, contrato de la spec) y, si podés, con un MODELO distinto: dos jueces del mismo modelo no son dos opiniones. A cada uno pasale la evidencia del paso 1 e instruílo a intentar REFUTAR desde su lente. CONGELÁ cada hallazgo ANTES de que entre al debate: `musubi receipt freeze --id <debate_id>/<lente>` con el cuerpo por stdin, y comprobalo con `receipt verify` antes de votarlo — si no, re-postear con la misma etiqueta reemplaza el texto en SILENCIO y el recuento queda fiel a una discusión que ya no existe. Después posteá con musubi_debate action=post id=<debate_id> agent=<lente> stance=<veredicto + evidencia> model=<el del sub-agente> evidence=<deterministica|inferida|ninguna>. Ante la duda, que refute.\n" +
				...
				"8. EL BUCLE DE CORRECCIÓN TIENE TOPE, Y AGOTARLO ES UN RECHAZO. Si ganó no_real, corregís y volvés a juzgar, pero acotado: K=3 vueltas (2 si el cambio es trivial). La vuelta siguiente no es «una ronda más»: es UN DEBATE NUEVO, porque el tally con máximo estricto CIERRA el debate y sobre un cerrado post, vote y advance dan error. El estado viaja en el topic: «<el cambio> · vuelta k/K · abiertos: <lente#hallazgo, …> · previo: <debate_id>». ALCANCE DECRECIENTE: a la vuelta k+1 sólo van los hallazgos ABIERTOS. AL AGOTAR K el veredicto es RECHAZADO POR AGOTAMIENTO y escalás a una persona con el estado completo. EL CANSANCIO NO APRUEBA: el riesgo de un bucle sin salida no es girar para siempre, es que el agente ceda — justo lo que el paso 7 evita.\n" +
