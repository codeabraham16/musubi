...
func cognitiveSkills(stack []detector.StackResult) []skills.Skill {
	...
	return []skills.Skill{
		...
		},
		{
			...
			Description:   "Revisión adversarial con la evidencia primero: corré las comprobaciones y sabotéa los invariantes nuevos, y recién ahí debatí en rondas con escépticos de lentes distintos y veredicto por mayoría determinista.",
			Triggers:      []string{"*"},
			Capabilities:  []string{},
			...
			Rules: "Antes de dar por bueno un cambio de riesgo (o en la fase verify de un flujo SDD), sometelo a un DEBATE adversarial con veredicto determinista en vez de una sola lectura complaciente:\n" +
				"1. LA EVIDENCIA VA ANTES QUE EL DEBATE. Un panel que opina sin haber corrido nada es teatro de verificación: aprueba, y el CI falla después. Corré VOS las comprobaciones del proyecto (build, vet/lint, tests) y quedate con la salida real; NO le creas al implementador que pasaron. Revisá el ALCANCE (que sólo cambien los archivos que corresponden) y buscá trampa: tests deshabilitados o salteados, aserciones comentadas, invariantes aflojados para que pase. Si NO podés correr las comprobaciones, eso no es un aprobado: entrá al debate con esa carencia como refutación.\n" +
				"2. SABOTEÁ CADA INVARIANTE NUEVO. Rompé a propósito el código que la prueba dice cubrir y comprobá que se ponga en ROJO; si sigue verde, esa prueba no estaba midiendo nada. Dos trampas ya medidas acá: (a) la mutación VACUA —el cambio no altera el comportamiento que la prueba observa, así que el verde es correcto y no informa—, y (b) el SITIO EQUIVOCADO —la prueba mide bien, pero otra cosa: el caso real fue un banco que comparaba contra una base que no corre en producción y devolvía una mejora inflada—. Antes de concluir que 'el sabotaje no rompió nada', verificá que el archivo haya cambiado de verdad.\n" +
				"3. DIMENSIONÁ EL PANEL CON EL CAMBIO, NO A OJO. Corré musubi_detect_changes: además de qué cambió, devuelve `revision` con el NIVEL y el PANEL derivados de siete señales del propio diff (archivos, líneas, símbolos, paquetes, callers y paquetes en el radio de impacto, y si hay borrados), con el desglose para que puedas rehacer la cuenta. Usá ESE panel —mínima 1 juez/1 ronda/quórum 1, estándar 3/2/2, profunda 5/2/3— en vez del mismo tribunal para todo: un typo y una refactorización con cuarenta llamadores no merecen lo mismo, y revisar de más enseña a saltearse la revisión. Si `revision.radio_ciego` viene true, el grafo no cubre parte de lo tocado y el nivel ya subió por eso: NO lo bajes a mano, porque un radio 0 sin cobertura no dice «no arrastra a nadie» sino «no puedo saberlo».\n" +
				"4. Abrí el debate con musubi_debate action=open topic=<el cambio a juzgar> rounds=<las del panel del paso 3> quorum=<el del panel> gated_choice=real. Con gated_choice, un aprobado no cierra si NINGÚN voto declaró evidencia deterministica —rechazar no lleva compuerta, así que un panel que no pudo verificar nada igual puede refutar—. Guardá el debate_id. (musubi_debate estructura las rondas y CUENTA los votos sin sesgo; la inteligencia de refutar es tuya.)\n" +
				"5. Lanzá los sub-agentes escépticos que pida el panel (jueces) con el Task tool (mcpServers:[musubi]), cada uno con un LENTE distinto (correctitud, seguridad, ¿reproduce el bug?, contrato de la spec) y, si podés, con un MODELO distinto: dos jueces del mismo modelo no son dos opiniones. A CADA uno pasale la evidencia del paso 1 e instruílo a intentar REFUTAR el cambio desde su lente, posteando su hallazgo con musubi_debate action=post id=<debate_id> agent=<lente> stance=<veredicto + evidencia> model=<el modelo del sub-agente> evidence=<deterministica|inferida|ninguna>. El modelo va en su propio campo y NO dentro de agent: agent es la clave única, y un lente que cambia de modelo entre rondas dejaría de ser el mismo votante. Ante la duda, que refute.\n" +
				"6. Cerrá la ronda con musubi_debate action=advance id=<debate_id>: devuelve las posturas de todos los lentes. Pasáselas a los escépticos para una 2ª ronda de CRÍTICA CRUZADA (cada uno revisa las refutaciones ajenas y ajusta la suya con otro post) — es lo que una sola lectura no te da.\n" +
				"7. Recogé el veredicto: cada escéptico vota con musubi_debate action=vote id=<debate_id> agent=<lente> choice=<real|no_real> model=<el suyo> evidence=<deterministica sólo si CORRIÓ las comprobaciones>. LA POSTURA POR DEFECTO ES RECHAZAR: el que no pudo verificar su lente vota no_real, porque aprobar de más deja un cambio malo en main y rechazar de más cuesta una vuelta. Cerrá con action=tally: el recuento es DETERMINISTA y queda PERSISTIDO (reproducible). Si el tally da no_consensus (empate/sin quórum), deferí el juicio final a musubi_judge. Y si gana 'no_real' —la mayoría refuta— la corrección va por el paso 8, que tiene tope.\n" +
				"8. EL BUCLE DE CORRECCIÓN TIENE TOPE, Y AGOTARLO ES UN RECHAZO. Si ganó no_real, corregís y volvés a juzgar, pero acotado: K=3 vueltas (2 si el cambio es trivial). OJO CON LA FORMA DEL BUCLE: no es «una ronda más», es UN DEBATE NUEVO. El tally con máximo estricto CIERRA el debate —y no_real ganando es un ganador—; sobre un debate cerrado, post, vote y advance devuelven error y ninguna acción lo revive. El estado que sobrevive entre debates va en el topic, que es texto libre y action=status devuelve entero: topic = «<el cambio> · vuelta k/K · abiertos: <lente#hallazgo, …> · previo: <debate_id>». Así la cadena queda auditable sin tocar el esquema. ALCANCE DECRECIENTE: a la vuelta k+1 sólo van los hallazgos ABIERTOS, no todo de nuevo; re-litigar lo ya resuelto es lo que gasta las vueltas sin avanzar. AL AGOTAR K: el veredicto es RECHAZADO POR AGOTAMIENTO y escalás a una persona con el estado completo (las K cadenas y los hallazgos abiertos). EL CANSANCIO NO APRUEBA: el riesgo real de un bucle sin salida no es girar para siempre, es que el agente, cansado, apruebe — justo lo que el paso 7 evita con la postura por defecto.\n" +
				"9. Registrá lo aprendido con musubi_save_observation (topic_key 'review/<change>'); el debate ya conserva las posturas y el veredicto. Si el hallazgo contradice una memoria previa, resolvé la relación con musubi_judge.\n",
		},
		{
