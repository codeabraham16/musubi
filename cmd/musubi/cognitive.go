	...
	return []skills.Skill{
		...
		},
		{
			...
			Rules: "Antes de dar por bueno un cambio de riesgo (o en la fase verify de un flujo SDD), sometelo a un DEBATE adversarial con veredicto determinista en vez de una sola lectura complaciente:\n" +
				...
				"3. Abrí el debate con musubi_debate action=open topic=<el cambio a juzgar> rounds=2 quorum=<mayoría de los escépticos, p. ej. 2 de 3> gated_choice=real. Con gated_choice, un aprobado no cierra si NINGÚN voto declaró evidencia deterministica —rechazar no lleva compuerta, así que un panel que no pudo verificar nada igual puede refutar—. Guardá el debate_id. (musubi_debate estructura las rondas y CUENTA los votos sin sesgo; la inteligencia de refutar es tuya.)\n" +
				"4. Lanzá N sub-agentes escépticos con el Task tool (mcpServers:[musubi]), cada uno con un LENTE distinto (correctitud, seguridad, ¿reproduce el bug?, contrato de la spec) y, si podés, con un MODELO distinto: dos jueces del mismo modelo no son dos opiniones. A CADA uno pasale la evidencia del paso 1 e instruílo a intentar REFUTAR el cambio desde su lente, posteando su hallazgo con musubi_debate action=post id=<debate_id> agent=<lente> stance=<veredicto + evidencia> model=<el modelo del sub-agente> evidence=<deterministica|inferida|ninguna>. El modelo va en su propio campo y NO dentro de agent: agent es la clave única, y un lente que cambia de modelo entre rondas dejaría de ser el mismo votante. Ante la duda, que refute.\n" +
				"5. Cerrá la ronda con musubi_debate action=advance id=<debate_id>: devuelve las posturas de todos los lentes. Pasáselas a los escépticos para una 2ª ronda de CRÍTICA CRUZADA (cada uno revisa las refutaciones ajenas y ajusta la suya con otro post) — es lo que una sola lectura no te da.\n" +
				"6. Recogé el veredicto: cada escéptico vota con musubi_debate action=vote id=<debate_id> agent=<lente> choice=<real|no_real> model=<el suyo> evidence=<deterministica sólo si CORRIÓ las comprobaciones>. LA POSTURA POR DEFECTO ES RECHAZAR: el que no pudo verificar su lente vota no_real, porque aprobar de más deja un cambio malo en main y rechazar de más cuesta una vuelta. Cerrá con action=tally: el recuento es DETERMINISTA y queda PERSISTIDO (reproducible). Si el tally da no_consensus (empate/sin quórum), deferí el juicio final a musubi_judge. Y si gana 'no_real' —la mayoría refuta— la corrección va por el paso 7, que tiene tope.\n" +
				"7. EL BUCLE DE CORRECCIÓN TIENE TOPE, Y AGOTARLO ES UN RECHAZO. Si ganó no_real, corregís y volvés a juzgar, pero acotado: K=3 vueltas (2 si el cambio es trivial). OJO CON LA FORMA DEL BUCLE: no es «una ronda más», es UN DEBATE NUEVO. El tally con máximo estricto CIERRA el debate —y no_real ganando es un ganador—; sobre un debate cerrado, post, vote y advance devuelven error y ninguna acción lo revive. El estado que sobrevive entre debates va en el topic, que es texto libre y action=status devuelve entero: topic = «<el cambio> · vuelta k/K · abiertos: <lente#hallazgo, …> · previo: <debate_id>». Así la cadena queda auditable sin tocar el esquema. ALCANCE DECRECIENTE: a la vuelta k+1 sólo van los hallazgos ABIERTOS, no todo de nuevo; re-litigar lo ya resuelto es lo que gasta las vueltas sin avanzar. AL AGOTAR K: el veredicto es RECHAZADO POR AGOTAMIENTO y escalás a una persona con el estado completo (las K cadenas y los hallazgos abiertos). EL CANSANCIO NO APRUEBA: el riesgo real de un bucle sin salida no es girar para siempre, es que el agente, cansado, apruebe — justo lo que el paso 6 evita con la postura por defecto.\n" +
				"8. Registrá lo aprendido con musubi_save_observation (topic_key 'review/<change>'); el debate ya conserva las posturas y el veredicto. Si el hallazgo contradice una memoria previa, resolvé la relación con musubi_judge.\n",
		},
		{
