...
func atenderShellDelCerebro(comandoID, base, token string, argv []string) resultadoDeComando {
	...
	// Con el nombre TLS declarado (A129): este canal le habla al MISMO cerebro que el latido, y
	// hasta hoy era el hermano que no lo llevaba.
	cli := clienteHaciaElCerebro(nombreTLSDelCerebro(), esperaEntradaAgente+15*time.Second, nil)
