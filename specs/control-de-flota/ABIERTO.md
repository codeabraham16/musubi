...
>
> **Cabos abiertos hoy: 26.** Es la cuenta de las filas de la tabla 1, y no se escribe a mano:
...
| A127 | **El detector de alcance de la compuerta de guiones pregunta por CO-OCURRENCIA, no por gateo, y enumerar las formas de arrancar una shell no converge** | `TestElSalteoEstaAcotadoALasPruebasQueCorrenUnaShell` arma el grafo de llamadas y exige que toda prueba que ejecute una shell pase por `guiones.Exigir`, `guiones.Portable` o `guiones.Unix`. Lo que pregunta en realidad es si el IDENTIFICADOR aparece en el cuerpo, no si la compuerta gatea la shell: **tres fugas verificadas acá, cada una con control positivo** —la misma invocación de shell SIN compuerta sale roja y NOMBRADA, y con la fuga puesta el detector no la nombra—: (1) un `if false { guiones.Exigir(t, …) }` lo satisface y la shell corre afuera del `if`, o sea que lo que se mide es CO-OCURRENCIA del identificador y no que la compuerta gatee; (2) la shell nombrada por un `const` de paquete; (3) la shell adentro de una tabla `map[string]func(*testing.T)` de arneses, que es una forma común en este repo. La causa común es que el reconocimiento de «esto arranca una shell» sólo entiende `*ast.Ident` y `*ast.BasicLit` en el sitio de la llamada. **Se reportaron más formas —`var` de paquete, ruta armada con `+`, campo de struct, valor de retorno, `argv[0]` con `argv[1:]…`, `cmd.Path` asignado, un envoltorio tipo `timeout 30 bash -c`, `os.StartProcess`, un func literal de paquete— pero esas NO están verificadas acá y quien las tome tiene que medirlas de nuevo**: quien las reportó retiró después parte de su propia tanda al descubrir que un sabotaje que NO SE APLICA se ve idéntico a una guarda que no caza, y que había leído ese verde como una fuga. **La fuga está confirmada sobre una prueba REAL y no sobre una sonda fabricada**: a `TestLaPodaDePuntosDeRetornoHaceLoQueDice` se le sacó la compuerta y se movió el `"bash"` a un `const` de paquete; el detector quedó verde y la prueba siguió corriendo `deploy/pruebas/poda-puntos-de-retorno.sh`. Control: con el literal inline, el mismo borrado sale rojo. **LO QUE NO HAY QUE HACER ES CERRARLAS UNA POR UNA**: una ronda anterior cerró once y aparecieron catorce, y no fue por falta de cuidado — la pregunta está mal planteada. La salida es hacer IMPOSIBLE ejecutar una shell sin pasar por la compuerta (un ayudante del paquete que gatee Y ejecute, más una guarda de que ninguna prueba llame a `exec` directo), y entonces la guarda pregunta UNA cosa en vez de N. El criterio para saber si la salida sirve es ése: si después del cambio pregunta una sola cosa, convergió; si sigue preguntando N, se enumeró. **Hay antecedente y es la evidencia del argumento**: una rama que enumeró y cerró once formas —`fix/compuerta-segundo-orden`, nunca abierta como PR— produjo las catorce que quedan; quien la retome tiene que saber que ese camino ya se recorrió. **Dos de las fugas SÍ se cerraron acá** porque eran del propio paquete y salían baratas: la sonda que derivaba su expectativa de la constante que custodia (hoy el hecho se clava en `TestLosSistemasDondeCadaModoMideSonLosQueSeDeclaran`) y la lista de herramientas, que con un solo elemento hacía indistinguible recorrerla entera de recorrer `herramientas[1:]` mientras los llamadores reales declaran hasta seis. No se toca el resto en los PRs abiertos porque alcanza a todas las pruebas del repo que corren una shell, y eso convierte una entrega en un refactor. **2026-09-15 · LA FUGA (1) ESTÁ CERRADA Y LAS OTRAS DOS SE RE-MIDIERON.** Se verificó con control positivo, no leyendo el cabo: envolver la compuerta en un `if false` dejaba la guarda en VERDE (borrarla salía ROJO). Hoy el detector pregunta si la compuerta **domina** a la shell —sentencia directa de un bloque que encierra al `exec`, y antes que él—, así que caen esa forma y la de la compuerta escrita DESPUÉS del `exec`. **Ojo con el primer intento, que es la parte reutilizable**: pedir que la compuerta fuera sentencia del PRIMER NIVEL del cuerpo acusaba a ONCE llamadas sanas (las sondas de la propia compuerta la ponen adentro de su `t.Run`, junto con la shell) — se corrigió la regla, no se las exceptuó. **(2) SIGUE ABIERTA y también está medida**: una shell nombrada por un `const` de paquete es invisible porque `esLiteralDeShell` sólo acepta un `*ast.BasicLit`. Y queda el refactor convergente, que es lo que cierra el cabo: **el tamaño se pide, no se recuerda** —`grep -rEn 'guiones\.(Exigir\|Portable\|Unix)\(' --include=*_test.go . \| grep -v internal/guiones/`—: **el 2026-09-19 son 26 llamadas en 19 archivos**, sin contar las 6 sondas del propio paquete, con 19 `"bash"`, 6 `"powershell"` y 1 `"sh"` como primer argumento —`git`, `go` y `os.Args[0]` NO hay que absorberlos—, y el paquete hoy **sólo gatea, no ejecuta**: hace falta crear el ayudante Y una guarda que prohíba el `exec` directo. **La rama `fix/compuerta-segundo-orden` no tiene nada que rescatar**: sus dos hallazgos no-enumerativos (`Portable` con un `t.Skipf` al final, los controles de uso ausentes en `Portable`) YA ESTÁN en main — verificado, los tres modos tienen sus dos controles. | sin asignar |
| A129 | **Faltan dos agentes Windows para cerrar TLS, y el paso que los sigue no se puede adelantar sin dejarlos incomunicados** | **2026-09-19 — LO QUE YA ESTABA HECHO Y NADIE HABÍA MIRADO.** El TLS del cerebro no hay que montarlo: `tailscale serve` ya sirve HTTPS en el **10000** sólo al tailnet y reenvía a `http://127.0.0.1:7717`, con el certificado del tailnet que **se renueva solo**. Verificado de punta a punta CON control negativo: por nombre da 200 con certificado válido; discando la IP y forzando el nombre —que es exactamente lo que hace el agente con `MUSUBI_BRAIN_TLS_NAME`— también; y discar la IP **sin** declarar el nombre **falla**, porque el certificado de Let's Encrypt lleva sólo el nombre como SAN y ninguna IP. `POST /fleet/heartbeat` por ese camino contesta 401 con un token falso, o sea que llega y autentica. **POR QUÉ NADIE LO VIO**: el mensaje del verificador mandaba a poner `tls_cert_file` adentro del cerebro, así que el instrumento que MIDE la postura también RECOMENDABA la salida y su recomendación heredaba su punto ciego — el chequeo lee el config del cerebro y por diseño no puede ver un TLS terminado por delante. Corregido en el mismo día: hoy el bloque recolecta también `tailscale serve status --json` y la dirección de escucha, y `TestLaPerillaDeTLSDecideCorriendo` pasó de cinco casos a ocho. **EL AGENTE DEL PROPIO SERVIDOR NO HAY QUE MIGRARLO**: apunta a `http://127.0.0.1:7717` y su unidad lo dice — «El cerebro es LOCAL: el agente no sale a la red». Su bearer nunca toca la red. **LO QUE FALTA SON DOS VARIABLES POR MÁQUINA**, en `gio` y en `davantis-1`: `MUSUBI_BRAIN_URL=https://100.79.126.62:10000` y `MUSUBI_BRAIN_TLS_NAME=musubi-server.tail89e295.ts.net`. El nombre es OBLIGATORIO y no decorativo: se disca la IP porque con NordVPN el MagicDNS no resuelve. **EL ORDEN NO SE PUEDE INVERTIR, Y ES LA MITAD DEL CABO.** Atar el cerebro a `127.0.0.1` antes de migrar los agentes los deja incomunicados a los dos en el mismo instante — son Windows, sin ssh, y `davantis-1` ya estuvo dos horas sin latir el 2026-09-19. El orden es: migrar los dos agentes y verificar el latido de cada uno, DESPUÉS atar el cerebro a loopback, y RECIÉN AHÍ prender `MUSUBI_EXIGIR_TLS=1`. Prenderla antes da rojo sobre un despliegue correcto, y eso es lo que apaga un canal. **NO HAY BLOQUEO DE CREDENCIAL, Y LO PRIMERO QUE ESCRIBÍ ACÁ ERA FALSO.** Durante unas horas esta fila dijo que la máquina de desarrollo no tenía ninguna credencial con `fleet: exec` y que `~/.musubi/mando.token` era una credencial muerta. Las dos cosas eran mentira, y salieron del mismo error de medición: el listado de principales se pidió con un `head -30` y el recorte se leyó como ausencia. Medido de nuevo entero: `~/.musubi/token` y `~/.musubi/mando.token` son **el mismo valor**, escritos en el mismo segundo, y corresponden al principal **`davantis-2`**, que tiene `fleet:` con `exec`, `metrics`, `screen` y `shell` sobre `'*'`. Verificado con UNA sola llamada de lectura —no se prueba una credencial mandándola a ver si rebota: cinco 401 bloquean por IP también a la buena—: `GET /metrics` por HTTPS al 10000, discando la IP con el nombre forzado desde la máquina de desarrollo, contesta 200 con certificado válido. O sea que el camino TLS ya está probado también desde AFUERA del servidor, con una credencial real. **Higiene**: hay dos copias byte a byte de esa credencial en `~/.musubi/`, así que rotar una deja la otra. | **sin asignar** — se puede hacer desde la máquina de desarrollo; falta decidir cuándo, porque toca dos máquinas Windows |
