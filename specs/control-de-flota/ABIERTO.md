...
| A126 | **El `exit 2` por Prometheus inalcanzable deja SIN MIRAR las cuatro secciones siguientes, y eso esconde defectos durante meses** | `verificar-despliegue.sh` sale 2 en la sección «reglas de alerta» cuando `REGLAS_JSON` está vacío, o sea cuando no pudo hablar con Prometheus. Como veredicto está bien —«no vi» no es «está bien»— pero se lleva puesto todo lo que viene después. Medido sobre el árbol de hoy: quedan sin correr **`scrapes`, `alertmanager`, `versión` y `guiones derivados`**. Lo que eso cuesta ya se cobró una vez: de los cinco `$VAR` pegados a un carácter no-ASCII que mataban el guion en el bash 3.2 de macOS, **cuatro eran preexistentes** y nunca se habían visto, porque viven después del corte y CI no llega hasta ahí. **Ya está mitigado a medias y conviene no volver a medirlo mal**: `esquema de la base` y `unidades de systemd` fueron subidas POR ENCIMA del corte —junto con `corre_alla`, que las dos necesitan— así que esas dos ya no se pierden. Lo que falta es que las cuatro restantes dejen de colgar de que Prometheus conteste: acumular el veredicto y salir al final, en vez de salir ahí. | sin asignar |
