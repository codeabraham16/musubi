...
| A121 | **`TestSondaDiseno` no lo corre NADIE, y cuando corre pega 209 veces contra la central de PRODUCCIÓN leyendo el token del entorno** | Es la única de las 5 funciones invisibles en linux que CI tampoco rescata: no corre en `go test ./...` (build tag `sonda`) ni en ningún paso del workflow. Dos daños. **(1)** La sonda de calidad de `musubi_design` puede pudrirse sin que nada se ponga rojo. **(2)** El peor: `go test` está declarado «local» en la cabeza de cualquiera, y bajo `-tags sonda` NO lo es. **Se disparó sin querer el 2026-09-08** durante la auditoría de esta máquina, contra el cerebro real. Un tag que convierte una corrida local en tráfico a producción tiene que decirlo antes, no después. | sin asignar |
...
| A123 | **179 sabotajes ejecutables y ninguno miente; quedan 672 promesas que nadie puede correr** | **2026-09-15 · OCHO LOTES EN EL DÍA: 139 → 179 MECANIZADAS, COBERTURA 16,3 % → 21,0 %.** El censo (`go run ./deploy/cmd/arnes -validar`) da hoy **854 anclas en 196 archivos**: **179 mecanizadas**, **3 exentas** y **672 en prosa y nada más**. El trinquete `TestLaDeudaDeSabotajesNoCreceYElCorpusNoSePodre` (`internal/mcp/arnes_censo_test.go`) sigue sosteniendo las DOS direcciones —prohíbe que la deuda suba y que el techo se afloje más de 30— y falla además con directivas rotas, con directivas que dejaron de apuntar y con anclas sin ubicar. **EL VEREDICTO EN LOTE NO CAMBIÓ Y SIGUE SIENDO EL DATO QUE EL CABO PEDÍA**: corrida completa de `-correr` el 2026-09-15 (**27 min 3 s**), **138 en ROJO (sanas), 0 en VERDE, 0 motivos repetidos**; contra las 38 auditadas a mano en su momento, de las que **6 mentían (15,8 %)**, las mecanizadas van en **0 %**. **LO QUE SÍ CAMBIÓ ES LO QUE SE SABE DEL TRABAJO QUE FALTA, Y EMPIEZA POR CORREGIR UNA MEDICIÓN PROPIA QUE ERA FALSA.** Un filtro que buscaba la cadena `Sabotaje:` reportó que sólo el **34 %** de las anclas en prosa nombran una mutación concreta, y de ahí se seguía que dos tercios del cabo exigirían volver a decidir qué defecto custodia cada guarda. **Es falso y no hay que planificar con eso**: el árbol usa al menos ocho fórmulas —`Sabotaje que la hace fallar:` es la más común con **347**, `Sabotaje:` sólo **232**, más `que la pone roja:`, `verificado:`, `visto rojo:` y `SABOTAJE:` en mayúsculas con `⇒`— y medido bien **prácticamente las 672 ya nombran su sabotaje**. **El costo por ancla no es redactar la prosa: es encontrar un `de` ÚNICO DENTRO DEL ARCHIVO QUE LA DIRECTIVA NOMBRA, y un corte que COMPILE.** Los dos obstáculos, los dos medidos en estos lotes: (a) **bloques repetidos byte por byte** — el bloque del limiter de `fleet_http.go`, y hasta el comentario de veinte líneas que lo precede, son idénticos en las tres puertas (latido 124, resultado 642, salud 838), así que B6 quedó **sin mecanizar a propósito**, con los conteos y el motivo escritos en su propia ancla; (b) **un corte que deja un símbolo sin referencias vivas no compila**, y el arnés contesta «sin veredicto», que **NO es ROJO**: la guarda queda sin medir y la cobertura baja sin que nadie lo note. **DÓNDE ESTÁ LO QUE QUEDA** (medido hoy, tras el octavo lote): 21 en `cmd/musubi/flota_test.go`, 16 en `cmd/musubi/servicios_test.go`, 15 en `internal/mcp/fleet_cronologia_test.go`, `internal/mcp/fleet_otlp_test.go` e `internal/memory/servicios_test.go`, 14 en `cmd/musubi/agent_test.go` y en `internal/mcp/servicios_test.go`, 12 en `internal/fleet/exposicion_test.go` — **pero los ocho archivos más cargados suman sólo 122, y quedan 550 anclas repartidas en 150 archivos más**. El grueso ya NO está en lotes concentrados: está en la cola, y ahí el trabajo deja de ser por archivo. **OJO CON EL TECHO**: es un derivado del ÁRBOL ENTERO clavado en una constante, así que dos ramas que mecanizan un lote cada una pasan verdes por separado y la segunda en mergear rompe — los ocho lotes fueron secuenciales por eso. | sin asignar |
