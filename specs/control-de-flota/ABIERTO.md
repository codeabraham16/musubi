...
>
> **Cabos abiertos hoy: 25.** Es la cuenta de las filas de la tabla 1, y no se escribe a mano:
...
| A131 | **Que el sabotaje declarado encienda no prueba que no haya OTRO camino: 8 de 8 guardas auditadas tenían uno** | **2026-09-22 · SALE DE CERRAR A123.** Con el corpus al 98,6 % ejecutable, la pregunta deja de ser «¿esta guarda corre?» y pasa a ser **«¿se puede satisfacer SIN arreglar el defecto?»**. El arnés no la contesta: mide que el corte DECLARADO ponga la guarda en rojo, no que no exista otro cambio que rompa el mismo invariante y la deje verde. **PRIMERA RONDA (retícula de consentimiento y capacidades, `internal/fleet`): 8 de 8 guardas tenían al menos un segundo camino**, con 6 a 10 candidatos probados cada una. **LA FORMA ES SIEMPRE LA MISMA**: la guarda tabular generaliza sobre un eje y CLAVA el otro, y el agujero vive en el clavado — la matriz de consentimiento recorría los cuatro grados con `puede_preguntar` fijo en true; la de capacidades verificaba los 25 pares de `Implica` y su hermana probaba UNA pareja de `Permite`; la de ida y vuelta del parser nunca pasaba `screen:view`, que es la capacidad cuya asimetría es todo el punto. **DOS ERAN VIVOS Y SIN GUARDA EN TODO EL ÁRBOL** (cerrados en #622): el parser podía leer `screen:view` como `screen` —una fila que concede MIRAR pasaba a permitir CONTROLAR— y `Permite` podía inventar implicaciones que `Implica` niega. **SEGUNDA RONDA (eje de aprobación): 3 de 4 confirmados** (#626) — la dimensión «máquina» de `AprobacionVigenteDe` no la custodiaba nadie, el un-solo-uso no se medía por el camino de `pide`, y la compuerta del interruptor se probaba con un solo principal mientras `principals.yaml` admite `write: any` con independencia del rol. **CÓMO SE MIDE, porque el protocolo es lo reusable**: aplicar la mutación, `go build`, correr la guarda; si queda VERDE **todavía no es hallazgo** hasta que una sonda propia dé PASS sobre el árbol sano y FAIL sobre el mutado, porque una mutación VACUA también da verde. Y antes de arreglar, medir la EXPOSICIÓN: cuatro hallazgos de la primera ronda viven en un plegado multi-fuente que **hoy no corre** (los 12 llamadores de `ConsentimientoEfectivo` pasan cero extras), así que son huecos de la API exportada y no caminos vivos. **TERCERA Y CUARTA RONDA (2026-09-22): EL CANDADO Y LAS CREDENCIALES, LAS DOS QUE ESTA FILA NOMBRABA COMO PRÓXIMAS.** El candado: su análisis recorre TODAS las tools —eje libre— y clava el alcance, cruzando a UN paquete vecino de los trece que hacen I/O. Extendido a los trece, los nombres en el borde pasan de 3 a 23 y **cero** tools sin `lockSelf` se encienden; y extenderlo de verdad **invertiría la guarda**, porque el emparejamiento es por nombre SIMPLE y `provision.Run` choca con un `Run` que `internal/mcp` define. No se arregló: la medición quedó escrita al lado de la guarda (#628). Las credenciales: `TestNadieLeeUnaCredencialConGetenvPelado` miraba sólo `os.Getenv`, y `os.LookupEnv` saltea `SecretoDeEnv` —que es la que honra `<VAR>_FILE`— con la guarda en VERDE; arreglado cerrando el conjunto COMPLETO de lectores del API (#629). **Y LA REGLA QUE SALIÓ DE QUE LOS DOS TUVIERAN EXPOSICIÓN CERO Y CONCLUSIONES OPUESTAS**: con exposición cero se arregla cuando el arreglo es barato Y no puede producir falsos positivos; si puede, se escribe la medición y se espera. **DÓNDE APARECEN Y DÓNDE NO, que acota la búsqueda**: aparecen en las guardas TABULARES y en las que BARREN el repo enumerando formas; NO aparecen en las que custodian un invariante puntual y se escribieron con el hermano en la mano —la del redactor de credenciales se auditó entera y salió LIMPIA: su autor ya había buscado al hermano (`NuevaPassPantalla`), lo encontró roto, dijo por qué no se arregla ahí (la pass se dicta por teléfono) y lo generalizó a una clase en `internal/redact` en vez de dejarlo como nota—. **Y SE TIRÓ UNA GUARDA YA ESCRITA**: la que vigilaba el borde del candado pasaba, pero no se pudo construir el mundo que enciende su aserción principal —quitarle el `lockSelf` a `musubi_ingest_url` puso roja la guarda VIEJA, no la nueva, porque el handler llega al borde adentro de `internal/mcp`—, y una guarda verde en las dos direcciones se lee como cobertura. **LO QUE QUEDA**: las superficies tabulares y de barrido que no se miraron, por tamaño: `internal/mcp/fleet_otlp_test.go` (29 anclas), `cmd/musubi/flota_test.go` (22), `internal/mcp/servicios_test.go` (21), `internal/mcp/fleet_politicas_test.go` (19) y `internal/fleet/cronologia_test.go` (18). Y una advertencia de método: un fan-out de 30 minutos se pierde ENTERO si cae la red —pasó— así que conviene por lotes chicos, y si muere, **rescatar los diffs de los worktrees antes de limpiarlos**: de ahí salieron los tres de la segunda ronda. | sin asignar |
| A130 | **OpenObserve está publicado a internet por un Funnel, y NO hay que apagarlo: es cómo llega el ERP de Altura** | **2026-09-21 — MEDIDO ENTERO ANTES DE PROPONER NADA, porque la nota vieja lo listaba como «público, sin tocar» con tono de agujero pendiente.** La cadena es `Funnel :8443 (Tailscale) → http://127.0.0.1:5081 → Caddy (usuario `mon`) → OpenObserve`, y el Caddy contesta con `Access-Control-Allow-Origin: https://altura-erp20.vercel.app`. O sea que está publicado **a propósito**: es la vía por la que el frontend del ERP de Altura, que vive en Vercel y por lo tanto en internet, alcanza a OpenObserve. La unit se llama `caddy-rum.service` —RUM, *Real User Monitoring*—, que es más evidencia de lo mismo. **APAGAR EL FUNNEL ROMPE EL ERP**, y ésa es la conclusión que cambió al medir. **Que es público está probado, no supuesto**: `musubi-server.tail89e295.ts.net` resuelve en 1.1.1.1 y en 8.8.8.8 a `209.177.145.97` y `209.177.145.192`, IPs públicas del ingress de Funnel, no del `100.64/10`. **LO QUE QUEDA EXPUESTO SIN CREDENCIAL**, barrido endpoint por endpoint: `/web/` 200 · `/healthz` 200 · `/config` 200 · `/metrics` 200 · `/api/**` **401 ✔** · `/debug/pprof/` 401. `/metrics` entrega **88 series** (versión, memoria, caché, volumen de consultas); `/config` entrega la versión exacta `v0.91.1` y el commit hash. **NO hay nombres de streams, ni usuarios, ni datos**: es huella para buscar un CVE y patrón de actividad, no fuga de información. Severidad baja, pero real. **Y ACÁ ESTÁ LO QUE CASI SE ROMPE.** La propuesta obvia era que Caddy contestara 404 en `/metrics`, `/config` y `/debug/pprof`. Antes de entregarla se midió el bundle del frontend (`/web/assets/index-*.js`, 4,7 MB): **`/config` aparece 13 veces y `/metrics` CERO**. El frontend pide `/config` al arrancar, así que bloquearlo habría roto la UI que usa el ERP — el arreglo causando exactamente el daño que el «problema» no estaba causando. **Regla: antes de cerrar una ruta «que nadie usa», grepeá el bundle del cliente que la consume.** **EL ARREGLO, ACOTADO A DOS RUTAS**, va en `/home/mon/caddy-rum/Caddyfile` (la ruta salió de `/proc/<pid-de-caddy>/mountinfo`, que la da sin necesitar `podman inspect`): un matcher `@cerradas path /metrics /debug/pprof /debug/pprof/*` con `respond @cerradas 404`, y después reiniciar `caddy-rum.service`. **Se verifica con LAS DOS MITADES, porque una sola miente**: que las dos rutas den 404 **y** que `/web/` siga en 200 con `/config` todavía en 200; si `/web/` cae, revertir del respaldo. **POR QUÉ SIGUE ABIERTO: ES UN BLOQUEO DE PERMISO, NO DE CONOCIMIENTO.** El archivo vive en el home de `mon`, que es `0700`; desde `musubi` no se puede listar `/home/mon` ni correr `podman inspect` de sus contenedores, y el bind-mount está montado `ro`. `sudo -l` dice `(ALL) ALL` pero **con contraseña**, que queda del lado del usuario. **EL ARREGLO YA ESTÁ ESCRITO Y EJERCITADO: `deploy/cerrar-metrics-del-funnel.sh`.** Es un guion y no una receta en prosa porque el Caddyfile vive en el home de `mon` (0700), así que quien lo aplica NO es quien lo escribió, y una receta de cinco pasos ejecutada por otro es donde se pierde el paso 4. Respalda, **ABORTA sin tocar nada** si `reverse_proxy` no aparece exactamente una vez (no edita a ciegas un archivo que nadie de este lado pudo leer), es idempotente, valida con `caddy validate` ANTES de recargar, comprueba **las dos mitades** —las rutas en 404 **y** `/web/` todavía en 200— y **revierte solo** si el ERP deja de responder. Ejercitado contra Caddyfiles sintéticos en sus tres caminos: inserta dentro del bloque, no duplica al correrlo dos veces, y con dos `reverse_proxy` aborta dejando el archivo byte a byte igual. **LO QUE NO ES DE ESTE CABO**: subir OpenObserve de `v0.91.1` acota el leak de `/config`, que no se puede tapar sin romper la UI. | **acción del operador** — un comando, desde `mon` o root: `sudo -u mon bash deploy/cerrar-metrics-del-funnel.sh` |
...
6. **El número es la identidad: uno solo por cosa, y para siempre.** Un número nuevo va por encima
   del máximo en uso (hoy **A131** y **B21**) y NO se recicla uno libre: `A6`-`A9`, `A15` y `A16`
