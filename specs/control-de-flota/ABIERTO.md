...
| A123 | **La guarda de PowerShell delimita sus bloques con una secuencia que el bash normal escribe, y no filtra comentarios** | `TestNingunBloqueDePowerShellEscapaComillasConBarra` parte el archivo en bloques usando `"` seguida de `'` como separador. Un `trap 'rm -f "$X"' EXIT` —bash correcto y corriente— produce esa secuencia, así que abre un bloque falso y se come ~30 líneas que no son PowerShell; medido el 2026-09-10 sobre `comparar-y-latir.sh`. Y después volvió a dispararse sobre el **comentario** que explicaba el caso, porque tampoco saltea líneas de prosa. Las dos veces cambié mi código en vez de aflojar la guarda —la guarda cubre un defecto real— pero mientras el delimitador sea ése, cualquiera que escriba un `trap` normal va a ver un rojo que no le corresponde, y la salida no le va a decir por qué. | sin asignar |
| A124 | **El `exit 2` por Prometheus inalcanzable deja SIN MIRAR todo lo que viene después, y eso esconde defectos durante meses** | `verificar-despliegue.sh` sale 2 en la sección 2 si no puede hablar con Prometheus. Es correcto como veredicto —«no vi» no es «está bien»— pero se lleva puesto el informe entero: versión del cerebro, esquema de la base, unidades de systemd, guiones derivados. Lo que eso cuesta se midió el 2026-09-10: de los **cinco** `$VAR` pegados a un carácter no-ASCII que mataban el guion en bash 3.2, **cuatro eran preexistentes** y nunca se habían visto, porque viven después del corte y CI no llega hasta ahí. Se corrigió a mano para dos secciones nuevas (se subió `corre_alla` por encima del corte); el resto del guion sigue colgando de que Prometheus conteste. | sin asignar |
