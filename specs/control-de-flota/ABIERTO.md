...
| A2 | **Temperatura en Windows — CÓDIGO HECHO, falta correrlo en una máquina de verdad** | **2026-09-05: se implementó.** La premisa original («WMI desde Go sin dependencias es COM crudo») ya estaba anotada como caída en esta misma fila: desde **A42** el agente de Windows lanza PowerShell en cada latido y desde **A70** le pide `Win32_Service`, así que lo que quedaba era el costo de una invocación más, no la imposibilidad. `internal/fleet/tempwindows.go` (la aritmética, FUERA del build tag) + `temperaturaWindows` en el colector. **Se hizo AHORA por A79**, que dice con todas las letras que «la térmica es la única que la flota podría ver sola» y que hoy no puede porque `davantis-1` no reporta temperatura. **Dos decisiones que esta fila dejaba abiertas, ya tomadas:** (1) LO CACHEADO NO SE PUBLICA — entre lectura y lectura el campo va `nil`, porque la `Muestra` lleva UN solo `Tomada` y un valor de hace dos minutos viajaría con el sello de hace un segundo, que es la serie congelada fabricada por nosotros; (2) se lee **cada 2 min** y no cada latido —~300 ms de CPU por arranque de `powershell.exe` en la máquina de un cliente, y a 30 s serían ~2.900 arranques diarios—, y son 2 y no 5 porque la ventana de obsolescencia de Prometheus son 5 min y una serie que llega justo en el borde parpadea. **LO QUE FALTA, Y NO ES CÓDIGO**: correrlo en una Windows real. Se cobra al actualizar el AGENTE, no al redesplegar el cerebro. La mayoría de los escritorios NO publican `MSAcpi_ThermalZoneTemperature` —el firmware la expone sólo si el fabricante quiso—, así que el resultado esperado es `nil`; la diferencia es que entonces será una **medición** («esta máquina no expone sensor») y no una ausencia de código, y esa rama de A79 se podrá cerrar en vez de quedar abierta por no haber mirado. | **S4c** — falta medir en `davantis-1` y `gio` |
