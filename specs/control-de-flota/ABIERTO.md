...
>
> **Cabos abiertos hoy: 28.** Es la cuenta de las filas de la tabla 1, y no se escribe a mano:
> la cruza `TestElEncabezadoNoMienteSobreCuantosCabosHayAbiertos`. Hasta el 2026-09-19 acá decía
> **29**, que era cierto el día que se escribió y dejó de serlo en cuanto se abrieron A124 y A127
> — nadie vuelve a mirar el encabezado. Los números CON FECHA de más abajo no son ese caso: cada
> uno es verdadero para su entrada y se quedan como están.
>
> **Última revisión: 2026-09-10 (auditoría del propio registro, la tercera).** Quedaban entonces
> **29 cabos abiertos**, todos con dueño o razón declarada. Antes de esta revisión la tabla 1
> tenía 48 filas y **22 de ellas declaraban en su propia celda de estado que el cabo ya estaba
> cerrado**: la tabla que contesta «¿qué falta?» sobrestimaba lo pendiente en un 76 %, y dos de
> esas filas —**A99** y **A102**— estaban abiertas de verdad, escondidas detrás de la palabra.
> Las veinte que sí estaban cerradas bajaron enteras a la sección 3. Además: **A116 mandaba
> desplegar el esquema 48 y el repo está en 53** (y la 48 de hoy es otra migración); **A120 seguía
> abierto con el código ya cerrado**; **A92 tenía una celda de estado invisible** que contradecía a
> la visible; **la regla 4 era falsa** y señalaba a otro track mientras los ocho cabos sin registro
> estaban adentro de éste. Se abre **A123**. El detalle está en la sección 3, y desde hoy la regla 1
> tiene guarda: `TestNingunaFilaDeLaTabla1SeDeclaraCerrada`.
>
> **Revisión anterior: 2026-09-02 (auditoría del propio registro, la segunda).** Se le aplicó a este
...
> (`internal/selfupdate/updater.go`), así que la excepción por ruta debería sobrevivir a una
...
| A79 | **Una máquina que se reinicia sola no producía ninguna alerta, y el uptime se exportaba desde el principio** | `musubi_fleet_device_uptime_seconds` estaba en las 21 series de flota desde que existe el exportador y **ninguna de las 22 reglas lo miraba**. Encontrado el 2026-09-03 leyendo el registro de eventos de `davantis-1` mientras se perseguía un cuelgue: **trece apagones sucios en diez días** —evento 41, doce con `BugcheckCode=0`, o sea sin pantalla azul: a la máquina la cortaron—. La flota los vio todos y no dijo nada. **Y lo peor no es que faltara la alerta**: `MaquinaCaida` SÍ disparó cada vez y se resolvió sola a los pocos minutos, cuando la máquina volvía. Trece avisos que aparecen y se apagan solos se leen como ruido de red, no como «esta PC se está cayendo» — el patrón sólo existe si alguien lo cuenta, y contar es justo lo que un humano no hace. **Se cerró el mismo día** con `MaquinaSeReinicio` (`uptime < 1800` con la guarda de máquina caída, y su sección de runbook), que sube las reglas de flota a 23. **La primera versión de esa regla contaba (`resets(...[24h]) >= 2`) y no servía**: medida contra los datos reales antes de desplegarla, `resets` tocó un máximo de 1 en diez días —0 de 69 horas llegaron a 2— porque los cortes de esa máquina caen a 44 horas uno de otro, y agrandar la ventana tampoco iba porque el TSDB de este Prometheus arranca el 2026-08-31 17:44 (perdió su historia ese día, con retención declarada de 90 d). La versión que quedó se probó al revés: habría disparado en los DOS reinicios guardados y en ninguna otra muestra de tres días. **Lo que queda abierto es la causa, y no es de software**: `BugcheckCode=0` y cero errores de WHEA dejan afuera al sistema operativo —fuente, corriente de pared, térmica o un cuelgue duro—, y la térmica es la única que la flota podría ver sola. ~~**No puede**: `musubi_fleet_device_temperature_celsius` la reporta UNA sola máquina de tres, y `davantis-1` no es esa.~~ **2026-09-05 — MEDIO DESTRABADO, y lo que queda trabado es justo la máquina de esta fila.** Se implementó la temperatura en Windows (A2) y se corrió en hardware real: `gio` reporta **27,85 °C**, verificado contra una consulta directa a WMI en esa misma máquina (`CurrentTemperature=3010` decikelvin = 27,85 °C exactos). La serie pasó de **1 de 4 máquinas a 2 de 4**. Pero `davantis-1` **sigue afuera y no por falta de código**: su agente no se puede actualizar por el canal —ver A31, medido el mismo día—, así que la única causa de A79 que la flota podría ver sola sigue sin poder verse EN LA MÁQUINA QUE TIENE LOS QUINCE APAGONES. Los dos cabos se tocan acá: A79 espera una medición que A31 impide entregar. **2026-09-05, MÁS TARDE — ESTO QUEDÓ VIEJO Y LO CORRIGE A90: `davantis-1` SÍ reporta temperatura** desde las 01:14 de ese día, así que la serie está en 3 de 4 máquinas y el problema ya no es de canal. **Es peor: la serie llega y no puede ver nada.** Las dos Windows reportan `27.850000000000023` con `stddev_over_time` **exactamente 0** sobre toda su historia, mientras `davantis-1` iba de 5,9 % a 86,3 % de CPU en la misma ventana. Son 3010 decikelvin —una constante que el firmware publica y no mide—, el mismo número en dos equipos de hardware distinto. Así que la única causa de A79 que la flota podría ver sola sigue sin poder verse en la máquina de los quince apagones, y ahora se sabe que **no es por falta de código ni de canal**: el arreglo de la elección de zona no porta, porque ahí hay una sola zona ACPI y no hay nada mejor que elegir. Ver A90. | **gio** (el hardware) |
| A96 | **Hay DOS `config.yaml` y manda el del repo: leer el del home dice lo contrario de la verdad, con cara de evidencia dura** | El `musubi daemon` corre con `cwd` en el proyecto, así que la configuración que gobierna es `<repo>/.musubi/config.yaml` y **no** `~/.musubi/config.yaml`. Hoy difieren en lo que más importa para diagnosticar: en el del repo `sync.enabled` es **true**, con `central_url` y `auth_token_env: MUSUBI_TOKEN` y drain cada 30 s; en el del home es **false**. **Medido el 2026-09-05**, persiguiendo la tormenta de 401 de A88/A89: se leyó el config del home, se vio «el sync está apagado» y se concluyó «entonces los daemons no pueden ser los que golpean» — descartando la causa correcta, que eran exactamente los daemons. El costo fueron tres hipótesis falsas seguidas y un `kill` a dos procesos que no tenían la culpa. **Lo que hace peligroso a este defecto es que no se siente como una duda**: el archivo existe, se abre, el campo está ahí y dice algo terminante. Una lectura equivocada no falla, cierra la investigación. Es la misma familia que las dos `memory.db` —una vieja y una viva— que ya mordió antes, y que ninguna de las dos veces quedó registrada como cabo propio. Las salidas: (a) que `musubi doctor` diga CUÁL config está gobernando y señale las diferencias con el otro cuando existan los dos; (b) que el daemon loguee la ruta del config que cargó en su primera línea —hoy no la dice, y es un dato de una línea que habría ahorrado horas—; o (c) unificar, que es lo más limpio y lo más caro. La (b) es casi gratis y sirve aunque se hagan las otras. **2026-09-05 — LA (b) ESTÁ HECHA, y en su primer arranque en producción encontró el mismo defecto allá.** El cerebro ahora dice al arrancar la ruta ABSOLUTA del config que cargó y delata al que no gobierna; la primera línea del despliegue `0acf11a` fue: «configuración cargada de `/home/musubi/musubi-brain/.musubi/config.yaml`» y «OJO — también existe `/home/musubi/.musubi/config.yaml` y NO gobierna a este proceso». O sea que **el servidor tiene el mismo par de configs que la máquina de desarrollo**, y cualquiera que hubiera diagnosticado algo del cerebro abriendo el del home habría leído el que no manda — exactamente el error que abrió esta fila. No hizo falta buscarlo: el aviso lo cantó solo el primer día. **2026-09-05 — LA (a) YA ESTABA HECHA Y ESTA FILA NO LO DECÍA, y al ir a hacerla apareció otro cabo dentro de ella.** `musubi doctor --check config_que_gobierna` existe, nombra el que gobierna, detecta el otro y **lista las diferencias del bloque `sync`** (sin nombrar el token: `auth_token_env` es el NOMBRE de una variable, nunca su valor), y vive en `internal/memory` para que la CLI y la tool MCP contesten lo mismo. Corriéndolo acá contesta el par real de esta máquina. **PERO CERRABA CON «el que manda es el del directorio de trabajo», que es exactamente la afirmación que A109 midió como FALSA** —los daemons corren sin `MUSUBI_HOME` y con `CLAUDE_PROJECT_DIR`, así que la raíz sale de la variable y el cwd sólo coincide— **y que A109 corrigió en `cmd/musubi/main.go` y no acá**. La guarda que A109 escribió prueba el COMPORTAMIENTO de `workspaceDirConOrigen`, no que la afirmación no esté escrita en otro lado; por eso el hermano sobrevivió, en el check que existe justamente para contestar esta pregunta. Corregido: ahora dice lo que sabe —cuál gobierna— y apunta a la primera línea del arranque para la causa, porque el check recibe la raíz ya resuelta y no cómo se resolvió. **La guarda nueva `TestNadieLeDiceAlOperadorQueDecideElDirectorioDeTrabajo` recorre TODOS los `.go` de producción**, mira los LITERALES y no los comentarios (el defecto es lo que se le dice al operador; un comentario que explica la prohibición no manda a nadie a mirar el cwd) y junta todos los literales del archivo antes de buscar, porque un mensaje largo se escribe concatenado y partido en dos ninguno contendría la relación. **Y el sabotaje encontró un defecto en la propia guarda**: la primera versión buscaba la cadena exacta `manda el del directorio de trabajo` y el defecto real decía «manda ES EL del directorio de trabajo» — dos palabras de más, y la guarda **habría pasado en verde sobre la instancia que la hizo existir**. Ahora busca la relación y no la frase; sabotaje en rojo y su otra dirección en verde. **Sigue abierta sólo la (c)** —unificar—, que es la más limpia y la más cara, y arrastra que `internal/memory/database.go` usa la PRESENCIA del archivo como señal de política de outbox, así que hay que mover ese gate en el mismo commit. | **gio** (falta sólo la (c): decidir si la quiere) |
| A90 | **La temperatura de las dos Windows es una constante del firmware: la serie existe, llega puntual, y no puede ver nada** | El arreglo de la zona térmica en Linux (`b97a81c`) destapó que las otras dos máquinas tienen el mismo síntoma y peor causa. **Medido el 2026-09-05 sobre toda la historia disponible de cada serie**: `min_over_time == max_over_time` para las dos, `27.850000000000023` en los dos extremos, y `stddev_over_time` **exactamente 0** a 2 h y a 7 d — contra 2,9 de `musubi-server`, que después del arreglo se mueve entre 31 y 46. **La excusa de la ociosidad queda descartada por medición**: en la misma ventana con cobertura térmica, `davantis-1` fue de **5,9 % a 86,3 % de CPU** —pico a las 01:53:56, con muestras térmicas a 01:52:48 y 01:56:48, las dos idénticas hasta el último dígito— y la temperatura no movió nada. 27,85 °C son **3010 decikelvin exactos**, el MISMO número en dos equipos de hardware distinto (`gio` 24 núcleos, `davantis-1` 8). **No es el defecto que se arregló en Linux y por eso no hay arreglo barato**: allá había varias zonas y el código elegía la peor (`acpitz` sobre `x86_pkg_temp`), y se resolvió eligiendo; acá el registro anota **una sola zona ACPI** (`ACPI\ThermalZone\TZ00_0`, `CurrentTemperature=3010`), así que no hay nada mejor que elegir — es un valor que el firmware publica y no mide. La aritmética está bien: `3010.0/10 - 273.15` da exactamente `27.850000000000023` y `internal/fleet/tempwindows.go` lo calcula así, sin constante empotrada. **La consecuencia es que A79 sigue trabado en la máquina de los quince apagones**: `TemperaturaAlta` (`> 85` con sus guardas, `for: 10m`) está en `inactive` y no puede disparar en ninguna de las dos. Y dos textos quedaron viejos: la anotación de la alerta dice que en Windows la serie está **ausente** —no lo está, evalúa siempre falsa, que es peor— y A79 dice que `davantis-1` «sigue afuera» cuando **reporta térmica desde las 01:14 de ese día**. Lo que falta decidir es si vale la pena otra fuente (LibreHardwareMonitor, IPMI, un sensor físico) para una máquina sola, o si la causa de A79 se persigue por afuera de la flota. | **gio** (el hardware, y si se paga otra fuente) |
| A92 | **`davantis-1` tiene DOS agentes latiendo sobre la misma fila, y eso deja a `AgenteDesactualizado` estructuralmente incapaz de disparar justo ahí** | Seis lecturas seguidas de la misma fila (sqlite en modo `ro`, cada 8 s) alternan versión: `0.131.0-flota.49b26b2` y `0.130.0-flota.e140e0c`, con **caps, `puede_preguntar`, `os` y `arch` idénticos** — es la misma fila con dos escritores. `last_sample.tomada` muestra **dos cadencias de ~32 s entrelazadas**, y hay dos muestras a **3 s** una de otra (05:59:19 con una versión, 05:59:22 con la otra). Qué tan viejo es el binario de más: `git rev-list --count e140e0c..b97a81c` = **150 commits**, del 2026-09-02, y `git merge-base --is-ancestor e140e0c a243281` da SÍ — es anterior incluso al cerebro que se acaba de reemplazar. **El efecto sobre la alerta es el que importa**: `changes(musubi_fleet_device_agent_stale[20m])` da **8** para `davantis-1` (contra 1 en `gio` y 0 en `musubi-server`), y de 40 muestras en veinte minutos **17 dicen «desactualizada»** — el 42 %. Pero la serie vuelve a 0 cada ~minuto, así que **nunca acumula las 6 h continuas que pide `for: 21600`**: la única alerta diseñada para avisar «esta máquina quedó atrás» no puede disparar en la máquina que tiene un binario de 150 commits atrás. `/api/v1/alerts` tiene un `pending AgenteDesactualizado` y es de `gio`; `davantis-1` no aparece. **Y hay un segundo daño más callado**: `agent_version` y `last_sample` los escriben procesos distintos, así que la fila es un mezclado y el par no es coherente — cualquier lectura que los cruce (por ejemplo «por qué versión falta este campo») miente. Es el mismo modo de falla silencioso de siempre, en el eje de versiones: nada rojo, todo verde, el eje apagado. Y es la máquina de A79. **La salida no es obvia y no es de software**: hay que entrar a esa Windows, ver qué dos servicios/tareas están corriendo `musubi` y matar el viejo — probablemente un residuo del intento de actualización de A31, donde el binario se reemplazó por un camino y el lanzador quedó apuntando al otro. Requiere permiso explícito sobre esa máquina. **2026-09-05 — CERRADO. gio autorizó enumerar y matar, y el zombi murió a las 14:5x.** La causa quedó MEDIDA y no inferida, que es lo que cambió respecto de la primera lectura. Los tres agentes decían lanzarse desde el MISMO `...\Local\Musubi\musubi.exe`, y la frontera es la fecha de escritura de ese archivo: **01:13:15 del 05/09**. PID 7180 arrancó el **04/09 a las 17:43:22 —ANTES—** así que corría una imagen que ya no existe en disco; los otros dos (01:14:38 y 09:13:06) arrancaron después. Lo que lo cierra es que **`musubi.exe` reporta `49b26b2`** y el proceso que escribía `e140e0c` arrancó antes de que ese archivo existiera con ese contenido: es el mecanismo que `cambiar-agente.cmd` documenta —el paso [2] renombra el binario en uso y el proceso viejo sigue vivo con la imagen renombrada—, confirmado por medición. **CORRECCIÓN DE UNA MEDICIÓN MÍA, del mismo día.** Escribí acá que «NINGUNO de los tres binarios reporta `e140e0c`» y que la versión vieja «estaba sólo en la memoria del proceso». **Las dos afirmaciones eran falsas.** `musubi.exe.viejo` **ES** `0.130.0-flota.e140e0c` — lo destapó la otra sesión al no poder medirlo y decirlo. Mi bucle de PowerShell hacía `$v = & $p version` con `$ErrorActionPreference='SilentlyContinue'`, y para un archivo cuya extensión no está en `PATHEXT` el `&` lanza «Cannot run a document in the middle of a pipeline»: **la asignación no se ejecuta y `$v` conserva el valor de la iteración ANTERIOR**. Así que reporté la versión de `musubi.exe` como si fuera la del `.viejo`. Una medición con la forma correcta, en el lugar correcto, contestando la pregunta previa — la misma familia que todo lo demás de hoy. Verificado por tres caminos: `&` (excepción), `Start-Process` con `-NoNewWindow` (que va por CreateProcess y **no** mira la extensión) y copiar el archivo a `.exe` (control): los dos últimos dan `e140e0c`, con el sha del original. **La conclusión de A92 no cambia** —el zombi era el 7180 y matarlo estabilizó la fila— pero el relato sí: la versión vieja SÍ está en disco, como `musubi.exe.viejo`, y eso es más simple y más creíble que «existía sólo en memoria». Lección de método: `-ErrorActionPreference` en `SilentlyContinue` dentro de un bucle convierte un fallo en un dato viejo. **Después de matar el 7180, la fila reporta `49b26b2` estable en tres muestras**: dejó de alternar, y con eso `AgenteDesactualizado` recupera la capacidad de disparar. **DOS COSAS QUE VALE DEJAR ESCRITAS.** (1) La máquina se cayó sola en medio de la medición —apagón 16 de A79, visto en vivo con `tailscale status` como fuente independiente del agente— y **NO reinició**: al volver, los tres PIDs eran los mismos, así que perdió red y no energía. Eso refutó una hipótesis de la otra sesión y una inferencia mía, las dos razonables y las dos falsas. (2) El comando de kill quedó `entregado` y NUNCA reportó resultado, porque **el agente que lo tomó era el 7180 y se mató a sí mismo antes de poder contestar**. La guarda que le puse —abortar si la ruta o la hora de arranque no coinciden— nunca hizo falta, pero era la decisión correcta por un motivo que no había previsto: el primer intento se encoló 65 s DESPUÉS de que la máquina ya estuviera muerta, y habría corrido contra PIDs de otra vida. **QUEDA UNA COSA SIN HACER, y es otra decisión**: siguen DOS agentes latiendo sobre la misma fila (17016 y 5040), los dos con el binario nuevo. Ya no hay versión que alterne, pero son dos escritores para una fila. Consolidar a uno no está cubierto por «matar el viejo». **2026-09-10 — ESTA FILA TENÍA UNA CELDA DE ESTADO INVISIBLE, y decía lo contrario de la que se ve.** Adentro del cuerpo había un `\| **gio** ✔ cerrado; queda consolidar a un solo agente \|` con las barras ESCAPADAS: no se renderiza como celda, así que el registro mostraba `— (cerrado)` mientras el texto de al lado admitía un pendiente. Es el defecto de B20 y A112 otra vez —una celda que existe en el archivo y no existe para quien lo lee—, y acá tapó un cabo abierto en vez de una palabra. La fila SE QUEDA en la tabla 1: lo que su título denuncia está arreglado, pero lo que su propio cuerpo declara sin hacer —«siguen DOS agentes latiendo sobre la misma fila»— no lo está, y no es código: es una decisión sobre una máquina de gio. | **gio** — falta consolidar a un solo agente en `davantis-1` |
| A94 | **Tres de las cuatro máquinas dicen que no saben preguntar, `gio` incluida: hoy `avisa` no le avisa a nadie y `pide` sería un candado total** | El eje de consentimiento tiene cuatro grados (`libre` < `avisa` < `pide` < `prohibido`) y los dos del medio dependen de que la máquina pueda hablarle a quien está sentado adelante. **Medido el 2026-09-05**: `puede_preguntar` es false en tres de las cuatro máquinas de la flota, y una de ellas es `gio` — la máquina de escritorio donde alguien está efectivamente sentado. Consecuencia: en esas tres, **`avisa` es indistinguible de `libre`** (nadie recibe el aviso) y **`pide` es indistinguible de `prohibido`** (no hay a quién preguntar, así que la respuesta nunca llega). El eje tiene cuatro grados y en tres de cuatro máquinas ofrece dos. Esto NO es una falla del eje —el diseño ya contempla degradar cuando no se puede preguntar, y `ConsentimientoEfectivo` lo hace— pero convierte en decorativa la mitad de la matriz justo en las máquinas donde importaría, y hace que A86 (endurecer `exec` bajo `pide`) tenga en la práctica el efecto de `prohibido` y no el de «pedile permiso». **Se junta con A91**: ahí `pide` no frena al auto-heal, acá `pide` tampoco puede preguntarle a nadie, así que el grado no significa lo que su nombre dice por dos motivos distintos y hay que arreglar los dos para que signifique algo. Lo que falta es decidir si el avisador se despliega en `gio` —hay código (`cmd/musubi/avisador.go`) y A57 lo cerró— o si se acepta que en esta flota el eje tiene dos grados y se documenta así en vez de ofrecer cuatro. **2026-09-05, MÁS TARDE — CORRIJO ESTA FILA: LA ESCRIBÍ YO Y EXAGERABA, y la corrección la hace MÁS puntiaguda, no menos.** «Tres de las cuatro» es numéricamente cierto y engañoso, porque dos de esos tres NO son un hueco: `altura-db` es **Tier B**, no corre nuestro binario y no tiene con qué preguntar por construcción; `musubi-server` es un servidor sin escritorio, donde `avisa` y `pide` no significan nada porque no hay nadie sentado. Contarlos infla el problema y esconde el real. **Sólo DOS máquinas podrían preguntar alguna vez —las dos Windows de escritorio— y una puede y la otra no**: `davantis-1` reporta `puede_preguntar = true` y `gio` `false`. Mismo SO, mismo tier, mismo rol. **Y `puede_preguntar` NO es configuración: es una capacidad MEDIDA** (`cmd/musubi/avisador_windows.go`), que comprueba la sesión del proceso con `ProcessIdToSessionId` — un servicio de Windows corre en la SESIÓN 0, aislada del escritorio, donde un `MessageBox` no falla y no lo ve nadie. **Así que «desplegar el avisador» NO arregla nada: el avisador YA ESTÁ en el binario y es justamente lo que reporta `false`.** Lo que hay que cambiar es CÓMO se lanza el agente en `gio` —de servicio a Tarea Programada `-AtLogOn`, en la sesión del usuario— que es exactamente lo que `davantis-1` hace y lo que el propio mensaje de error del código recomienda con esas palabras. Medido en `davantis-1`: `\Musubi Agente de Flota :: wscript.exe //B agente-oculto.vbs`, tarea programada, no servicio. Es la forma dominante de defecto de este track otra vez, esta vez en el DESPLIEGUE y no en el código: la cautela aplicada en una máquina y ausente en su hermana. | **gio** (autorizar cambiar el lanzador del agente en SU máquina) |
| A95 | **La guarda del sobre de llamada la tiene el central y no el binario local, así que el defecto sólo aparece como dead-letter que nadie lee** | La guarda de `internal/memory/sobre_de_llamada.go` —que impide guardar una observación con el cierre de su propia llamada pegado adentro del `content`, perdiendo `mem_type` e `importance` en silencio— vive en `saveObservation` del cerebro. **Un binario local más viejo no la tiene**, así que guarda la fila mal formada sin protestar, y recién falla cuando el sync intenta empujarla al central, que sí la rechaza. **Medido por otra sesión el 2026-09-05: 8 filas así.** El problema no es el rechazo —es correcto— sino **dónde aparece**: en el `dead-letter` del outbox, con el motivo en `last_error`, una superficie que nadie mira y que no tiene alerta. Es la misma forma que A88/A89: el sistema sabe qué pasó, lo anota, y la anotación no llega a ningún ojo. Y es la forma inversa de la familia que este track persigue: acá no es que N caminos deberían hacer lo mismo y N-1 lo hacen — es que **el camino estricto y el permisivo están en máquinas distintas**, así que la validación depende de qué binario te tocó. Las salidas son dos y no son excluyentes: (a) que el chequeo del doctor `swallowed_envelope` corra también localmente y no sólo contra el central, y (b) que el dead-letter del outbox tenga alerta —hoy creció de 7 a 10 en una hora con `sent` congelado en 2034 y nadie se enteró—. Medido y reportado por `musubi-ce`; queda acá porque cruza los dos lados. | (sin asignar — (b) es la que hace visible a todas las demás) |
| A116 | **El latido lleva un TECHO de 64 servicios y el recorte NO viaja, así que el cerebro lee una lista truncada como si fuera el inventario completo — y revoca lo que no vino** | Encontrado el 2026-09-05 limpiando 24 contenedores parados que gio pidió sacar de `davantis-1`. Al verificar quedó a la vista lo otro: **los 11 contenedores de `altura-erp` que están CORRIENDO no se reportan, y nunca se reportaron.** El mecanismo es `serviciosParaElLatido` (cmd/musubi/servicios.go): ordena por `prioridadDeReporte` —fallado 0, detenido 1, corriendo 2, desconocido 3—, **a igualdad desempata por NOMBRE**, y corta en `fleet.ServiciosPorLatido = 64`. `davantis-1` reporta exactamente 64: está EN el tope, o sea truncando. Los 24 supabase parados se comían el presupuesto **por estar parados** —prioridad más alta— y desplazaban a los que funcionan; borrados ésos, los de `altura-erp` siguen afuera porque `supabase_*` empieza con `s` y pierde el desempate alfabético contra el catálogo de servicios de Windows. **O sea que lo primero que se deja de ver es lo que anda bien, y en una máquina con muchos servicios eso incluye el stack de producción entero.** **LA PARTE QUE LO VUELVE INVISIBLE:** `fleet.ReporteServicio` lleva sólo `Nombre`, `Clase` y `Salud` — **no lleva «me quedaron N afuera»**. Del lado del cerebro, una lista truncada de 64 es INDISTINGUIBLE de un inventario completo de 64. El agente sí lo dice, con `avisarUnaVez`, en su propio log, en la máquina, una vez por arranque del proceso: donde nadie mira. **Y LA PODA LA TOMA COMO AUTORIDAD**, que es lo que convierte una ceguera en una afirmación falsa: `PodarServiciosAusentes` hace `UPDATE services SET revoked = 1 WHERE device_id = ? AND revoked = 0 AND declared = 0 AND name NOT IN (vivos)` — **global por máquina, no por clase**. Así que el cerebro no sólo no ve `altura-erp`: **anotó que ya no está**. Medido: 26 servicios de clase `docker` y 87 de Windows marcados `revoked = 1` en esa máquina por la misma rotación. Y hay una trampa al diagnosticar esto: `musubi_fleet_services` **esconde los revocados por default**, así que sin `incluir_revocados: true` parece que se borraron y no que se marcaron. **QUIÉN DECIDE CON ESA LISTA:** `ServicioCaido`, `ServicioSinNoticias` y `MaquinaSinInventario` la leen como si fuera el inventario de la máquina. La guarda que existe —`TestPodarServiciosAusentesConListaVaciaNoBorraNada`, el early-return de `len(vivos) == 0 && !vacioAfirma`— cubre la lista TOTALMENTE vacía y no un hueco con forma de clase o de prioridad: es la lección aprendida de un lado y no del hermano, otra vez. **EL ARREGLO NO ES EL NÚMERO 64.** Un techo es defendible —un latido no puede crecer sin límite— y discutirlo es una decisión de producto. Lo que no es defendible es el silencio: un conteo que no sale de la máquina convierte «vi 64 de 76» en «hay 64», y a partir de ahí todo lo que se apoye en esa lista decide con un dato falso que se ve sano. Lo que hace falta es que el recorte VIAJE: un campo en el reporte, una serie, y una alerta — con eso el problema deja de ser invisible en toda la flota y no sólo en la máquina donde se lo encontró de casualidad. Una alternativa complementaria, más barata y de menor alcance, es filtrar qué servicios de Windows vale la pena enumerar: 64 servicios de Windows en un escritorio son casi todos ruido del sistema operativo. **Cómo se encontró importa**: no lo mostró ninguna guarda ni ninguna alerta. Lo mostró contar —`len(services) == 64` contra un tope que es exactamente 64— después de que una limpieza pedida por otra razón cambiara los números. Igual que A115: la conducta era lo único que lo decía. **2026-09-05 — CERRADO EN EL REPO. gio pidió LAS DOS, y son dos cosas distintas que conviene no mezclar.** **(1) EL RECORTE VIAJA, Y UNA LISTA RECORTADA YA NO AUTORIZA A PODAR** — que es la mitad de correctitud. El agente manda `servicios_omitidos` con el inventario (y el número entra en la HUELLA: si no, una máquina que deja de truncar sin que cambien los 64 visibles se quedaría con el valor viejo del otro lado). El cerebro lo guarda en `devices.servicios_omitidos` (esquema **48**) y **suspende la poda mientras haya recorte**: no se puede podar «sólo los que vinieron» porque el cerebro sabe CUÁNTOS faltan, no CUÁLES, así que la única respuesta correcta es no podar — lo que llegó se guarda igual, y se apaga la única operación que afirma algo sobre lo que NO llegó. Se escribe SIEMPRE, incluido el 0: un número que sólo se escribe cuando hay problema no puede decir que el problema se fue, y la poda quedaría suspendida para siempre sobre una condición que ya no existe. Sale por `musubi_fleet_device_services_omitted` —**siempre presente, incluido el 0**, al revés que `token_rotable`, y el porqué está escrito al lado: allá el 0 sería una acusación que nadie midió, acá es lo que hace hoy un agente viejo— y la mira `InventarioDeServiciosIncompleto`, con su sección de RUNBOOK. **Es la regla que YA ESTABA ESCRITA en `cmd/musubi/servicios.go` para una fuente rota** —«el inventario se manda COMPLETO o NO SE MANDA»— aplicada al hermano que se había quedado afuera; acá no se puede no mandar, porque el techo es permanente y esa máquina no reportaría nunca, así que la lista viaja anotada y lo que se apaga es la poda. **(2) EL ALCANCE DE WINDOWS, que es distinto del techo y la distinción importa.** El techo corta por prioridad y desempata ALFABÉTICAMENTE: qué se pierde depende de cómo se llame y cambia solo — arbitrario y silencioso, y por eso se cuenta. Lo nuevo es una regla de alcance, estable y escrita en el RUNBOOK: **la plomería del sistema operativo que está corriendo bien no se enumera**, reconocida por dónde EJECUTA (`PathName` bajo `<unidad>:\\Windows\\`) y **no por una lista de nombres**, que se pudre con cada versión y esconde lo que no previó (A93). Dos cosas que esa regla NO hace, las dos deliberadas y las dos con su aserción: la plomería **rota** sí viaja —un `Dhcp` detenido es justo lo que hay que ver— y un servicio **sin `PathName` se reporta igual**, porque esconder algo por un campo que faltó es indistinguible de que no exista. Medido sobre los 65 que reportaba `davantis-1`: la mayoría era `RpcSs`, `DcomLaunch`, `EventLog`, `Winmgmt`, `gpsvc`, y contra el tope quedaban apretados `Tailscale`, el antivirus y los contenedores. **Cuatro sabotajes, cuatro rojos con mensajes distintos**, y uno de los hallazgos fue de la prueba contra sí misma: el CSV de prueba estaba mal escapado, el parser devolvió `nil`, y **dos aserciones de la forma «X no está» pasaron por AUSENCIA y no por acierto** — ahora hay un control que corta si el parseo no produjo nada. **Lo que falta es despliegue y no trabajo**: el esquema 48 necesita redesplegar el cerebro, y el alcance de Windows necesita actualizar el agente de `davantis-1`. **2026-09-10 — «ESQUEMA 48» ESTÁ OBSOLETO POR CINCO MIGRACIONES Y ADEMÁS SEÑALA OTRA COSA.** Verificado en `f9be15a`: la **48** de hoy es `schema_read_floor`, y `devices.servicios_omitidos` —la columna de esta fila— vive en la **51** (`cuantos_servicios_no_entraron`). Las 49…53 entraron después de la última edición de este registro y nada las custodiaba desde acá: el registro mandaba desplegar un número que el repo ya había pasado y que además nombra otra migración. EL OBJETIVO NO SE ESCRIBE COMO NÚMERO: es el esquema VIGENTE del repo, que sale de `internal/memory/migrations.go`. Un número acá es un derivado escrito a mano, o sea una copia — y se queda viejo con la próxima migración, que es exactamente lo que esta fila denuncia. **2026-09-16 — EL BLOQUEO QUE ESTA FILA DECLARA ESTÁ SATISFECHO, Y SE MIDIÓ EN EL SERVIDOR, NO SE SUPUSO.** `deploy/verificar-despliegue.sh` corrido hoy contra `musubi-server` —vía `musubi-comparar.service`, que es el instrumento que el repo tiene para esto y ya corre por timer— contesta **«✔ la base está en el esquema 57, que es al que apunta el binario»**. Ese chequeo no lee nada local: saca el número del binario del cerebro con `musubi version --esquema` y el de la base leyendo los bytes 60-63 de `/home/musubi/musubi-brain/.musubi/memory.db`, los dos EN EL SERVIDOR. Y 57 es también la última migración del repo (`expansion_como_relevancia_elegida`). La columna `devices.servicios_omitidos` entra en la migración **51**, así que a esquema 57 YA EXISTE. **LO QUE NO SE VERIFICÓ, Y POR ESO NO SE CIERRA LA FILA**: que el dato VIAJE hasta la herramienta. La columna existe, el código que la escribe y la expone está en el repo, pero comprobar que `musubi_fleet_list` la muestre para cada máquina es una consulta AL CEREBRO CENTRAL, y el MCP de esta sesión es el cerebro LOCAL (su `musubi_fleet_list` devuelve la flota vacía, que es correcto: acá no hay flota). Queda como el único paso pendiente, y es una lectura, no un despliegue. **LA OTRA MITAD DE ESTA FILA NO SE TOCÓ**: sigue faltando el agente nuevo en `davantis-1` —el del alcance de Windows— y eso no se puede verificar desde acá. **2026-09-16 · LA CADENA ENTERA VERIFICADA, ESLABÓN POR ESLABÓN.** (1) El servidor corre el esquema **57** —medido con `verificar-despliegue.sh` contra `musubi-server`, que lee el binario y el header de la base ALLÁ—, y las columnas entran antes (50 y 51). (2) `internal/memory/devices.go` las trae en `columnasDevice`. (3) `internal/mcp/methods_fleet.go` las expone en `musubi_fleet_list`, y **sólo cuando no están vacías**, que es lo correcto: un campo vacío que igual sale se lee como un hecho. (4) Y el eslabón que no se puede fingir: `verificar-cobertura`, que DERIVA del Prometheus del servidor, reporta estas dimensiones como **cubiertas en las tres máquinas Tier A** y las declara como límite estructural sólo en `altura-db`, que es Tier B y no corre agente. Una dimensión no puede figurar cubierta si su serie no existe. **Para A116**: `devices.servicios_omitidos` (migración 51) existe, y la serie `musubi_fleet_device_services_omitted` la consume `InventarioDeServiciosIncompleto` con `> 0`, `for: 1h` y las mismas exclusiones. **De las dos cosas que esta fila mandaba desplegar, la del cerebro está hecha.** Queda la otra, que no se puede verificar desde acá: el agente nuevo en `davantis-1`, el del alcance de Windows. | **gio** — falta el agente nuevo en `davantis-1` (el del alcance de Windows) |
| A117 | **Faltan dos menores del respaldo off-host: que registre identidades cuando haya `principals.yaml`, y una copia huérfana del guion que puede envejecer en silencio** (el respaldo en sí quedó verificado el 2026-09-16, y en el camino aparecieron un binario que no podía respaldar su propia base y una unidad cuyo `ExecStart` apuntaba a un archivo sin bit de ejecución) | **2026-09-16 · CERRADO.** Primero, dos correcciones al diagnostico del 2026-09-08, las dos medidas: (a) los respaldos NO estaban donde el cabo miraba — van junto a la base (`filepath.Dir(e.path)/backups`), o sea `~/musubi/.musubi/backups`, no `~/.musubi/backups`; habia tres, el mas nuevo del 2026-09-11, no uno de 44 dias. (b) Los dos checks del doctor YA eran honestos: `checkOffhostBackup` y `checkSnapshotLocal` devuelven `Unmeasured: true` con el comentario «no se afirma un 'ok' verde (seria mentir)». Lo que faltaba no era codigo del doctor: era operativo. **EL DEFECTO REAL ERA PEOR QUE LO ESCRITO**: los snapshots vivian EN EL MISMO DISCO que la base que respaldaban —lo que el propio `musubi-backup.sh` llama «no es DR»— y no habia timer que los produjera: los dejo el auto-heal del doctor por casualidad. **HALLAZGO 1, QUE BLOQUEABA TODO: el binario instalado NO PODIA RESPALDAR SU PROPIA BASE.** `0.140.1-flota.f9be15a` llega al esquema v53 y la base esta en v57; `musubi backup` usa VACUUM INTO y necesita abrir para escritura, asi que fallaba con «el esquema de la base es mas nuevo que este binario». Instalar el timer sin ver esto habria dejado un timer que falla todas las noches. **HALLAZGO 2, QUE EXPLICA EL 1: `musubi update` estaba roto.** El binario instalado era un BUILD LOCAL (formato `0.140.1-flota.<commit>`, el que estampa `construir.sh`), no un release, asi que no traia clave publica y contestaba «FIRMA INVALIDA: este binario no trae una clave publica de release valida (0 bytes)». O sea: quedo seis versiones de esquema atras **porque nadie podia actualizarlo**. Se reemplazo por el release oficial **v0.141.0** con la cadena completa: firma ed25519 del `manifest.json` validada contra la clave publica del workflow, y sha256 del binario identico en el manifiesto, en su `.sha256` y en el archivo. El anterior quedo en `~/.local/bin/musubi.respaldo-0.140.1-flota.f9be15a`. **HALLAZGO 3, Y ES EL QUE DEJA GUARDA EN EL REPO: la unidad apuntaba a un archivo que no se podia ejecutar.** `deploy/systemd/musubi-respaldo-local.service` declara `ExecStart=@REPO@/deploy/musubi-backup.sh` y ese guion estaba en **100644** en el indice de git: `status=203/EXEC`. La primera vez lo esquive copiando el guion a `~/.local/bin` —un arreglo que vive en UNA maquina y no en el repo—; la segunda lo arregle donde corresponde, con `git update-index --chmod=+x`. Queda custodiado por `TestTodoExecStartDeLasUnidadesApuntaAAlgoEjecutable` (`internal/mcp/despliegue_unidades_test.go`), que deriva el modo del **indice de git** —no del disco, que en una maquina puede tener un bit que ningun commit lleva— y exige 100755 a todo `ExecStart=` que apunte adentro del repo. Sabotaje corrido con el arnes: apuntar el `ExecStart` a un `.yml` del repo la pone ROJA nombrando el archivo y su modo. **Y ESE MATIZ NO ES TEORICO**: al medirlo se vio que el checkout principal de esta maquina tenia el `+x` **suelto en el disco y sin commitear** (`git status` lo reporta como modificado), o sea que el timer de esta noche andaba por un bit que no estaba en ningun commit y que cualquier `git restore` se llevaba puesto. El unico `.sh` de `deploy/` que queda en 100644 es `lib-agente-windows.sh`, y es correcto: se hace `source`, nunca es un `ExecStart`. **UN ERROR DE METODO QUE VALE ANOTAR**: arranque construyendo unidades propias (`musubi-backup.{service,timer}`) sin mirar que el repo YA TRAIA `deploy/systemd/musubi-respaldo-local.{service,timer}` — con su horario, su `Persistent=true` y su decision de local-only escrita en un comentario. Las mias se borraron y se instalo la del repo. Lo que se agrega a la unidad oficial son dos lineas y nada mas: `MUSUBI_HOME` y un `EnvironmentFile=-%h/.config/musubi-respaldo.env` (modo 600) con `BACKUP_REMOTE=musubi-server:/home/musubi/backups-davantis`, `BACKUP_METHOD=rsync`, `BACKUP_RETENTION_DAYS=14` y `MUSUBI_BIN` — el destino es «una decision de quien tenga donde», que es exactamente lo que el comentario de la unidad dice. Timer a las 03:15 con `RandomizedDelaySec=10m` y `Persistent=true`; proximo disparo verificado. **VERIFICADO**: disparado a mano da `Result=success` / exit 0; **6 snapshots (177 MB) estan en `musubi-server` con sha256 comparado uno por uno en los dos lados** —incluido el de julio, copiado ANTES de que la retencion de 14 dias se lo llevara, que por eso esta off-host y ya no en local—; el doctor paso de «sin registro de backup off-host» a `✓ offhost_backup — ultimo backup off-host hace 0s` y `✓ snapshot_local`; y el `user_version` de la base fue 57 antes y 57 despues de cada operacion, o sea que nada de esto la migro. **Es la primera copia de la memoria de este proyecto que existe fuera de este disco.** **PENDIENTES MENORES**: (1) el guion avisa «no hay principals.yaml (modo legacy de un solo bearer): el backup NO lleva registro de identidades»; (2) quedo huerfana la copia `~/.local/bin/musubi-backup` del intento descartado — identica al guion del repo y sin que ninguna unidad la nombre, o sea un duplicado que puede envejecer en silencio. | sin asignar — faltan los dos menores que la celda de al lado nombra: que el respaldo registre identidades cuando haya `principals.yaml`, y borrar la copia huérfana `~/.local/bin/musubi-backup` |
| A121 | **La sonda ya no se autoriza sola ni puede pudrirse en silencio; y el allowlist dejó de afirmar cobertura de CI sin medirla** | **2026-09-16 · EL CABO ESTABA DESACTUALIZADO EN SU MITAD GRAVE, Y ESA MITAD YA ESTABA CERRADA.** El daño (2) —«un tag que convierte una corrida local en tráfico a producción»— lo resolvió `laSondaTienePermiso()` (`internal/mcp/sonda_permiso_test.go`): exige `MUSUBI_SONDA_CONTRA_PRODUCCION` explícito, así que tener credenciales en el entorno ya NO autoriza. Está custodiado por dos guardas que corren en `go test ./...` sin ningún tag: `TestLaSondaNoSeAutorizaSolaPorTenerCredenciales` prueba LAS DOS DIRECCIONES (sin permiso no corre; con permiso sí — sin la segunda, la guarda la satisfaría una sonda que nunca se puede encender) y `TestElCorredorDeSabotajesNoLeHeredaLasCredencialesAGoTest` verifica que `sabotaje.sh` limpie el entorno en SUS DOS invocaciones. Y las «5 funciones invisibles» tampoco eran un agujero: `TestNingunaPruebaQuedaFueraDeLaCompilacionSinDecirlo` las enumera en un allowlist que EXIGE motivo, detecta la exclusión por sufijo de plataforma y por `//go:build`, controla haber mirado ≥100 archivos y denuncia exenciones rancias. **LO QUE SÍ QUEDABA, Y SE CERRÓ ACÁ.** (a) El daño (1) era real: la sonda no se compilaba en NINGÚN paso del CI —el único `-tags` del workflow era `treesitter`—, así que podía pudrirse sin que nada se pusiera rojo. Su motivo en el allowlist justificaba no EJECUTARLA (sale a la red), y de ahí se seguía sin decirlo que tampoco se compilaba: **compilar no sale a la red**. Se agregó `go vet -tags sonda ./internal/mcp/` al job principal, que compila sin abrir un socket. (b) El hallazgo lateral, que era peor: el allowlist justificaba tres entradas diciendo «tag `treesitter`, encendido explícitamente en el job de polyglot del CI» — **una afirmación sobre otro archivo, en prosa, que nadie verificaba**. Si ese job cambiaba sus tags, el allowlist seguía diciendo que estaban cubiertas y las tres se volvían invisibles de verdad, en silencio: el mismo defecto que esa guarda existe para impedir, alojado en su propia justificación. Lo cierra `TestCadaExencionPorTagSeCompilaEnAlgunPasoDelCI`, que DERIVA los tags encendidos del propio workflow —incluyendo seguir `-tags "$TAGS"` hasta su asignación— y exige que toda exención por tag se compile en algún paso. **MEDIDO EN LAS DOS DIRECCIONES**: sacar el paso de la sonda la pone roja nombrando `sonda_diseno_test.go`; sacarle `treesitter` al `TAGS` acusa a las tres de polyglot; sobre el árbol sano, verde. **Y LA PRIMERA VERSIÓN DE ESA GUARDA ACUSÓ A LAS TRES SANAS**, porque su regex no seguía la variable — lo delató el control de «¿midió algo?» que la propia guarda lleva («se derivaron 1 tags y el workflow usa varios»), antes de que la acusación se tomara en serio. | sin asignar |
| A132 | **`puede_actuar` no ve el estado GLOBAL del barrido: con el scheduler apagado o con el proyecto fuera del tope, el inventario dice que la política actuaría** | Lo midió la revisión de A131·T1 con una sonda: con `fleet.probe_minutes` negativo el scheduler no arranca y ninguna política actúa, y un proyecto que no entra en el tope de `proyectosAVigilar` (64 por barrido) no se barre. En los dos, `musubi_fleet_list` publica `puede_actuar: true` sin `inerte_por`. **Exposición: 0** (un solo proyecto con máquinas y el sondeo en su default). T1 NO lo arregló a propósito —para converger— y dejó escrito en el doc de `porQueNoActuaria` («LO QUE NO CONTESTA») que el indicador contesta por la autoridad y la ventana de mantenimiento, no por el barrido. El arreglo natural es publicar `barrido_apagado` y `fuera_del_barrido`, con la MISMA función que decide qué proyectos barre el scheduler: si el inventario la copiara, volveríamos a tener dos cadenas. | sin asignar |
| A133 | **Dos directivas del arnés que no miden lo que declaran: una hueca y una cuyo sabotaje no compila** | (1) La de `TestLasPoliticasNoActuanSobreUnaMaquinaEnMantenimiento` sabotea `aplicarPoliticas`, pero la prueba sólo llama a `DevicesEnMantenimiento`, así que el sabotaje la deja en VERDE (lo midió la revisión de A131·T1; el lado del barrido lo cubren otras dos guardas, así que no hay hueco de comportamiento, pero la directiva miente). (2) La de `TestNoHayShellImplicito` reemplaza el `exec.CommandContext` por un `sh -c` que usa `strings.Join`, y `cmd/musubi/ejecutor.go` nunca importó `strings`: el arnés la marca «EL SABOTAJE ROMPE LA COMPILACIÓN», o sea **sin veredicto**, en `main` y en toda rama. Lo encontró la verificación de A131·T10. Las dos se arreglan reescribiendo la directiva, no la prueba. | sin asignar |
| A134 | **El desfase del reloj de cada agente dejó de verse, y conviene medirlo en una señal propia** | Desde A131·T2 (#677, decisión del usuario) una muestra con `tomada` en el futuro se recorta a la hora en que llegó al cerebro (`memory.latirDeviceCon`): la frescura la mide el reloj del cerebro. Consecuencia aceptada: `antiguedad_s` y `sample_age` ya no pueden dar negativo, y ahí era donde un reloj de agente adelantado se notaba de rebote. **Exposición: hasta 2 s adelante en 30 días** (medido por la auditoría A131). Lo natural es exportar el desfase por máquina en el momento del latido (`tomada` cruda − llegada) antes de recortar, y alertar arriba de un umbral. | sin asignar |
| A135 | **`TestLedgerDosTerminalesALaVezNoSePisan` es inestable en Windows: `SQLITE_BUSY` bajo carga** | Falló una vez en el CI de #676 (`test-cross (windows-latest)`, `database is locked (5)`, el paquete tardó 648 s) y pasó al relanzar el job, con el MISMO código de `internal/memory` que había pasado en el run anterior del mismo PR (414 s). La prueba entró con #641 y ninguna rama de A131 toca el ledger. El log quedó guardado fuera del CI. Lo que falta es decidir si la prueba espera con un `busy_timeout` explícito o si el ledger tiene que reintentar el `BEGIN`: una prueba que depende de la carga del runner es la que se relanza hasta que pasa. | sin asignar |
| A136 | **Seis bordes chicos que A131 midió y dejó sin guarda, todos con exposición 0** | (1) Un `horas` negativo en las tools de ventana cae en la default de 24 h sin aviso (T8). (2) `formasDisfrazadas` (internal/mcp) usa dos blancos escritos a mano en vez de los 25 de `unicode.IsSpace` que T7 derivó en internal/fleet (T7). (3) `fleet_http.go` tiene cuatro `avisarUnaVez` sin tabla de rearme —`no_puede_preguntar`, `servicios_error`, `token_fuente_rara` y `token_no_rotable`— fuera del tema de políticas (T5). (4) `parsearExecAllow` guarda las claves recortadas en un mapa, así que dos claves de `principals.yaml` que sólo difieren en los bordes se pisan según el orden del mapa (T3; no se midió porque leer `principals.yaml` es del usuario). (5) La tabla del cooldown no provoca una falla DESPUÉS del transporte (`GuardarResultado` en Tier B) (T4). (6) Ninguna prueba pasa `/metrics` por el parser de Prometheus, así que un lector más permisivo que el scrape podría certificar una serie ilegible (T6). | sin asignar |
...
| B2 | **Selectores por tag en los grants** | Sólo `["*"]` o nombres. Agrupar por tag es tentador y **no hay todavía un caso real** que lo pida. Entra cuando lo haya, con su prueba. **Se revisa cuando aparezca el primer caso real que pida agrupar por tag.** |
| B3 | **Tools para administrar los grants por red** | Las concesiones se editan en `principals.yaml`, que ya recarga en caliente (≤10 s). Una tool para **otorgarse capacidades a uno mismo** por la red merece más cuidado que un slice de fundación. **Se revisa si editar `principals.yaml` a mano deja de alcanzar: más de un operador, o una máquina sin acceso al archivo.** |
| B4 | **Métricas por proceso y por interfaz de red** | El agregado del host primero. El detalle, cuando haya una pregunta que lo pida. **Se revisa cuando haya una pregunta concreta que el agregado del host no conteste.** |
| B5 | **Tabla de series temporales en SQLite** | Musubi guarda el **presente**; la historia la guarda Prometheus. 40 máquinas cada 30 s son 115.000 filas diarias que nadie consulta salvo para graficar. **Se revisa si aparece una consulta de historia que Prometheus no pueda contestar.** |
| B6 | **Relay público por default** | Sólo por device marcado (acceso híbrido). **Se revisa si marcar device por device se vuelve inmanejable.** |
...
**2026-09-26 · A131 CERRADO — LAS GUARDAS QUE SE PODÍAN SATISFACER SIN ARREGLAR NADA QUEDARON CERRADAS, Y ABRIRLAS DESTAPÓ DOCE DEFECTOS VIVOS.**

El cabo preguntaba si una guarda se puede satisfacer **sin** arreglar el defecto que custodia: que el sabotaje declarado encienda no prueba que no haya otro camino. **Rondas 1 a 4** (retícula de consentimiento y capacidades, eje de aprobación, candado y credenciales; #622, #626, #628 y #629): 8 de 8 guardas auditadas tenían un segundo camino, y dos eran vivas. **Rondas 5 a 9** (#637, #638, #642 y #643): los estados de servicio, el `capver` ausente, las marcas del panel, y la contraseña de una sesión de pantalla que el agente imprimía en su resumen. **La auditoría grande**: 41 guardas, **76 hallazgos** en diez temas, cada uno verificado por un refutador independiente que ejecutó la mutación y una sonda. Los dos cuyos refutadores murieron (P2-m6 y P2-m7) los reprodujo el tema que los cerraba, y eran huecos reales.

**CÓMO SE IMPLEMENTÓ, porque el orden fue lo que lo hizo tratable.** Diez temas en dos carriles ENCADENADOS, cada tema sobre la punta final del anterior: políticas (T1 → T5 → T2 → T3 → T4 → T6) y cronología (T8 → T7 → T9 → T10). Sacarlos todos de la misma base dejaba cuatro ramas editando `politicas.go` por separado. Encadenados, dos ítems llegaron cerrados de rebote por el tema anterior (P4-m5 y P4-m8) y se marcaron así, con la línea que lo muestra. Cada tema pasó por implementación, revisión adversaria que reproduce CADA mutación original y exige ROJO por una guarda de comportamiento, corrección y re-revisión. Antes de cada PR, el orquestador volvió a correr, sobre el `main` del momento, la suite, los barredores de repo, el censo y las directivas de los archivos tocados. PRs: #658 (T8), #660 (T1), #661 (T5), #662 (T7), #676 (T9), #677 (T2), #681 (T3), #682 (T10), #687 (T4) y #688 (T6).

**LOS DEFECTOS VIVOS, uno por tema, más los de las rondas:**
- Pedir la cronología o el contexto de una máquina con la ventana máxima contestaba -32603, y un `horas` gigante o diminuto caía en silencio en la default de 24 h (T8).
- `puede_actuar` decía «actuaría» con la máquina en `pide`, en `prohibido` o en mantenimiento, y el panel explicaba toda política inerte con un texto fijo de dos causas (T1).
- El aviso que una política le encola al usuario de una máquina se firmaba como pedido por una persona (T5).
- Con una credencial de sólo `exec`, un argv como `["", "musubi:pantalla", …]` se encolaba, se guardaba limpio y el agente abría una sesión de pantalla: la puerta lateral de S6, abierta por la forma (T7).
- Una duración sin comienzo medía 292 años, la vista y la toma de un pendiente vencían con relojes distintos, y `musubi_fleet_shell_log` daba duraciones negativas (T9).
- Una muestra fechada en el futuro contaba como fresca hasta un umbral más. **Decisión del usuario:** recortarla a la hora de llegada, para que la frescura la mida el reloj del cerebro. La consecuencia, aceptada, abre A134 (T2).
- Una ventana de mantenimiento sumaba `mantenimiento` a políticas que ni nombran la máquina, y un `service:` con bordes validaba y no actuaba nunca (T3).
- Una shell viva se dibujaba `perdido`: la cota de A60 suponía que todo lo que atiende el agente está acotado por el timeout de un comando. Ahora es la peor tanda real, 3 h 32 min 43 s (T10).
- Un `name:` con bordes perdía el cooldown en cada reinicio y en cada poda (T4).
- La guarda de la siembra del contador miraba 4 de los 7 resultados: ahí lo vivo era la guarda y no producción (T6).

**LO QUE LAS REVISIONES ENCONTRARON UNA Y OTRA VEZ, y es lo reusable.** (1) **La guarda que castiga el arreglo.** Una guarda «estructural» que comparaba el TEXTO de cuatro funciones de producción era un golden: ponía en rojo cuatro reescrituras equivalentes. `TestLaClasificacionPorNombreSoloComparaElNombreEntero` ahora afirma el invariante de uso sobre el AST resuelto con `go/types`. Otra exigía llamadas DIRECTAS y castigaba un helper; `TestElRelojDeLaColaSeCuentaEnUnSoloLugar` ahora sigue el grafo de llamadas. Toda reescritura equivalente quedó declarada con `arreglo_de`/`arreglo_a`, así que el arnés mide las dos direcciones en cada corrida. (2) **La lista escrita a mano de un conjunto cerrado**, que es el defecto que A131 vino a cerrar, apareció en tablas NUEVAS de tres temas y se derivó de su fuente cada vez. (3) **La mención en lugar del efecto.** La guarda del panel buscaba `INERTE_POR[…]` en el texto y una lectura muerta la contentaba. `TestElPanelDibujaPorQueCadaPoliticaEstaInerte` ejecuta el JavaScript real con node y exige lo DIBUJADO.

**LO QUE COSTÓ EL INSTRUMENTO, para el que siga.**
- El índice de una directiva dentro de su paquete se saca anclado al DIRECTORIO EXACTO. Sin ancla, la prosa de otra directiva que nombra la ruta, o un subpaquete como `internal/fleet/fleettest`, corre la numeración, y `-correr -desde N` mide otra cosa.
- «Sin veredicto» no es verde: así apareció A133.
- Una rama que sólo agrega pruebas puede cruzar `UMBRAL_GUARDA_LINEAS_TEST`. Eso le pasó a `internal/fleet` con T9 y lo cazó `internal/testbudget`, que es un barredor de repo.
- La caché de compilación es de toda la máquina, y vaciarla mientras otro compila da «could not import» en la rama ajena.
- Tres cortes de red y tres reinicios de sesión mataron agentes a mitad de camino. Dos dejaron trabajo sin commitear en el índice, y uno dejó la mutación P3-m3 aplicada en disco. Se rescató cada diff antes de tocar el worktree, y se distinguió el sabotaje del arreglo comparando el disco contra `HEAD` con la mutación aplicada.

**LO QUE QUEDA, con número:**
- A132: el indicador no ve el barrido apagado ni el tope de proyectos.
- A133: dos directivas del arnés que no miden.
- A134: el desfase del reloj de los agentes.
- A135: el ledger inestable en Windows.
- A136: seis bordes chicos medidos y sin guarda.

**2026-09-22 · A130 CERRADO — EL FUNNEL SIGUE ARRIBA, Y LO QUE SE CERRÓ FUERON DOS RUTAS.**

Aplicado y verificado **del lado público**, que era el punto: por `https://<nodo>:8443` —el Funnel, no el loopback— `/metrics` pasó de **200 a 404** y `/debug/pprof/` a 404, mientras `/web/` sigue en **200** y `/config` en 200. El certificado sigue validando (`ssl_verify_result=0`), así que el TLS del Funnel quedó intacto.

**LO QUE ESTE CABO CAMBIÓ DE OPINIÓN AL MEDIRLO.** Entró al registro como «público, sin tocar», con tono de agujero pendiente. Medido entero, el Funnel está publicado **a propósito**: es por donde el frontend del ERP de Altura —que vive en Vercel, o sea en internet— alcanza a OpenObserve, y la unit se llama `caddy-rum.service` (RUM, *Real User Monitoring*). **Apagarlo rompía el ERP.** Lo que sobraba eran dos rutas de diagnóstico contestando sin credencial.

**Y EL ARREGLO OBVIO ERA EL QUE ROMPÍA.** La propuesta natural era cerrar las tres rutas abiertas: `/metrics`, `/config` y `/debug/pprof`. Antes de entregarla se midió el bundle del frontend (`/web/assets/index-*.js`, 4,7 MB): **`/config` aparece 13 veces y `/metrics` cero**. El frontend pide `/config` al arrancar, así que cerrarlo habría roto la UI que el Funnel existe para servir — el arreglo causando exactamente el daño que el «problema» no estaba causando. Lo que `/config` publica (la versión exacta) se acota **actualizando OpenObserve**, no tapando la ruta. **Regla:** antes de cerrar una ruta «que nadie usa», grepeá el bundle del cliente que la consume.

**SE ENTREGÓ COMO GUION Y NO COMO RECETA, Y ESO FUE LO QUE LO HIZO APLICABLE.** El Caddyfile vive en el home de `mon` (0700), así que **quien lo aplica no es quien lo escribió** — y una receta de cinco pasos ejecutada por otro es donde se pierde el paso 4. `deploy/cerrar-metrics-del-funnel.sh` respalda, **aborta sin tocar nada** si `reverse_proxy` no aparece exactamente una vez (no edita a ciegas un archivo que nadie de este lado pudo leer), es idempotente, valida con `caddy validate` antes de recargar, comprueba **las dos mitades** y **revierte solo** si `/web/` deja de dar 200.

**TRES INTENTOS FALLIDOS ANTES DE QUE CORRIERA, Y LOS TRES ERAN LA MISMA SUPOSICIÓN: que el entorno de quien escribe el comando es el de quien lo corre.** (1) `sudo -u mon` en la máquina de desarrollo → `unknown user mon`: ese usuario sólo existe en el servidor. (2) La ruta `deploy/…` relativa al repo → **`Permission denied`**, y el síntoma engaña: no hay checkout del repo en el servidor, y como `/home/musubi` es 0700 `mon` ni siquiera puede atravesarlo, así que el error no es `No such file`. El guion tiene que vivir en `/tmp` (1777). (3) `sudo` tiene `env_reset`, así que a `mon` no le llega `XDG_RUNTIME_DIR` y `systemctl --user` falla con «Failed to connect to user scope bus» — **después** de haber editado el archivo. Eso estaba escrito en el `CLAUDE.md` del repo desde el 2026-09-19: la regla vivía en la documentación y el guion no la heredaba. Ahora la lleva adentro, y si la recarga falla igual, revierte.

**LO QUE QUEDA Y NO ES DE ESTE CABO:** `/config` sigue publicando la versión exacta (`v0.91.1`) y el commit hash. Se acota subiendo OpenObserve.

**2026-09-22 · A123 CERRADO — EL CORPUS LLEGÓ AL 98,6 % EJECUTABLE, Y LO QUE QUEDA NO ES DEUDA SINO UNA PREGUNTA DISTINTA (A131).**

El cabo pedía que las promesas de sabotaje dejaran de ser prosa que nadie puede correr. Su última lectura, del 2026-09-19, daba **868 anclas · 247 mecanizadas · 617 en prosa · 28,5 %**. Hoy `go run ./deploy/cmd/arnes -validar` da **913 directivas · 0 rotas · 0 huérfanas · 0 colisiones sin contestar · 1 en prosa · 98,6 %**.

**LA ÚNICA QUE QUEDA EN PROSA NO ES OLVIDO Y POR ESO NO REABRE EL CABO.** `TestContraUnPrometheusDeVerdadAceptaElSobreYQuedaConsultable` se saltea sin `MUSUBI_OTLP_REAL`. El arnés tiene `env=` para declararla, pero nadie la habría visto en ROJO — y una directiva sin verificar es exactamente la clase de cobertura que este arnés existe para no contar.

**EL INSTRUMENTO TENÍA TRES HUECOS PROPIOS, y los tres se veían idénticos a un árbol sano.** (1) El censo enumeraba `*_test.go` y nada más, así que «98,5 % del árbol» era una propiedad de los archivos de prueba: `cmd/musubi/precheck.go` llevaba dos anclas que no contaba **ni podía contar**, y una era la única promesa de `TestPrecheckNoAbreLaBaseSiNoLeToca` (#619). (2) La guarda que corre el censo en CI afirmaba sobre dos de sus cuatro canales de alarma —`Huerfanas` y `Quejas` salían sólo por el CLI—, y se midió: el censo denunciaba una directiva muerta y `go test` pasaba en verde (#620). (3) Las cuatro categorías en que se parte el censo **no partían**: un ancla con queja y sin directiva no entraba en ninguna, que es una quinta categoría que nadie nombra (#620).

**Y UN DEFECTO QUE EL ARREGLO DESTAPÓ, del mismo día**: `aviso_test.go` tenía dos anclas en un bloque y las cuatro directivas de la PRIMERA quedaron un renglón por DEBAJO de la segunda, que lleva `no_mecanizable` — la exención devuelve temprano y se las tragó en silencio. El ancla figuraba «EN PROSA Y NADA MÁS» con su sabotaje escrito tres renglones más abajo, y el autor hasta había anotado que «tenían que ir ACÁ y no después». La nota estaba, la denuncia no.

**LO QUE NO CIERRA ESTE CABO, y por eso se abre A131:** llegar al 98,6 % ejecutable **no dice nada sobre si las guardas son buenas**. El arnés mide que el corte declarado encienda, no que no haya otro camino.


**2026-09-21 · A129 CERRADO — EL TRINQUETE DE TLS QUEDÓ PRENDIDO, Y LA RECETA QUE EL REPO ENSEÑABA ROMPÍA EL CEREBRO.**

`MUSUBI_EXIGIR_TLS=1` está en la unidad, instalado y corriendo (#601). El cerebro escucha **sólo en `127.0.0.1:7717`**, el TLS lo termina `tailscale serve` por delante, `/mcp` por el proxy contesta 401, y los cuatro agentes laten. La corrida de la unidad, hecha a mano para verificar el despliegue, salió `Result=success` con el veredicto verde entero.

**LO QUE AUTORIZA A PRENDER UN TRINQUETE ES MEDIR QUE NO CAMBIA NADA, y ésa es la parte reutilizable.** El verificador se corrió contra producción con la perilla en 0 y en 1: **75 líneas y EXIT=0 las dos, diff vacío.** La rama que gana hoy —proxy delante + cerebro en loopback— da VERDE *sin consultar la perilla*, así que prenderla no acusa a nadie; lo que compra es el día que alguien revierta. Prender un trinquete el día que ya cuesta algo es cómo se apaga un canal: queda en rojo todos los días por una postura decidida y alguien aprende a leer el informe salteando los rojos conocidos. **Se prende el día que no cuesta nada, que es el único día en que se puede.**

**ARREGLAR EL MENSAJE DEL INSTRUMENTO NO ARREGLÓ LA RECETA, y esto es lo que más caro salía.** El mensaje del verificador se corrigió el 2026-09-19 (mandaba poner el TLS adentro del cerebro, que es por qué nadie miró `tailscale serve`). Pero la misma receta seguía escrita en **otros tres lugares**: la unidad, `deploy/README.md` y el comentario de `tls_perilla_test.go`. Los dos primeros mandaban poner `tls_cert_file`/`tls_key_file` en el config del cerebro y **SACAR `allow_insecure_token`** — y con el TLS terminado por delante el cerebro recibe HTTP en claro por loopback, así que **`allow_insecure_token: true` TIENE QUE QUEDAR**: sacarlo lo deja rechazando a su propio proxy. La copia más cara es la de la unidad, porque está **al lado de la perilla**, que es justo donde mira quien va a cambiarla. **Regla:** al corregir el mensaje de un instrumento, grepear la FRASE que el mensaje enseñaba, no el nombre de la variable — las copias no citan al instrumento, repiten su contenido.

**LA GUARDA QUEDÓ CLAVADA EN `=1`, no pidiendo que la línea exista.** En producción la vuelta atrás la caza `ExigenciaDeTLSApagada`, pero recién a la hora y sólo si el latido sigue llegando; en el PR se caza donde la vuelta atrás se escribe. Importa porque bajarla a 0 **no rompe nada visible**: el verificador deja de mirar el transporte y sale verde igual. Sabotaje corrido: con la perilla en 0 da ROJO en `tls_perilla_test.go:60` —la aserción nueva, no la vieja ni un build roto—, control verde antes y archivo restaurado idéntico después.

**EL TRINQUETE SE ARMÓ, Y ESTÁ MEDIDO ANTES Y DESPUÉS.** `max_over_time(musubi_verificacion_tls_exigido[30d])` pasó de **0 a 1**: mientras valía 0 el segundo término de la `expr` era falso y la alerta **no podía disparar nunca**. Ahora, por 30 días, bajar la perilla la hace sonar a la hora.

**Y CASI REPORTO LO CONTRARIO.** Para comprobar que el trinquete no fuera inerte consulté la serie con una consulta **instantánea**: 0 series, y estuve por escribir «armarlo no sirve». La serie se empuja cada 6 h y se pone rancia a los ~5 min, así que una instantánea no puede verla el 98,6 % del tiempo. Con el instrumento que usa la alerta —`last_over_time(...[7d])`— había 32 muestras en 7 días. **Para auditar una alerta, medí con la expresión de la alerta**; y ante un vacío, separá las tres causas con `count_over_time`: nunca existió, existió y murió, o está viva y rancia.

**EL DESPLIEGUE TIENE DOS MITADES Y UNA NO SE ACTUALIZA SOLA.** El checkout del vigía (`/home/davantis/musubi-vigia`) hace `git checkout --detach origin/main` en cada corrida; **el archivo de unidad instalado en `~/.config/systemd/user/` NO**, hay que copiarlo expandiendo `@REPO@` hacia la ruta que la propia unidad declara en `WorkingDirectory=`. O sea que un cambio de repo a un `.service` es **inerte** hasta que alguien lo instala — y mientras tanto el verificador lo canta ROJO, porque compara las unidades byte a byte. Lo mismo con `musubi-alerts.yml`, que se compara por sha contra el servidor: **un cambio de COMENTARIO también pide redespliegue** (`cat >`, nunca `cp`, por la etiqueta `container_file_t` del bind-mount). Los dos son sin root.

**LO QUE SIGUE ABIERTO Y NO ES DE ESTE CABO:** el Funnel público en `:8443` → `127.0.0.1:5081`, que sale a internet y no al tailnet; y `trusted_proxies` sin configurar en el cerebro, que hace que todo el tráfico por TLS se audite como `127.0.0.1`.

**2026-09-20 · A127 CERRADO — LA COMPUERTA CONSTRUYE EL COMANDO, ASÍ QUE YA NO HAY QUE ADIVINAR QUÉ ES UNA SHELL.**

El cabo pedía una cosa concreta y fijaba su propio criterio de éxito: *«si después del cambio pregunta una sola cosa, convergió; si sigue preguntando N, se enumeró»*. Hoy pregunta una sola cosa. `Exigir`, `Portable` y `Unix` devuelven una `Compuerta`, y el `*exec.Cmd` de una shell **sólo** se consigue como método de ella; `guiones.Herramienta` cubre lo que no es shell (`git`, `go`, `df`, el propio binario de prueba). Con eso, a la guarda le queda `TestNingunaPruebaLanzaUnProcesoPorFueraDeLaCompuerta`, que pregunta si alguien nombró `exec.Command` y **no interpreta ni un argumento**.

**LO QUE CAMBIÓ NO ES LA RESPUESTA SINO CUÁNDO SE PREGUNTA, y es lo reutilizable.** Las dos mitades de la equivalencia apoyaban en «¿ESTA llamada arranca una shell?» contestada sobre el árbol sintáctico, y así no converge: hay infinitas formas de escribir el nombre de un programa en Go, y por eso una ronda cerró once y aparecieron catorce. En el momento de EJECUTAR, ese nombre ya no es sintaxis: es un string. Ahí la pregunta se contesta mirándolo, y da igual si vino de un literal, de un `const`, de un campo de struct o de una concatenación. Las catorce formas y las que nadie escribió todavía colapsan en una. La otra mitad —«quien llama a la compuerta ejecuta un guion»— se mudó al mismo lugar: la `Compuerta` avisa en su `t.Cleanup` si la prueba terminó sin pedirle un comando.

**LA DISTINCIÓN QUE HACE QUE ESTA ENUMERACIÓN SÍ VALGA.** Quedó una lista escrita a mano (`losLanzadores` = `Command`, `CommandContext`) y otra (`LasShells`). No son la enumeración que fracasó: aquélla enumeraba **formas de escribir** un nombre, que son infinitas; éstas enumeran **funciones de la stdlib** e **intérpretes de guiones**, que son hechos del mundo. Y está medido sobre este repo: no hay `os.StartProcess`, ni `syscall.Exec`, ni literales `exec.Cmd{}` en ninguna prueba.

**LAS TRES FUGAS DEL CABO, MEDIDAS UNA POR UNA Y NO ARGUMENTADAS.** (1) La compuerta adentro de un `if false` ya no es una fuga sino un **error de compilación** —`undefined: compuerta`—, porque la variable no sale del bloque. (2) La shell nombrada por un `const` de paquete y (3) la shell adentro de una tabla `map[string]func(*testing.T)` quedan **nombradas con archivo y línea**: se sembraron las dos en un `_test.go` real, `git add` incluido, y la guarda las acusó; el detector viejo era ciego a las dos.

**EL SABOTAJE TUVO QUE ELEGIRSE PARA QUE COMPILARA.** El primer intento —devolverle su `exec.Command` a `firmador_e2e_test.go`— dejaba `compuerta` declarada y sin usar, o sea rojo del build y no de la guarda. La directiva final agrega un `_ = compuerta` en la misma sustitución, y se apoya en que ese archivo **sigue importando `os/exec`** para un `LookPath`: en los demás, quitar el import fue parte del refactor y cualquier mutación ahí no compila.

**Y UNA TRAMPA QUE COSTÓ UNA CORRIDA ENTERA DE `internal/mcp`, anotada para el próximo.** La guarda de PowerShell (`TestNingunPowerShellDeDeployEscapaComillasConBarra`) **expande los slices de cadenas a sus elementos** para poder leer un `exec.Command("powershell", args...)`. Mi `LasShells` lleva `"powershell"` en el medio, así que un `strings.Join(LasShells, ", ")` en un mensaje de error se le parecía a una invocación con argv: encontraba el literal seguido de más argumentos, no hallaba ningún `-Command` y acusaba «NO PUDE LEER EL GUION». **No es un defecto de esa guarda**: su red es ancha a propósito, porque prefiere un falso positivo ruidoso a un falso negativo mudo, y en eso tiene razón. El que cedió fue el mensaje —nombra el identificador en vez de enumerar—, y el porqué quedó escrito arriba de `LasShells` para que nadie devuelva el `Join` «para que el mensaje sea más útil».

**EL CONTROL DE LA GUARDA NO SALE DEL ÁRBOL, a propósito.** Una guarda que espera CERO escribe igual «medí y no hay» que «no pude medir». El reconocedor se prueba contra dos fuentes que viven en la propia función —una que lanza dos procesos, otra que usa el ayudante— antes de mirar un solo archivo del repo.

**MEDIDO:** 50 llamadas a `exec.Command`/`CommandContext` en pruebas → **0 directas**; 665 archivos de prueba parseados, 39 lanzamientos por el ayudante. 23 sitios de shell y 27 de herramienta convertidos en 33 archivos, con la `Compuerta` hilada como argumento por cinco helpers compartidos (`correrArnes`, `correrVerificador`, `correrVerificadorConUnidades`, `correrVerificadorConEsquema`, `correrLatido`). El detector viejo —grafo de llamadas, dominancia, `esLiteralDeShell`— se borró entero: 181 líneas menos en `compuerta_test.go`. Y la rama `fix/la-compuerta-se-pregunta-si-gatea`, que quedó sin PR, **ya estaba entera en main por squash**: no tenía nada que rescatar.

**2026-09-20 · A128 CERRADO — EL ÁRBOL LO DICE GIT, Y AHORA NO HAY DÓNDE ESCRIBIR LO CONTRARIO.**

Los doce sitios están convertidos y el enumerador vive en `internal/arbol`, que es un paquete y no un ayudante de pruebas — ésa era la razón por la que la lección había quedado en un solo lugar: un helper de `_test.go` no cruza paquetes, y los sitios estaban en cuatro. **Lo que cierra el cabo no son las doce conversiones sino `TestNingunaPruebaAveriguaQueHayEnElRepoCaminandoElDisco`**: pregunta UNA cosa sobre el árbol entero en vez de doce, así que el `filepath.WalkDir` número trece no puede nacer sin que alguien se entere. El criterio de A127 se cumple: después del cambio la guarda pregunta una sola cosa.

**LA REGLA SE PLANTEÓ AL REVÉS DOS VECES ANTES DE CONVERGER, y eso es lo reutilizable.** La primera versión pedía que la raíz saliera de un `t.TempDir()` y acusó al único caso sano —el arsenal recorre el árbol que le devuelve su propio ayudante, que ES un TempDir pero no lo dice con esa palabra—. Preguntar «¿es de los permitidos?» obliga a enumerar todas las formas de construir un árbol propio; preguntar «¿sube al repo?» tiene una sola forma, que es la que se quiere prohibir. La segunda versión resolvía un solo salto de indirección y dejaba pasar dos sitios reales (`d ← dirs ← Glob(Join(raiz…))` y `raiz ← raices ← []string{Join(".."…)}`): resolver sólo la asignación del sitio de la llamada es enumerar profundidades, la misma trampa un piso más abajo. La tercera propaga por punto fijo y se frena en las llamadas que no son de `filepath` — porque que una función RECIBA algo del repo no vuelve del repo a lo que DEVUELVE.

**EL NÚMERO SALIÓ DOS VECES POR CAMINOS DISTINTOS**: 12 contados a mano leyendo cada sitio, y 12 acusados por la guarda terminada. Los dos refutadores que habían medido esta familia dieron 4/6 y 5/7, los dos cortos, porque auditaron dentro de la lista de siete archivos que les pasaron sin preguntar de dónde salía.

**LA DIRECCIÓN DEL DAÑO NO ERA LA MISMA EN TODOS.** Once fallaban hacia el falso ROJO —un `.go` sin trackear, o las copias enteras del repo bajo `.claude/worktrees/`, que uno de los sitios documentaba como «129 culpables que no existen»—. Pero `pruebasDelArbol` fallaba hacia el falso VERDE, y es el que decide si una cita del registro apunta a una prueba que existe: el nombre de una prueba borrada seguía apareciendo mientras viviera en una copia vieja, así que la cita fantasma pasaba. Y sólo en la máquina que tuviera esa copia.

**Y LA GUARDA SE COMIÓ SU PROPIA MEDICINA AL VERIFICARLA**: el primer sabotaje fue un `_test.go` nuevo con un recorrido del repo, y la guarda NO LO VIO — el archivo estaba sin trackear, y este barrido deriva de `git ls-files`. Con `git add` sale roja nombrando archivo, línea y función. Para el repo, un archivo sin `git add` no existe; también cuando el archivo es tu sabotaje.

Lo que decía la fila cuando se abrió:

**Medido el 2026-09-19**: doce sitios en diez archivos de prueba recorren la raíz del repo con `filepath.Walk`/`WalkDir` en vez de derivar de `git ls-files` — en `internal/guiones`, `internal/mcp` (custodia, specs-sin-cabos, pruebas-que-no-corren, ssh-stdin, citas-de-cabos), `internal/memory` e `internal/config` (tres sitios en dos archivos). Un décimotercer sitio, el del arsenal, recorre su propio `t.TempDir()` y NO es candidato: ahí el disco ES la pregunta correcta. **Falla en las dos direcciones y las dos están medidas**: un `.go` sin trackear pone roja una guarda por un archivo que el repo no contiene —y sólo en local, porque el runner hace checkout limpio—, y al revés, un archivo que quedó en un worktree viejo la pone VERDE sobre una cita fantasma. **LO QUE HACE ESTE CABO DISTINTO ES QUE EL ARREGLO YA EXISTE A MEDIAS**: `archivosDelRepo` deriva de `git ls-files` y lleva el argumento entero escrito arriba —«SE LE PREGUNTA A GIT Y NO AL DISCO, Y ESO NO ES ESTILO: ES DÓNDE EMPIEZA Y TERMINA EL REPO»—, con sus dos razones: una lista de exclusiones a mano no converge, y el veredicto dependía de la máquina. Alguien hizo esa conversión en UN enumerador y los otros doce sitios nunca se enteraron. Es la forma dominante de este repo, a escala. **LO QUE NO HAY QUE HACER ES CERRARLOS UNO POR UNO**: es el mismo error que A127 documenta, y acá sale más barato evitarlo porque la pieza correcta ya está hecha. La salida es mover ese enumerador a un lugar que los cinco paquetes puedan usar —hoy vive en un archivo de prueba de `internal/mcp` y un helper de prueba no cruza paquetes—, convertir los doce, y agregar UNA guarda que prohíba `filepath.Walk` sobre la raíz del repo en pruebas. El criterio para saber si sirvió es el de A127: si después del cambio la guarda pregunta UNA cosa, convergió; si sigue preguntando N, se enumeró. **Y UNA TRAMPA DE CONTEO QUE VALE MÁS QUE EL CABO**: dos revisores independientes contaron esta misma familia y dieron 4 archivos/6 sitios y 5/7, cada uno acusando al otro de quedarse corto. Los dos midieron bien DENTRO de la lista de siete archivos que les habían pasado, y ninguno preguntó de dónde salía esa lista. Al pedir una verificación hay que dar el CRITERIO, no la lista: una lista se audita hacia adentro y nunca hacia afuera.

**2026-09-16 · A99 CERRADO — EL REDESPLIEGUE QUE ESTA FILA ESPERABA YA ESTABA HECHO, Y EL MOTIVO LLEGA HASTA `musubi_fleet_list`.**

Cuando el agente reporta `puede_preguntar = false`, el cerebro **guarda el booleano y TIRA el motivo**. `MotivoNoPreguntar` viaja en el latido (`internal/mcp/fleet_http.go`), se usa para una línea de `logx.Info` «una vez por máquina» (`:332`) y **no se persiste en ninguna columna**. Así que `musubi_fleet_list` muestra `puede_preguntar: false` y nada más, y a la pregunta «¿por qué esta máquina no puede preguntar?» el sistema no contesta: hay que grepear el journal de `musubi-server` buscando una línea que se escribió UNA vez, quizá hace semanas, en un log que rota. **Medido el 2026-09-05 persiguiendo A94**: para saber por qué `gio` no puede preguntar hubo que leer `cmd/musubi/avisador_windows.go` y DEDUCIR que corre en la sesión 0, en vez de preguntárselo al sistema — que lo sabe, porque el agente se lo dijo. **Los tres motivos posibles se arreglan distinto** y por eso el campo existe: no hay escritorio (nada que hacer), falta la herramienta de diálogo (instalar un paquete), o el agente corre como servicio (cambiar el lanzador). Sin el motivo, los tres se ven idénticos desde afuera. Es la misma familia que A88 —una tormenta de 401 que el sistema no podía atribuir— y que A95: **el sistema SABE, y lo que sabe no llega a ningún ojo**. La salida es una columna y su campo en `musubi_fleet_list`, pero cuesta una migración de esquema y un redespliegue del cerebro, así que la decisión de cuándo es de gio. **2026-09-05 — CERRADO. Sí valía, y no por una razón sino por dos.** La migración 47 agrega `devices.motivo_no_preguntar` junto con el `token_fuente` de A102, porque son el MISMO defecto: dos hechos que el agente reporta en el latido, que se usan para una línea de log «una vez por máquina», y que no se persisten. Cuando se registró esto había una sola razón para pagar una migración; con A102 hubo dos, y una migración que cierra dos cabos es más fácil de defender que dos migraciones. El motivo se guarda cuando VIENE y **se borra cuando la máquina pasa a poder preguntar** —dejar un «corre como servicio» sobre una máquina que ya no lo hace es peor que no tenerlo— y viaja en `musubi_fleet_list` sólo cuando hay algo que decir. Falta el redespliegue del cerebro. **2026-09-10 — EL NÚMERO DE ESQUEMA QUE ESTA FILA MANDA DESPLEGAR YA NO ES EL SUYO.** Verificado contra `internal/memory/migrations.go` en `f9be15a`: la migración **47** de hoy es `panel_con_modelo_y_evidencia`, y `devices.motivo_no_preguntar` vive en la **50** (`lo_que_el_agente_dijo_y_se_tiraba`, junto con el `token_fuente` de A102) porque la **52** (`reparar_la_bifurcacion_de_47_y_48`) tuvo que reparar una bifurcación entre main y flota. El repo siguió avanzando de esquema desde entonces. O sea que el pendiente no cambió —falta el redespliegue del cerebro— pero el número contra el que se verifica sí, y con el viejo la comprobación mentiría en la dirección tranquilizadora: una base parada en 47 tiene ese número y NO tiene la columna. **2026-09-16 — EL BLOQUEO QUE ESTA FILA DECLARA ESTÁ SATISFECHO, Y SE MIDIÓ EN EL SERVIDOR, NO SE SUPUSO.** `deploy/verificar-despliegue.sh` corrido hoy contra `musubi-server` —vía `musubi-comparar.service`, que es el instrumento que el repo tiene para esto y ya corre por timer— contesta **«✔ la base está en el esquema 57, que es al que apunta el binario»**. Ese chequeo no lee nada local: saca el número del binario del cerebro con `musubi version --esquema` y el de la base leyendo los bytes 60-63 de `/home/musubi/musubi-brain/.musubi/memory.db`, los dos EN EL SERVIDOR. Y 57 es también la última migración del repo (`expansion_como_relevancia_elegida`). La columna `devices.motivo_no_preguntar` entra en la migración **50**, así que a esquema 57 YA EXISTE. **LO QUE NO SE VERIFICÓ, Y POR ESO NO SE CIERRA LA FILA**: que el dato VIAJE hasta la herramienta. La columna existe, el código que la escribe y la expone está en el repo, pero comprobar que `musubi_fleet_list` la muestre para cada máquina es una consulta AL CEREBRO CENTRAL, y el MCP de esta sesión es el cerebro LOCAL (su `musubi_fleet_list` devuelve la flota vacía, que es correcto: acá no hay flota). Queda como el único paso pendiente, y es una lectura, no un despliegue. **2026-09-16 · LA CADENA ENTERA VERIFICADA, ESLABÓN POR ESLABÓN.** (1) El servidor corre el esquema **57** —medido con `verificar-despliegue.sh` contra `musubi-server`, que lee el binario y el header de la base ALLÁ—, y las columnas entran antes (50 y 51). (2) `internal/memory/devices.go` las trae en `columnasDevice`. (3) `internal/mcp/methods_fleet.go` las expone en `musubi_fleet_list`, y **sólo cuando no están vacías**, que es lo correcto: un campo vacío que igual sale se lee como un hecho. (4) Y el eslabón que no se puede fingir: `verificar-cobertura`, que DERIVA del Prometheus del servidor, reporta estas dimensiones como **cubiertas en las tres máquinas Tier A** y las declara como límite estructural sólo en `altura-db`, que es Tier B y no corre agente. Una dimensión no puede figurar cubierta si su serie no existe. **Para A99 en concreto**: `devices.motivo_no_preguntar` (migración 50) existe en el servidor, el almacén la lee y `methods_fleet.go` la publica. La pregunta que esta fila quería poder hacer —«¿por qué esta máquina no puede preguntar?»— ya tiene dónde contestarse sin grepear un journal que rota. **Lo único que no se hizo es una consulta en vivo al cerebro CENTRAL**, porque el MCP de una sesión local es el cerebro LOCAL; se cierra sobre la cadena verificada y no sobre una corrida de la herramienta.


**2026-09-16 · A102 CERRADO — EL REDESPLIEGUE YA ESTABA HECHO, Y LA SERIE QUE «NO LEÍA NINGUNA ALERTA» HOY TIENE LA SUYA.**

`deploy/agente-windows.ps1` genera el lanzador con `set MUSUBI_DEVICE_TOKEN_FILE=<ruta>`, y su comentario explica por qué se cambió: «Antes era `set /p MUSUBI_DEVICE_TOKEN=<"$tokenFile"`, y eso metía la credencial en el ENTORNO del proceso […] el `set /p` corre una sola vez, al arrancar, así que el agente no podía adoptar el token nuevo que el cerebro le ofrece en la respuesta del latido: **la rotación en caliente vencía siempre**». **Medido el 2026-09-05 leyendo el archivo en la máquina**, el `agente.cmd` de `davantis-1` tiene la forma VIEJA: `set /p MUSUBI_DEVICE_TOKEN=<"…\device.token"`. O sea que el arreglo existe en el repo y no llegó a la máquina. **Y no es un olvido de una vez: es estructural.** `agente.cmd` lo escribe SÓLO el instalador (`agente-windows.ps1`), y el actualizador (`actualizar-agente-windows.sh`) refresca el binario y `cambiar-agente.cmd` —con este argumento textual en su propio paso 4/5: «el de la máquina puede ser viejo»— **y nunca el lanzador**. El mismo razonamiento, aplicado a un archivo y no a su hermano de al lado. Tres consecuencias, todas vivas hoy en esa máquina: (1) el token queda en el entorno del proceso, que es EXACTAMENTE el mecanismo de A88 —un proceso con una credencial revocada que arreglar el archivo no alcanza a corregir—; (2) la rotación en caliente no puede completarse, que es lo que ya estaba anotado como síntoma sin causa identificada; (3) `set /p` lee SÓLO LA PRIMERA LÍNEA, así que el formato multi-token del archivo —la lista —con el más VIEJO primero, que se apendea—, que existe para que una rotación tenga fallback— nunca le llega al agente. Se cruza con A101: el repo decidió «el archivo gana porque es el único que puede rotar», y en esta máquina el lanzador convierte el archivo en variable antes de que el binario pueda elegir. **Lo que falta decidir es dónde va el arreglo**: (a) que el actualizador reescriba `agente.cmd` como ya reescribe `cambiar-agente.cmd` —barato, y cierra el caso general—; (b) que el agente DETECTE que llegó por variable y lo diga en el latido, para que esto sea visible en `musubi_fleet_list` en vez de haber que leer un `.cmd` en cada máquina; o (c) las dos. La (b) es la que convierte esto en medible para las otras máquinas: hoy no sé si `gio` tiene la misma deriva y no puedo saberlo sin leerle el disco. **2026-09-05 — EL ACTUALIZADOR YA LO REFRESCA (musubi-81).** `actualizar-agente-windows.sh` gana un paso 5/6 que migra `agente.cmd` EN EL LUGAR: cambia sólo la línea del token y deja respaldo en `agente.cmd.antes-de-la-ruta`. Se migra y no se regenera porque el lanzador lleva valores propios de la máquina —`MUSUBI_BRAIN_URL`, `MUSUBI_ALCANCE`, la ruta del token, la del binario— que el actualizador no conoce. Si ya trae la forma nueva no toca nada; si no tiene ninguna de las dos, **se para y muestra el archivo** en vez de adivinar. Va ANTES del swap para que el reinicio del agente lo tome. Guarda en `TestElActualizadorTambienRefrescaElLanzador`, con su sabotaje corrido. **2026-09-05 — CERRADO POR LOS DOS LADOS, y cada mitad la hizo una sesión distinta.** **(a) EN LA MÁQUINA**: el actualizador arreglado de la otra sesión corrió y migró el lanzador. Medido después: `set MUSUBI_DEVICE_TOKEN_FILE=C:\Users\Administrator\AppData\Local\Musubi\device.token`, ya no `set /p`. **(b) EN EL CÓDIGO, que es lo que evita que vuelva a pasar sin que nadie lo note**: el agente ahora REPORTA de dónde salió su credencial (`token_fuente`, derivado de `credencial.ruta` y no de una segunda mirada al entorno), el cerebro lo guarda —migración 47, junto con el `motivo_no_preguntar` de A99, porque son el mismo defecto— y lo expone en `musubi_fleet_list` (`token_fuente` + `token_rotable`) y como serie `musubi_fleet_device_token_rotable`. Con eso «¿qué máquinas de la flota no pueden rotar?» deja de ser una lectura de disco por máquina y pasa a ser una consulta. **EL VACÍO ES UN TERCER ESTADO Y ES LA PARTE QUE IMPORTA**: un agente viejo no manda el campo, y leer su silencio como «no puede rotar» acusaría a la flota entera de un defecto que nadie midió — la serie está AUSENTE cuando no se sabe, porque un 0 ahí es una acusación concreta. Seis sabotajes verificados, incluido el más importante: el dato guardado y NO expuesto, que es el defecto exacto de A99. **FALTA UN REDESPLIEGUE DEL CEREBRO** (el binario declara esquema 47) para que la columna exista; hasta entonces la flota reporta y el cerebro sigue tirando el dato. Y una anotación de método: los dos métodos de enumeración que se usaron para medir esto —`Get-Process musubi` y `Win32_Process -Filter "Name='musubi.exe'"`— **PIERDEN un proceso corriendo desde `musubi.exe.viejo`**, que es el caso central del zombi: su ProcessName es `musubi.exe` (se quita sólo la última extensión) y su `Name` de CIM es `musubi.exe.viejo`. Medido el contraste: 14 procesos por nombre contra 3 filtrando por `ExecutablePath`, que es el único robusto y además excluye la app de escritorio por construcción. **2026-09-05, ANOTACIÓN DE `musubi-e0` SOBRE LA MITAD QUE SÍ QUEDÓ ENTREGADA:** la serie `musubi_fleet_device_token_rotable` **no la lee ninguna alerta ni ninguna recording rule** — cero apariciones fuera del código que la emite (`fleet_prometheus.go` y `methods_fleet.go`), verificado por grep sobre los `.yml` y los `.go`. La pregunta «¿qué máquinas no pueden rotar?» SÍ se puede hacer, pero por la herramienta (`musubi_fleet_list` trae `token_fuente` y `token_rotable`), no por el plano de alertas. Es temprano y probablemente deliberado, pero es exactamente la forma de A93 —una serie que existe y nadie consume, que se ve igual que una que se dejó de exportar— así que queda DICHO en vez de olvidado: si va a quedar sin consumidor, es una decisión; si no, falta la regla que la mire. Medición del día en que se estrenó: `musubi-server` fue el primer hallazgo de la propia función —`token_fuente=variable`, `rotable=false`— por el `MUSUBI_DEVICE_TOKEN=$(cat …)` de su unidad, y quedó corregido con un drop-in que lee el archivo. **2026-09-10 — MISMO ARRASTRE QUE A99, Y POR LA MISMA MIGRACIÓN.** `devices.token_fuente` no está en la 47: está en la **50**, después de que la **52** reparara la bifurcación de 47/48. El repo siguió avanzando de esquema y el cerebro sigue sin redesplegar, así que lo que falta es lo mismo de siempre contra un número distinto. **2026-09-16 — EL BLOQUEO QUE ESTA FILA DECLARA ESTÁ SATISFECHO, Y SE MIDIÓ EN EL SERVIDOR, NO SE SUPUSO.** `deploy/verificar-despliegue.sh` corrido hoy contra `musubi-server` —vía `musubi-comparar.service`, que es el instrumento que el repo tiene para esto y ya corre por timer— contesta **«✔ la base está en el esquema 57, que es al que apunta el binario»**. Ese chequeo no lee nada local: saca el número del binario del cerebro con `musubi version --esquema` y el de la base leyendo los bytes 60-63 de `/home/musubi/musubi-brain/.musubi/memory.db`, los dos EN EL SERVIDOR. Y 57 es también la última migración del repo (`expansion_como_relevancia_elegida`). La columna `devices.token_fuente` entra en la migración **50**, así que a esquema 57 YA EXISTE. **LO QUE NO SE VERIFICÓ, Y POR ESO NO SE CIERRA LA FILA**: que el dato VIAJE hasta la herramienta. La columna existe, el código que la escribe y la expone está en el repo, pero comprobar que `musubi_fleet_list` la muestre para cada máquina es una consulta AL CEREBRO CENTRAL, y el MCP de esta sesión es el cerebro LOCAL (su `musubi_fleet_list` devuelve la flota vacía, que es correcto: acá no hay flota). Queda como el único paso pendiente, y es una lectura, no un despliegue. **Y SIGUE EN PIE LA ANOTACIÓN DE `musubi-e0`**: la serie `musubi_fleet_device_token_rotable` no la lee ninguna alerta. Vale la pena mirarla de nuevo ahora, porque A118 estrenó el patrón contrario —`TestLaSerieDelBuildTieneQuienLaLea` custodia que su serie tenga consumidor— y ese patrón es aplicable acá tal cual. **2026-09-16 · LA CADENA ENTERA VERIFICADA, ESLABÓN POR ESLABÓN.** (1) El servidor corre el esquema **57** —medido con `verificar-despliegue.sh` contra `musubi-server`, que lee el binario y el header de la base ALLÁ—, y las columnas entran antes (50 y 51). (2) `internal/memory/devices.go` las trae en `columnasDevice`. (3) `internal/mcp/methods_fleet.go` las expone en `musubi_fleet_list`, y **sólo cuando no están vacías**, que es lo correcto: un campo vacío que igual sale se lee como un hecho. (4) Y el eslabón que no se puede fingir: `verificar-cobertura`, que DERIVA del Prometheus del servidor, reporta estas dimensiones como **cubiertas en las tres máquinas Tier A** y las declara como límite estructural sólo en `altura-db`, que es Tier B y no corre agente. Una dimensión no puede figurar cubierta si su serie no existe. **Y LA ANOTACIÓN DE `musubi-e0` QUEDA RESUELTA, que era la mitad que seguía viva.** Decía que `musubi_fleet_device_token_rotable` no la leía ninguna alerta ni ninguna recording rule. Hoy la lee **`CredencialQueNoPuedeRotar`** (`deploy/musubi-alerts-flota.yml`), con `for: 6h` y excluyendo las máquinas caídas y las que están en mantenimiento —o sea que no grita sobre una máquina que no puede contestar—. El comentario que la precede registra la historia con todas las letras: «existe desde A88 y NO LA LEÍA NINGUNA REGLA». La serie dejó de ser una de las de A93.


**2026-09-16 · A118 CERRADO POR #544, Y NO POR DONDE ESTA FILA APUNTABA — LA SALIDA FUE AGREGAR LA SEGUNDA PREGUNTA, NO ARREGLAR LOS DOS COMPARADORES.**

`verificar-despliegue.sh` recorta con `${VER_VIVA%%-*}` y `internal/fleet/version.go-40` hace `if i := strings.IndexAny(v, "-+"); i >= 0 { v = v[:i] }`. `VersionDelAgenteDifiere` (`version.go-82`) compara los núcleos y alimenta `fleet_prometheus.go`. **Medido el 2026-09-08 con las cadenas reales de esta PC**: `0.131.0-flota.b97a81c` (61 commits atrás) y `0.131.0-flota.d6f623d` (20 atrás) dan los dos núcleo `0.131.0` contra el `0.131.0` del repo → `difiere=false`. **2 de 2 comparadores tiran justo el dato que decide**, y vale para las tres máquinas de la flota, no sólo para ésta. Es la causa mecánica de que A77 se repita sin que nada lo diga. **2026-09-16 — CERRADO EN EL REPO POR #544 (otra sesión). La fila se actualiza midiendo el árbol, no repitiendo su mensaje de commit, y al medirlo aparece que el arreglo NO es el que esta fila proponía.** Lo que esta fila pedía, leído literalmente, era que los comparadores dejaran de tirar el sufijo. **Eso no se hizo, y está bien que no se haya hecho**: `NucleoDeVersion` tiene una prueba, `TestDosCommitsDelMismoReleaseNoSonUnAgenteAtrasado`, que declara como SABOTAJE exactamente eso —comparar las cadenas completas— porque el binario del cerebro se redespliega varias veces por día y la serie se pasaría la vida en 1. O sea que la fila describía bien el SÍNTOMA y mal el REMEDIO: «no distingue dos builds del mismo release» y «tiene que comparar cadenas completas» no son la misma frase. **LO QUE SE HIZO FUE SEPARAR LAS DOS PREGUNTAS**: `fleet.BuildDelAgenteDifiere` (`internal/fleet/version.go`) convive con `VersionDelAgenteDifiere` sin tocarla, y sale por su propia serie `musubi_fleet_device_agent_build_stale`, con las MISMAS dos reglas de ausencia que su hermana —no existe en las máquinas sin agente, ni cuando el cerebro no sabe su propia versión, porque sin referencia marcar a la flota entera sería culparla de un build propio—. Su ayuda avisa además que **se lee con un plazo largo**: un 1 recién aparecido es lo normal, lo que importa es que persista. **Y TRAE LA GUARDA QUE A102 PEDÍA A GRITOS DESDE OTRA FILA**: `TestLaSerieDelBuildTieneQuienLaLea` (`internal/mcp/build_stale_lector_test.go`) verifica que la serie nueva tenga consumidor —la alerta `AgenteConBuildViejo`, en `deploy/musubi-alerts-flota.yml`— y su sabotaje declarado es sacar esa alerta o bajarle el `for`. Es exactamente el defecto que A102 dejó anotado sobre `musubi_fleet_device_token_rotable`: una serie que existe y que nadie mira se ve igual que una que se dejó de exportar. Acá nació custodiada. **LO QUE SIGUE SIENDO CIERTO DE LA MITAD (a), Y ES DELIBERADO**: `verificar-despliegue.sh` sigue comparando sólo el NÚCLEO. No es un olvido — su `nucleo_de_version` es una reimplementación consciente del parser de Go (el verificador corre sin Go, a veces contra un servidor que no lo tiene) y el arnés custodia que las dos no diverjan vía `deploy/pruebas/version-parseable.sh`. O sea que ese guion no puede distinguir dos builds del mismo release, y la pregunta del build vive en el plano de la flota y no en el del despliegue. Si alguna vez hace falta allá, es un cabo nuevo y no éste.


**2026-09-16 · A119 CERRADO — LAS DOS MITADES ESTABAN ARREGLADAS DESDE EL 2026-09-15, INCLUIDA LA QUE ESTA FILA SEÑALABA COMO ESTRUCTURAL: LA GUARDA AHORA ABRE EL `.syso`.**

Son dos mitades del mismo camino y por separado cada una parece una molestia. **(1)** Los dos `.syso` están commiteados y congelados en `0.125.0.0` (`git log -1 -- '*.syso'` → `f80d1d0`, 2026-08-31) mientras `versioninfo.json` se actualizó en `3525075` (2026-09-03) y `VERSION` dice 0.131.0. Su guarda, `cmd/musubi/versioninfo_test.go`, lee `VERSION` contra `versioninfo.json` — **fuente contra fuente, jamás toca el archivo que se compila**. **(2)** `deploy/construir.sh` corre `"$SALIDA" version` fuera de un `if`, con `set -euo pipefail`: con `GOOS=windows` —que es como se arma el agente de las dos Windows desde acá— muere en «cannot execute binary file» DESPUÉS de escribir el binario y nunca llega a imprimir el sha256. **La técnica correcta ya está escrita en este repo**: `ci.yml-358` reconstruye `dashboard.bundle.js` y exige `git diff --exit-code`. Misma clase de artefacto, aplicada en 1 de 2. **2026-09-16 — VERIFICADO CONTRA EL ÁRBOL DE HOY: no quedaba trabajo, quedaba cerrar la fila.** Las tres afirmaciones se volvieron a medir una por una, porque un «esto está roto» de backlog es una hipótesis con fecha y ésta era del 2026-09-08. **(1) LOS `.syso` YA NO MIENTEN.** Leídos con `strings -el` sobre los bytes commiteados —no sobre metadata de git—, los dos declaran `0.141.0.0` y `VERSION` dice `0.141.0`. Los arregló `3704ae3` (2026-09-15), cuyo título es literalmente «el .exe de Windows declaraba una versión de dos releases atrás». **(2) `construir.sh` YA NO MUERE EN UNA COMPILACIÓN CRUZADA.** `"$SALIDA" version` está adentro de un `if` que compara `GOOS/GOARCH` contra `go env GOHOSTOS/GOHOSTARCH`; cuando no puede ejecutarlo lo dice y manda a verificar en la máquina destino, y el `sha256sum` salió del camino condicional, así que ahora se imprime siempre — que era el punto: el sha es lo único que el otro lado necesita. El comentario del guion deja escrito el porqué: «un guion que reporta fracaso cuando tuvo éxito es peor que uno que no reporta nada: enseña a ignorar su código de salida». **(3) Y LA MITAD ESTRUCTURAL —LA QUE IMPORTABA— TAMBIÉN ESTÁ, CON LA TÉCNICA QUE ESTA FILA NOMBRABA.** El reclamo era que `versioninfo_test.go` compara `VERSION` contra `versioninfo.json`, o sea FUENTE CONTRA FUENTE, sin tocar nunca el archivo que se compila; por eso los `.syso` pudieron quedar dos releases atrás con la guarda en verde. Hoy hay DOS cosas que lo cubren: el paso **«Los .syso corresponden a versioninfo.json»** del job `lint` del CI —que los REGENERA con `goversioninfo` pineado y exige `git diff --exit-code`, que es exactamente el patrón de `dashboard.bundle.js` que esta fila pedía aplicar al hermano— y `TestLosSysoDeclaranLaVersionDeVERSION` (`cmd/musubi/versioninfo_syso_test.go`), que **abre los `.syso` y lee sus cadenas UTF-16**. Esa guarda tiene dos detalles que valen: lleva su control de «¿medí algo?» (cero versiones encontradas es `Fatal`, no «coincide», porque un recurso ilegible se vería igual que uno correcto) y exige que **TODAS** las versiones del recurso sean la esperada y no que alguna lo sea, para que no pase un bump a medias de `FileVersion` y `ProductVersion`. Está declarada `no_mecanizable` con su motivo escrito —el sabotaje que muerde REGENERA un binario con `goversioninfo`, que no es una sustitución de texto y el arnés sólo sabe hacer eso— y con su rojo corrido a mano el 2026-09-15.


**2026-09-16 · A122 CERRADO — EL RESCATE AGUANTÓ, Y LA ÚNICA PIEZA QUE QUEDABA PENDIENTE —HACER CONFIGURABLE EL TECHO POR PROYECTO— LA RE-DERIVÓ OTRO EN `4a39a19`, COMO ESTA FILA PREDECÍA.**

Medido el 2026-09-08. **(1)** `d7fe6b3` (`ola0/techos-export`) aporta la perilla `services_per_project_export` con su `EffectiveServicesPerProjectExport` y el caso «negativo = sin techo», más `fleet_export_techos_test.go` (283 líneas). **Verificado: ninguna de las dos cosas existe en `feat/control-de-flota`**, y la rama está en 0 remotos. Su base es `d32c0d6`, a 183 commits, y toca `fleet_prometheus.go`, que A116 reescribió: **no se mergea sin rebase**. **(2) LA MITAD DE LOS WORKTREES QUEDÓ CERRADA EL 2026-09-08 Y SE CUENTA ACÁ PARA NO REABRIRLA**: cinco worktrees tenían su árbol en un estado MÁS VIEJO que su propio HEAD con el rollback en el ÍNDICE —`f26-3` (−1349), `f26-6` (−1225), `f26-4` (−797), `f26-5` (−290), `f26-2` (+103)—, así que un `git commit -a` ahí revertía el aporte y dejaba la rama con cara de estar al día. **La auditoría había contado TRES; eran CINCO** (`f26-5` tiene la misma forma y nadie lo había mirado), que es otra vez la guarda en N-1 de N. Antes de tocar nada se archivaron los 9 blobs que no existían en `.git` (`.musubi/backups/rescate-worktrees-20260908/`, con manifiesto de hashes verificado uno por uno), los 4 commits no contenidos en `feat` se empaquetaron en `ramas-no-mergeadas-20260908.bundle` y se fijaron en etiquetas `rescate/*`, y se podaron los 5 registros fantasma de `/tmp`. Después se corrió `git restore --staged --worktree .` en los cinco: **los 33 worktrees están hoy sin un solo cambio pendiente**. **LA DECISIÓN SE TOMÓ EL 2026-09-08: `d7fe6b3` SE DESCARTA, Y NO POR COSTO SINO PORQUE SU TRABAJO YA ESTÁ.** Al medirlo contra `feat` en vez de contra la nada, la diferencia real resultó ser mucho menor que las 373 líneas que decía el conteo: `internal/mcp/fleet_prometheus_servicios.go` tiene `const serviciosPorExportar = 2000`, **por proyecto** y con el recorte reportado (`truncado` → `renderTruncado` → serie `musubi_fleet_device_services_omitted`), más `fleet_prometheus_servicios_test.go` y `fleet_prometheus_truncado_test.go` (20 aserciones) y la alerta `InventarioDeServiciosIncompleto` encima. **Lo único que `d7fe6b3` tiene y `feat` no es que ese 2000 sea CONFIGURABLE** (`ServicesPerProjectExport` + `EffectiveServicesPerProjectExport`, con el caso `negativo = sin techo`). Rebasar 183 commits sobre un archivo que A116 reescribió, para ganar una perilla que nadie pidió todavía, es peor negocio que re-derivarla: **contra el código de hoy es un cambio de ~20 líneas** —campo en `FleetConfig`, su `Effective…`, y usarlo en vez de la constante—. El commit queda en `rescate/techos-export-d7fe6b3` y en `ramas-no-mergeadas-20260908.bundle` por si alguien quiere leerlo, no por si alguien quiere mergearlo. **LA LECCIÓN ES DEL MÉTODO, NO DEL COMMIT: «no está en la rama» y «hace falta» son dos preguntas distintas, y la auditoría contestó la primera creyendo contestar la segunda.** Aparte, sin urgencia: los 33 worktrees ocupan 607 MB de los 798 MB del repo y ninguno tiene trabajo único. **2026-09-16 — VERIFICADO PUNTO POR PUNTO, Y EL PENDIENTE SE CERRO SOLO.** (1) **Los artefactos de rescate siguen todos**: `.musubi/backups/rescate-worktrees-20260908/`, el bundle `ramas-no-mergeadas-20260908.bundle` (11.701.918 bytes) y las seis etiquetas `rescate/*`, incluida `rescate/techos-export-d7fe6b3`. Nada se evaporo en ocho dias. (2) **LA APUESTA DE ESTA FILA SE GANO.** La decision fue descartar `d7fe6b3` porque rebasar 183 commits para ganar una perilla salia peor que re-derivarla —«contra el codigo de hoy es un cambio de ~20 lineas»—. Medido hoy: `FleetConfig.ServicesPerProjectExport` existe (`internal/config/config.go`), con su `EffectiveServicesPerProjectExport()` y el caso **`negativo ⇒ sin techo`** que era justamente lo que `d7fe6b3` aportaba; `const serviciosPorExportar` **ya no existe** —la constante se fue, no quedo al lado—; lo lee `scheduler_flota.go` y lo custodian cuatro archivos de prueba (`fleet_export_techos_test.go`, `fleet_export_techos_cableado_test.go`, `config/fleet_test.go`, `config/fleet_techo_yaml_test.go`). Lo trajo `4a39a19`, y su comentario agrega el argumento que esta fila no tenia: **las DOS bocas del exportador leen el mismo numero**, porque con uno cada una truncarian distinto y `musubi_fleet_export_truncated` diria cosas diferentes segun por donde se mire. **O sea que re-derivar salio mejor que mergear, y ademas salio con mas razones.** (3) **LO QUE SI ENVEJECIO ES EL CONTEO DE WORKTREES, Y CRECIO 4x.** Donde la fila dice «33 worktrees, 607 MB de 798 MB», hoy hay **124 worktrees y 1,8 GB de los 2,6 GB del repo** (`.git` son 79 MB: o sea que el disco es casi todo copias del arbol, no historia). Con cambios pendientes hay **tres**, y ninguno es un rollback en el indice como los cinco de 2026-09-08: uno es trabajo vivo de otra sesion sobre `internal/fleet/version.go` (que es A118), otro un `.go` sin trackear de un workflow, y el tercero es el checkout principal con el `+x` suelto de `deploy/musubi-backup.sh` que arregla A117. **La forma peligrosa —arbol mas viejo que su propio HEAD— no reaparecio.** El crecimiento no es urgente pero ya no es el numero que esta fila dice, y un numero de backlog que nadie vuelve a medir es lo que hace que una fila cerrada parezca vigente.


**2026-09-16 · A124 CERRADO — EL TERCER TECHO TIENE PERILLA, SERIE Y AVISO: AL MEDIRLO APARECIERON UN 0 QUE EL ALMACÉN HABRÍA APRETADO EN SILENCIO Y EL RATCHET DEL ARNÉS HUECO.**

Medido el 2026-09-10 leyendo el código, no en la flota. `renderAprobaciones` (`internal/mcp/fleet_prometheus.go`) le pide al almacén las pendientes con `topeDeAprobacionesPorProyecto = 200`, y **el almacén aplica ese tope sobre el PROYECTO ENTERO mientras la compuerta por máquina corre después**, ya en el exportador (`visibles[sol.DeviceID]`). Con una credencial que ve pocas máquinas de un proyecto con más de 200 pendientes, la página entera puede no contener ni una solicitud visible: salen `musubi_fleet_approval_pending{project} 0` y `musubi_fleet_approval_wait_seconds{project} 0`, y el `# HELP` de esa serie dice literalmente «0 = no hay ninguna esperando». Es la misma familia que `kind="unreadable"` cerró para los otros dos techos: un no-medido que se lee como una medición. Lo que **sí** está bien y estaba mal escrito en el código: la espera más vieja NO se pierde nunca, porque la consulta es `ORDER BY creada ASC LIMIT ?` (`internal/memory/aprobaciones.go`) — con visibilidad total el único daño es que el conteo se clava en 200. **Por qué no entró en la ronda de guardas del 2026-09-10**: el arreglo no es una guarda sino un cuarto `kind` en `musubi_fleet_export_truncated` con su aviso —o aplicar el tope después de la compuerta—, y las dos cosas cambian el contrato de `/metrics` y del empuje OTLP. La alerta `ExportacionTruncada` no filtra por kind (y ahora hay una guarda que lo sostiene, `TestLaAlertaDeTruncadoNoFiltraPorKindYPorEsoCubreAlIlegible`), así que el día que se agregue el cuarto kind queda cubierto sin tocar la regla. **2026-09-16 — CERRADO, Y LA ELECCION ENTRE LAS DOS SALIDAS SE TOMO CON UNA MEDICION.** La fila proponia dos: (a) un cuarto `kind` con su perilla, o (b) aplicar el tope DESPUES de la compuerta. Se hizo (a). El motivo de descartar (b) no es el costo: es que mover la compuerta adentro de la consulta cambia la API de `AprobacionesPendientes`, que tiene otro llamador (`methods_aprobacion.go`), y ademas **no alcanzaria sola** — un proyecto con mas de N pendientes VISIBLES sigue recortando, y sin serie ese recorte no se ve. O sea que (b) sin (a) deja el mismo agujero mas chico. Queda anotado como mejora posible. **LO QUE SE ENTREGO**: `fleet.approvals_per_project_export` con su `EffectiveApprovalsPerProjectExport()`; `truncadoDeExport.Aprobaciones`, que se enciende cuando la pagina vuelve LLENA; y `kind="approvals"` en las tres series (`musubi_fleet_export_truncated`, `musubi_fleet_export_limit` y `musubi_fleet_export_usage`, esta ultima con el uso del PEOR proyecto, no el promedio). Cableado de punta a punta: YAML → config → `ConfigurarFlota` → campo del servidor → `/metrics`. **HALLAZGO 1, QUE DIO VUELTA EL DISENO DE LA PERILLA: el almacen lee un tope `<= 0` como 50, no como «sin techo».** `AprobacionesPendientes` hace `if tope <= 0 { tope = 50 }`. Copiar la convencion de la perilla hermana —`services_per_project_export`, donde negativo ⇒ sin techo— habria hecho que escribir `-1` **APRETARA** el techo de 200 a 50 en vez de sacarlo, y sin una sola linea que lo dijera. Por eso esta perilla NO tiene apagado: clampea a 200, el almacen nunca recibe desde aca un valor que vaya a reinterpretar, y no se abre una consulta sin `LIMIT`. La guarda sostiene la PROPIEDAD «nunca devuelve <= 0» sobre nueve entradas, no tres casos sueltos. **HALLAZGO 2: la boca OTLP no exporta aprobaciones**, asi que `Aprobaciones` no puede encenderse nunca por ese camino. NO se le agrego un cuarto `avisoMientras`: seria una senal PLANA, que quien la lee interpreta como «medi y no corto» cuando en realidad es «aca no se mide» — el mismo defecto que este cabo vino a cerrar, del otro lado. Queda dicho en un comentario, con la regla de que si algun dia el empuje exporta aprobaciones hay que agregar **el calculo Y el aviso, juntos**. **HALLAZGO 3, LATERAL Y EL MAS CARO: el ratchet del arnes estaba HUECO en main, y lo encontro el propio arnes.** `arnes -correr -paquete ./internal/mcp` dio 117 corridas, 116 en ROJO y UNA EN VERDE: `TestLaDeudaDeSabotajesNoCreceYElCorpusNoSePodre`. Su sabotaje agrega EXACTAMENTE UN ancla en prosa, asi que la guarda solo puede ponerse roja si `anclasEnProsaAlDia <= pendientes` — o sea que **queda hueca con cualquier holgura >= 1**. La deuda ya estaba en 623 (la bajo otro lote de 22 anclas que no ajusto el numero) contra un techo de 642, y `holguraDelTecho = 30` no se despierta hasta pasadas las 30: entre las dos condiciones del predicado hay **una ventana de 30 anclas en la que el ratchet no mide nada y tampoco se pone roja para avisarlo**. Bajado el techo a 623, el sabotaje vuelve a dar `LA DEUDA SUBIO: 624 anclas en prosa contra un techo de 623`. **Y UNA GUARDA VECINA QUE ENUMERABA DOS DE CUATRO**: `TestElTruncadoDelExportadorSaleComoSerieYNoComoComentario` lista `[]string{"projects", "services"}` — `unreadable` ya se le habia escapado y `approvals` tampoco la habria movido. La guarda nueva DERIVA el conjunto de `truncadoDeExport` por reflexion y exige que encender un campo encienda exactamente un punto, asi que la proxima dimension no puede nacer sin serie. **VERIFICADO**: las cuatro guardas nuevas dieron ROJO con su sabotaje y con motivos DISTINTOS, que es lo que descarta que se cubran entre si. Y un tropiezo propio del metodo: el censo subia +1 con 4 directivas escritas, porque el archivo de guardas era nuevo y **sin trackear** — el censo deriva de `git ls-files`, asi que para el no existia.


**2026-09-15 · A126 CERRADO — EL `exit 2` SE LLEVABA CINCO SECCIONES Y EL VEREDICTO ENTERO, Y LA
PROPIA FILA CONTABA CUATRO.**

`verificar-despliegue.sh` salía con `exit 2` en «reglas de alerta» cuando no podía leer las reglas
cargadas de Prometheus. El veredicto era el correcto —«no vi» no es «está bien»— pero el `exit`
estaba de más. **Medido el 2026-09-15 con el puerto de Prometheus cerrado, antes de tocar nada: el
informe entero eran 32 líneas y terminaba en el título de esa sección.** Quedaban sin correr
`scrapes`, `alertmanager`, `versión`, **`el watchdog externo`** y `guiones derivados` —**cinco, no
las cuatro que decía esta fila**: omitía el watchdog— y, lo que la fila tampoco nombraba, **el
bloque del veredicto no se imprimía nunca**: la corrida salía 2 sin una sola línea que dijera por
qué. Después: **61 líneas, las diez secciones y las dos líneas de veredicto.**

**EL CÓDIGO DE SALIDA NO CAMBIÓ, y es lo que hace barato el arreglo.** `dudoso` prende
`SIN_VERIFICAR`, que el bloque final ya traducía a la misma salida 2. No se bajó ningún listón: se
dejó de confundir «esta comparación no se puede hacer» con «esta corrida se terminó». Es además el
idioma que el propio guion ya usaba para esa misma variable unas líneas más arriba, en las recording
rules del SLA.

**LO QUE APARECIÓ AL ABRIR EL CORTE — DOS DEFECTOS MÁS, LOS DOS DE LA FAMILIA DE SIEMPRE.** Era
previsible: ahí abajo ya se habían escondido cuatro de los cinco `$VAR` pegados a un carácter
no-ASCII que mataban el guion en el bash 3.2 de macOS.

1. **`scrapes` declaraba divergencias sobre cero información.** Con Prometheus mudo la lista de
   targets quedaba vacía y el bucle acusaba en ROJO a cada job del repo de «no estar en
   ...
   Prometheus; ésta era la sexta y la única que no preguntaba: la guarda estaba en cinco de seis
   caminos. «alertmanager» tenía la misma forma con su propio servicio, y ahora la resuelve
   `AM_VIVO`. Los dos pasan a `dudoso`, que dice cuántos quedaron sin mirar y por qué.
2. **Un volcado de Python crudo en medio del informe.** De dos invocaciones de `python3` hermanas,
   la de `JOBS_VIVOS` no silenciaba stderr y la de `AM_CFG` sí: cuando la respuesta no era el JSON
   esperado se imprimían diez líneas de traceback entre el título de la sección y sus veredictos.

**LAS CUATRO GUARDAS NO ENUMERAN SECCIONES** (`internal/mcp/despliegue_informe_entero_test.go`), y
ésa era la condición para que esto no vuelva: la última sección se **deriva** del guion, la
prohibición de salir se busca entre dos anclas de **código** —el primer `titulo` y la primera
lectura de `$DIVERGE`—, y las otras dos preguntan por una propiedad y no por un inventario. Los
cuatro sabotajes se corrieron y cada uno cae en su propia aserción.

**Y UNO DE LOS CUATRO SABOTAJES SALIÓ INERTE EN EL PRIMER INTENTO, lo que vale anotar.** Apagar
`PROM_VIVO` dejaba la guarda en VERDE, porque al arreglar `scrapes` le quedaron DOS compuertas
—«¿contestó?» y «¿vino vacía la lista?»— y sacar una sola sigue terminando en `dudoso`. Se declaró
entonces el que sí muerde (la mitad de Alertmanager, que tiene una sola), y queda escrito adentro
de la prueba: quien quiera sabotear la mitad de Prometheus tiene que apagar las dos.

**2026-09-11 · A125 CERRADO — LA DECISIÓN DE ALCANCE SE TOMÓ: UN REGISTRO POR TRACK.**

La fila nombraba siete cabos —`specs/riel-local/tasks.md` (3) y
`specs/profundidad-derivada-del-diff/tasks.md` (4)— que no estaban acá ni en un registro propio,
o sea exactamente lo que la primera línea de este archivo prohíbe: algo abierto sin dueño. No los
adoptaba: los contaba, para que la decisión se tomara sobre algo medido.

**gio decidió: cada track lleva su `ABIERTO.md`.** Se descartó el archivo único para todo el repo,
y el motivo queda escrito porque es lo que nadie vuelve a anotar: éste ya pasa las 3900 líneas, y
sumarle los tracks vecinos lo vuelve ilegible — que es otra forma de que un cabo se pierda, sólo
que más difícil de ver que la anterior.

Lo que existe ahora: `specs/riel-local/ABIERTO.md` con `RL1`–`RL3` y
`specs/profundidad-derivada-del-diff/ABIERTO.md` con `PD1`–`PD4`. **Los números llevan prefijo del
track**, porque con tres registros en el repo un «A1» suelto no diría de cuál archivo es.

**Y LA GUARDA CAMBIÓ DE FORMA, que es la mitad que importa.** Antes tenía un TECHO —«no más de dos
carpetas sin registro»— porque la pregunta estaba pendiente y lo único que podía hacer era avisar
si se acumulaban. Con la decisión tomada, `TestLosCabosDeLasCarpetasVecinasEstanContados` le pide
a CADA carpeta con cabos declarados lo mismo: su propio `ABIERTO.md`, **con al menos tantas filas
como cabos declara**. Un archivo vacío no alcanza: tiene que NOMBRARLOS. Así una carpeta nueva
entra al barrido sola, sin que nadie suba un número — la guarda pregunta UNA cosa en vez de N.

Sabotajes: borrar `specs/riel-local/ABIERTO.md` → ROJO; dejarlo con 2 filas contra 3 cabos → ROJO,
nombrando las dos cantidades. Control tras restaurar, verde.
...
El arreglo de la mañana estaba bien; las guardas que lo sostenían estaban ancladas en la **forma
literal** del bug viejo. Un barrido adversario las evadió once veces, y esto es lo que quedó cerrado:

- **UN ESPACIO.** Este archivo entero como estaba en `f9be15a` —las mismas 22 filas cerradas adentro
  de la tabla 1, byte por byte— con **un espacio de padding de alineación de más** en la columna del
  número pasaba en VERDE: los dos reconocedores de fila exigían exactamente un espacio detrás de la
  barra. Ya no se adivina la forma de una tabla: se **parsea** —partir por la barra respetando `\|`,
  limpiar la celda, trabajar con celdas—, y la columna que decide se pide **por su nombre**
  (`Slice`, `Por qué no`), nunca «la última».
- **EL VEREDICTO EN LA SEGUNDA ORACIÓN.** La guarda cortaba en el primer punto, así que 18 de las 22
  filas escapaban escribiendo «falta el redespliegue. Por lo demás quedó cerrado» — que es A99/A102
  con las cláusulas dadas vuelta.
- **EL VOCABULARIO.** Reconocía tres palabras sacadas de las 22 celdas que ya estaban; «hecho»,
  «resuelto», «completado», «listo» y «ya salió» pasaban. **Una lista de palabras malas siempre le
  falta la próxima**, así que la pregunta se dio vuelta: la columna que decide tiene que **empezar
  con una de las cuatro formas de asignación** que este registro usa (un slice, `sin asignar`,
  `gio`, `acción del operador`). Es una lista de formas BUENAS: una palabra de cierre nueva falla en
  vez de pasar.
- **Y EL FALSO POSITIVO DE A31 SE ARREGLÓ MIRANDO DONDE DECIDE, NO CON UNA EXCEPCIÓN.** «`VerificarFirma`
  **falla cerrado**» estaba en la celda de `Slice` porque esa celda arrastraba **4880 caracteres** de
  medición: cincuenta veces la siguiente más larga. La prosa se plegó a «Por qué no está», que es su
  columna, y ahora hay un **tope de largo** para la que decide. Por eso se puede revisar la celda
  entera —todas sus oraciones— sin una sola excepción.
- **LA TABLA 2 NO TENÍA NINGUNA GUARDA**, y se habían declarado deliberadas las dos ausencias. Se
  re-decidieron: `TestNingunaFilaDeLaTabla2EsUnCementerio` cubre a la vez la hermana de la regla 1
  (una decisión que se declara cerrada) y la **regla 3** («cada línea dice bajo qué condición se
  revisa»), que **seis de veintiuna filas no cumplían** —B3, B5, B6, B13, B20 y, con otra redacción,
  B2 y B4—. Las siete la dicen ahora.
- **EL BARRIDO DE SPECS TENÍA CUATRO AGUJEROS, Y TRES ERAN DE UNA LÍNEA.** El piso era GLOBAL, así
  que renombrar `## 2 · Lo que queda fuera` a `## Fuera de alcance` en un `spec.md` dejaba una
  sección entera invisible y la prueba pasaba con 14 secciones y 45 ítems (de 15 y 47): ahora el
  piso es **por archivo** y además un título que hable de lo que queda afuera y no se reconozca se
  denuncia como renombre. El glob **no recursaba**, así que un `spec.md` en un subdirectorio del
  track era invisible: ahora se camina el árbol. `tieneCasa` daba por registrado un ítem por las
  palabras «despliegue», «por diseño», «Cero dependencias» y `HECH` **como substring**, que son
  prosa corriente en estos specs: cada marca está anclada ahora en la forma que decide (HECHO y
  DESCARTADO tienen que decir **en qué slice**). Y sólo se reconocía la viñeta `- **`: el asterisco,
  el ítem sin negrita y el indentado **ni siquiera sumaban al contador**, así que el piso tampoco los
  veía. Con las cuatro cosas arregladas el barrido pasó de 47 a **66 ítems**, y los diecinueve que
  aparecieron destaparon **tres cabos reales sin número** en
  `specs/flota-shell-contra-sshd-real/tasks.md`, que son **B18**, **B14** y **B15** y ahora lo dicen.
- **LOS NÚMEROS DE ESQUEMA Y LOS NOMBRES DE PRUEBA NO SE CRUZABAN CON NADA**, y así se pudrieron
  A99, A102 y A116. `TestElRegistroNoCitaEsquemasNiPruebasQueNoExisten` cruza cada esquema citado
  contra `internal/memory/migrations.go` —leyendo el campo `version:` y no un comentario— y cada
  `TestXxx` citado contra el árbol; y la columna que decide de la tabla 1 no puede mandar a
  desplegar a un esquema que no sea el más alto del repo, que es el defecto de A116 exacto.
  La única cita fantasma que queda es la de la historia de `TestElTokenNuevoSePersisteAntesDeEstrenarse`,
  y se acepta porque su párrafo **dice que no existe**: la excepción no es una lista de nombres.

**2026-09-10 · EL REGISTRO DECÍA COSAS FALSAS SOBRE SÍ MISMO, Y NINGUNA DE LAS CINCO GUARDAS QUE LO
CUSTODIAN PODÍA VERLO. Tercera auditoría del propio archivo.**

**El defecto de fondo no es ninguna de las filas: es que la regla 1 no tenía guarda.** La regla dice
«al cerrar un slice, **borrar** su línea de la tabla 1». Se cumplió a mano hasta que dejó de
cumplirse, y como nadie la comprobaba, **veintiuna de las cuarenta y ocho filas de la tabla 1
declaraban en su propia celda de estado que el cabo ya estaba cerrado**. La tabla que existe para
contestar «¿qué falta?» contestaba con un 44 % de ruido, y peor: quien contara las filas para
saber cuánto queda contaba mal por casi el doble. Ahora la custodia
`TestNingunaFilaDeLaTabla1SeDeclaraCerrada`, y **no pregunta por la palabra suelta** — un grep de
«cerrado» sobre la celda da un falso positivo real que estuvo puesto todo el tiempo: **A31** dice
«`VerificarFirma` **falla cerrado**», que es el modo de fallo de una verificación de firma y no el
estado de un cabo. La guarda mira la **cabeza del veredicto** de la columna que decide.

**LAS TRES CORRECCIONES QUE NO SON DE FORMA, y las tres se midieron contra el código, no contra un
informe:**

**(1) A116 mandaba desplegar «el cerebro a esquema 48» y el repo está en 53.** Las migraciones
49…53 entraron después de la última edición de este registro, en el mismo PR (#426) que cerró
A115/A116/A120, y nada acá lo custodiaba. Y no era sólo un número viejo: **la 48 de hoy es otra
cosa** (`schema_read_floor`), mientras la columna que A116 necesita —`devices.servicios_omitidos`—
vive en la **51**. Un objetivo de despliegue obsoleto que además señala otra migración falla en la
dirección tranquilizadora: una base parada en 48 tiene ese número y no tiene la columna. Lo mismo
con A99 y A102, que decían «migración 47»: hoy la 47 es `panel_con_modelo_y_evidencia` y las dos
columnas que ellos esperan (`motivo_no_preguntar`, `token_fuente`) están en la **50**, porque la
**52** tuvo que reparar una bifurcación entre main y flota.

**(2) A99 y A102 estaban ABIERTOS escondidos detrás de la palabra «cerrado».** Sus celdas decían
«✔ cerrado; falta el redespliegue», que es una contradicción en cuatro palabras: si falta el
redespliegue, la columna no existe en la base y el cabo está vivo. Un barrido que filtre las filas
SIN «cerrado» no los ve, y el que filtre las que LO tienen los borra. Ahora su estado dice lo único
que decide: **falta el redespliegue del cerebro**, contra el esquema 53.

**(3) A92 tenía una celda de estado INVISIBLE que decía lo contrario de la visible.** En el cuerpo
había un `\| **gio** ✔ cerrado; queda consolidar a un solo agente \|` con las barras **escapadas**:
Markdown no lo dibuja como celda, así que el registro mostraba `— (cerrado)` mientras el texto de
al lado admitía un pendiente que nadie veía. Es el defecto de **B20** y **A112** por tercera vez
—una celda que existe en el archivo y no existe para quien lo lee—, y esta vez lo que se comió no
fue una palabra sino un cabo abierto. **A92 se queda en la tabla 1.**

**LA REGLA 4 ERA FALSA, y su falsedad tenía la forma que este archivo persigue: señalaba afuera
mientras el agujero estaba adentro.** Decía que `specs/riel-local/` «es EL que hoy tiene ítems vivos
sin registro propio». Medido: los tres ítems de `specs/riel-local/tasks.md` declaran su razón en el
lugar (uno dice «despachado»), y en cambio **el propio track de flota tenía OCHO ítems fuera de
alcance que ninguna guarda miraba** —tres en `specs/flota-alertas-y-politicas/spec.md` y cinco en
`specs/flota-shell-interactiva/spec.md`—, invisibles por dos agujeros que se tapaban entre sí en
`TestNingunCaboDeFlotaSeQuedaSinRegistro`: el glob miraba sólo `tasks.md`, y el regex del
encabezado rechazaba `## 2 · Lo que queda fuera` por el número y el punto medio. **Cualquiera de
los dos arreglado por separado habría dejado la mitad del agujero**, y por eso ninguna auditoría
anterior lo notó: las dos secciones fallaban por las dos razones a la vez. Uno de esos ocho
—«Cooldown persistente. Vive en memoria»— **declaraba faltante algo que A24 cerró con la migración
33** y que `TestElCooldownSobreviveUnReinicioDelCerebro` ejercita en verde: un spec mintiendo sobre
lo que falta, que es exactamente el daño que la guarda existe para impedir.

**Y una cita a código inexistente, la tercera de la serie de `cerrarSesionesColgadas`:** A109 y
`internal/config/secreto_env_test.go` nombraban `TestElTokenNuevoSePersisteAntesDeEstrenarse` como
la guarda del orden del archivo de tokens. **Esa función no existe en el repo.** La que sí lo fija
es `TestElAgenteGuardaElTokenNuevoYLoEstrena` (`cmd/musubi/agent_token_test.go`), y su aserción es
literalmente `se esperaba [viejo nuevo]`. Un doc que nombra código inexistente hace que nadie vaya
a mirar lo que sí hay.

**Se abre A123** con lo que quedaba sin número al cerrar A120: los **346 sabotajes declarados sin
veredicto**.

**LO QUE CAMBIÓ EN LAS GUARDAS.** `TestNingunCaboDeFlotaSeQuedaSinRegistro` barre ahora **todos los
`.md` del track**, no sólo `tasks.md`, acepta encabezados numerados, y **cuenta las secciones que
encontró contra un piso**: si el reconocedor se rompe, la prueba falla en vez de pasar vacía —cero
secciones tiene que significar «el barrido se rompió», nunca «no había nada que mirar».
`TestElRegistroDeAbiertosSigueEnPie` dejó de ser cuatro `strings.Contains` que pasaban con la tabla
reducida a una sola fila: ahora parsea las tablas por dentro y exige encabezados, pisos de filas y
celdas de estado no vacías. Y se suma `TestNingunaFilaDeLaTabla1SeDeclaraCerrada`, con su control
positivo y su falso positivo conocido clavados los dos.

**Las veinte filas que bajan de la tabla 1 van enteras acá abajo**, con el texto que tenían, para
que cerrar un cabo no cueste tener que ir a buscar al historial de git por qué se cerró.

**A88 CERRADO (2026-09-05) — El candado anti fuerza-bruta es por IP, así que un vecino con la credencial vieja deja afuera a la persona — y el cerebro no sabe decir quién falló**

`newAuthLimiter(5, time.Minute)` (`internal/mcp/http.go:139`) bloquea **por IP**: cinco 401 y sesenta segundos de 429 **antes de mirar el token** (`:160`); un auth bueno resetea el contador (`:184`). **Medido el 2026-09-05, rotando el token de `davantis`.** A las 00:54 —minutos de la revocación— `musubi_http_requests_total{result="unauthorized"}` arrancó a subir y no paró: 0 → 185 en una hora, entre +1 y +7 por minuto; el contador se reinició a las 01:50:18 con el reinicio de `musubi-brain.service` y siguió subiendo igual. El origen es **un `musubi daemon`** —el servidor MCP stdio de OTRA sesión de Claude Code en la misma máquina— con el token revocado **en su entorno**: un proceso lee `os.Getenv` de su copia, fijada en el `exec`, así que arreglar el archivo y el `.bashrc` no le llega nunca. **Y matarlo no alcanza**: Claude Code lo revive desde el proceso de la sesión (arrancado ANTES de la rotación), y el hijo nuevo hereda la misma credencial muerta — verificado por `sha256` del valor en `/proc/<pid>/environ`: el daemon 42142 se mató a las 01:49 y el 308056 nació a las 01:59:18 con el mismo token viejo. **El daño no lo paga el que golpea, lo paga el vecino**: gio pasó una hora comiendo 429 con una credencial válida, porque comparte IP con el ruidoso. Y el ritmo real oscilaba entre 1 y 7 por minuto contra un umbral de 5, así que la puerta se abría y se cerraba sola — lo que desde afuera se lee como mala suerte de timing y manda a diagnosticar cualquier otra cosa. **La segunda mitad del cabo es la que más duele**: el cerebro **no registra los 401 en el journal** y la métrica no lleva etiqueta de IP ni de principal, así que a la pregunta «¿quién está fallando?» el sistema no puede contestar. Hubo que muestrear `ss` en las dos puntas para nombrar al culpable. Sin atribución, una tormenta de auth es indiagnosticable desde adentro. Las dos salidas son decisiones, no obviedades: (a) que el candado no cuente sólo por IP —o que un principal válido desde esa IP compre una gracia—, y (b) etiquetar el rechazo con algo atribuible sin volverlo un registro de secretos. **2026-09-05 — CERRADO, las dos mitades.** (a) El candado se mira AHORA DESPUÉS de resolver el token (`internal/mcp/http.go`), así que una credencial válida no paga nunca por el vecino. No le regala nada a quien prueba: `resolve()` es un hash y una comparación en tiempo constante, sin I/O ni escrituras, y el que falla sigue viendo 429 al agotar sus intentos — un 429 no distingue «token equivocado» de «IP castigada», así que tampoco aprende nada. **El mismo arreglo va en `deviceDeRequest`** (`shell_agente_http.go`), donde el orden original tenía mejor razón —`DevicePorToken` consulta la base y el candado ahorraba esa consulta— pero el daño era peor: dejaba afuera al AGENTE de una máquina de la flota con su token de device válido, y la sesión de shell se caía sola sin decir por qué. Un SELECT por índice cuesta microsegundos; que un agente legítimo no entre cuesta una sesión. (b) La atribución existe: `internal/mcp/authlog.go` escribe una línea WARN con `ip`, `motivo` y `ruta`, **una por IP por minuto**, y esa línea dice **cuántas se callaron** —el freno no puede esconder el volumen, que es justo el dato que separa «un cliente roto» de «una tormenta»—, con techo en el mapa para que rotar IPs de origen no haga crecer la memoria sin límite. La atribución va al LOG y no a una etiqueta de la métrica: una serie por IP es cardinalidad sin techo, y encima la escribiría el atacante. En `/metrics` quedan dos contadores acotados: `musubi_auth_failures_total{motivo}` —`sin_credencial` vs `credencial_desconocida`, que son dos mundos distintos, uno es un cliente propio mal configurado y el otro es alguien probando— y `musubi_auth_lockouts_total`. Con esa sola etiqueta el diagnóstico de hoy habría durado segundos: habría dicho `sin_credencial` y nombrado la IP. Seis pruebas nuevas en `auth_candado_test.go`, incluida la que falla si alguien mueve el candado de vuelta arriba. **VERIFICADO EN PRODUCCIÓN** el mismo día, contra el cerebro real tras desplegar `0acf11a`: cinco intentos con un token falso desde `100.114.63.7` dieron 401, 401, 401, 401 y **429** —el quinto castiga, como dice el código— y acto seguido la credencial VÁLIDA desde esa misma IP castigada contestó **200**. Esta mañana esa última llamada era un 429 y tuvo a gio afuera siete horas. La atribución también quedó medida: el journal escribió **una sola línea** para los cinco fallos —`ip=100.114.63.7 motivo=credencial_desconocida ruta=/mcp agente=curl/8.5.0`—, o sea que el freno por IP funciona sin esconder nada, y los contadores marcaron `musubi_auth_failures_total{motivo="credencial_desconocida"} 5` y `musubi_auth_lockouts_total 1`.

**A89 CERRADO (2026-09-05) — `MUSUBI_TOKEN_FILE` está documentado como «lo recomendado» y NINGÚN `.go` lo lee: seguir el consejo desarma el daemon y abre una tormenta silenciosa de 401**

`deploy/musubi-tool.sh:21` dice textual «`MUSUBI_TOKEN_FILE=/ruta` un archivo modo 600 con el valor **(recomendado)**», y `:38` explica que `MUSUBI_TOKEN` le gana. Pero eso vale **sólo para los scripts de shell**: `grep -rn MUSUBI_TOKEN_FILE --include=*.go` devuelve **cero**. El binario lee la env var que nombra `sync.auth_token_env` con `os.Getenv`, y nada más. **Medido el 2026-09-05**: siguiendo la recomendación se cambió `export MUSUBI_TOKEN=…` por `export MUSUBI_TOKEN_FILE=…` en el `~/.bashrc`; los scripts siguieron andando —así que la señal era «arreglado»— y el `musubi daemon` quedó **sin credencial alguna**, fallando su drain cada 30 s contra el central. Resultado: `musubi_http_requests_total{result="unauthorized"}` subiendo ~2/min sin parar, el candado por IP (A88) mordiendo a la persona, y el `dead-letter` del outbox creciendo (7 → 10 en una hora) con `sent` congelado en 2034 — observaciones descartadas de a una, cada una anotando el mismo 401 en `last_error` que nadie mira. **Y hay una segunda trampa que costó tres hipótesis equivocadas**: el daemon corre con `cwd` en el repo, así que NO usa `~/.musubi/config.yaml` (donde `sync.enabled` es **false**) sino `<repo>/.musubi/config.yaml` (donde es **true**, con `central_url` y `auth_token_env: MUSUBI_TOKEN`). Leer el config del home y concluir «el sync está apagado, no pueden ser ellos» descarta la causa correcta con evidencia que parece dura — es la misma familia que las dos `memory.db`. Las salidas: (a) que el binario lea `*_FILE` como hacen los scripts —el patrón `_FILE` es estándar y el arreglo es de diez líneas—, o (b) que la documentación deje de recomendar lo que sólo la mitad del sistema entiende. Hoy la ayuda del propio repo lleva al usuario a romperse. **2026-09-05 — CERRADO por la salida (a), que era la barata.** `internal/config/SecretoDeEnv(nombre)` resuelve la variable directa o el archivo `<VAR>_FILE` —con la misma precedencia que `musubi-tool.sh`, y con `TrimSpace`, porque un archivo escrito con `echo` termina en salto de línea y ese salto viaja adentro del bearer y produce un 401 que no dice por qué—. Devuelve **error** si el archivo fue nombrado y no se pudo leer: una configuración rota merece ruido, no un secreto vacío que reaparece como un 401 sin causa. Lo usan `NewSyncClient` y `nuevoEmpujadorOTLP`, y **los dos fallan cerrado** si `auth_token_env` está declarado y no resuelve a nada. **El hallazgo lateral vale tanto como el arreglo**: `fleet_otlp.go:301` YA tenía esa regla escrita palabra por palabra —«un auth_token_env declarado que apunta a una variable VACÍA: el empuje sale sin credencial y el destino responde 401 para siempre, con la configuración puesta»— y su hermano `NewSyncClient` no la tenía. La lección estaba aprendida de un lado y no se aplicó del otro: es el mismo patrón que este archivo persigue, una fila adentro del código. Nueve pruebas nuevas entre `secreto_env_test.go` y `syncclient_credencial_test.go`.

**A91 CERRADO (2026-09-05) — El auto-heal no pasa por el eje de consentimiento: `prohibido` no lo detiene y `avisa` no avisa — y con eso el costo declarado de A86 es falso en cinco superficies**

`ConsentimientoEfectivo()` tiene **ocho llamadores y ninguno está en `politicas.go` ni en `scheduler_flota.go`** (`methods_shell.go` ×3, `methods_pantalla.go` ×4, `methods_exec.go`, más el de lectura en `methods_fleet.go`). Del lado de la máquina tampoco hay red: `cmd/musubi/agent.go` y `avisador.go` no mencionan consentimiento — el agente corre lo que hay en la cola. **Medido el 2026-09-05 corriendo el barrido real `aplicarPoliticas`** con el grado del device fijado y `puede_preguntar = true` en los tres casos: `avisa` → 1 comando encolado, `pide` → 1 comando encolado, `prohibido` → **1 comando encolado**. Y en la fila `avisa` el único comando es el `journalctl`: **no hay ningún `musubi:avisar`**. Las guardas de `actuarSiCorresponde` son cuatro —ventana de mantenimiento, cooldown, principal existe, `PuedeSobreDevice(…CapExec)`, `argvPermitido`— y su propio comentario dice «**LAS DOS COMPUERTAS, LAS MISMAS QUE PARA UNA PERSONA**»: son dos, y una persona ahora pasa TRES. El camino está vivo: `select origen, count(*) from device_commands` da `politica` = 7, todos de `auto-heal` sobre `musubi-server` con exit 0, y ese principal ya tiene `fleet.exec: ['*']`, así que lo único que hoy limita el alcance es la lista `devices` de la política. **`prohibido` está descrito en todas las superficies como el candado del dueño, que «no se abre NUNCA, aunque la capacidad esté perfectamente concedida»**: cierra los tres caminos con una persona detrás y no cierra el exec cuando la orden la dispara un temporizador. **La segunda mitad es epistémica y toca una decisión ya tomada.** El costo de A86 está escrito con estas palabras en CINCO lugares —`registry.go:1386` y su golden, `Threat_Model.md:196`, `ABIERTO.md:767`, el comentario de `methods_exec.go` y el de la celda `exec/pide` en `consentimiento_matriz_test.go`—: «una máquina en `pide` NO recibe auto-heal por exec». **Sí lo recibe.** Y el argumento con el que se eligió endurecer —«los dos errores no cuestan igual: bloquear de más SE NOTA, el auto-heal deja de actuar y alguien lo ve»— no tiene con qué cumplirse: el auto-heal no deja de actuar, y **no existe ninguna métrica ni alerta de un rechazo por consentimiento** (el grupo `musubi-politicas` tiene once reglas y ninguna lo nombra; el camino sólo emite `musubi_fleet_policy_actions_total`). Los dos filos son operativos al revés de lo temido: quien ponga `pide` creyendo que corta la automatización **no la corta**, y quien lea la doc va a bajar una máquina a `avisa` «para no perder el auto-heal», aflojando el eje por un costo que no existe. La decisión puede seguir siendo la correcta; la premisa no era cierta, y conviene saberlo antes de que alguien la cite como precedente. **No lo introdujo A86** —es anterior— pero A86 lo vuelve incoherente: endurece al humano y deja intacto al robot. **2026-09-05 — CERRADO. Decisión de gio: EL EJE GOBIERNA AL AUTO-HEAL.** `actuarSiCorresponde` tiene ahora una TERCERA compuerta —su comentario decía «las dos compuertas, las mismas que para una persona» mientras una persona ya pasaba tres—: `prohibido` frena, `pide` frena con el mismo criterio de A86 (un barrido por temporizador no tiene dónde esperar un sí, y a diferencia de un `exec` a mano no hay nadie que pueda reintentar), y `avisa` actúa **encolando el aviso**. El aviso REUSA `encolarAvisoDeAcceso`, que es todo el punto de A83: escribir una cuarta copia del bloque habría dejado la causa intacta para el quinto camino. **Y el costo declarado de A86 pasó de falso a verdadero sin que nadie tocara esos cinco textos** — hoy una máquina en `pide` de verdad no recibe auto-heal—, lo que se dejó anotado en la celda `exec/pide` de la matriz porque un texto que se vuelve cierto solo es donde más fácil se vuelve a mentir. **LA OTRA MITAD, que era la premisa de la decisión, también se cerró**: el argumento «bloquear de más SE NOTA» no tenía mecanismo, y ahora `musubi_fleet_policy_actions_total{result=~"consentimiento_.*"}` cuenta `prohibido` y `pide` SEPARADOS —el segundo mide exactamente cuánto se ganaría implementando la pregunta por política, que es la puerta que queda abierta y está dicha— con la alerta `PoliticaFrenadaPorConsentimiento` en `info` y `for: 6h`, porque una política frenada es un estado y no un evento. **La guarda que impide el QUINTO camino es lo que más vale**: no cuenta caminos sino que exige, por AST, que todo archivo que llame a `EncolarComando` llame también a `ConsentimientoEfectivo` — las dos veces que esto pasó (A83 con la shell, A91 con el auto-heal) el camino nuevo llegó en un archivo NUEVO. Su primera versión miraba el TEXTO y un comentario la dejaba en verde con el defecto puesto; lo destapó correr el sabotaje, y quedó escrito adentro. Cuatro sabotajes verificados en rojo, incluido preguntar por `avisa` antes que por `pide`, que reproduce el defecto original de A83/A85.

**A93 CERRADO (2026-09-05) — Las 11 reglas del SLA no las instala ningún guion ni las vigila ninguna alerta — y son las únicas de las tres familias sin red cruzada**

`deploy/musubi-recording.yml` (grupo `musubi-sla`, 11 reglas) **no lo copia nadie**: `deploy/docker/preparar.sh` instala `musubi-alerts.yml`, `musubi-alerts-backup-offhost.yml` y `musubi-alerts-flota.yml` y ninguna más; `install-musubi-prometheus.sh` sólo la primera; `grep -rn musubi-recording deploy/ --include=*.sh --include=*.yml` no devuelve nada fuera del encabezado del propio archivo; y `redesplegar-cerebro.sh` no menciona reglas. **Ninguna alerta las cuenta**: de las 55 reglas cargadas, las dos que usan `prometheus_rule_group_rules` son `ReglasDelCerebroSinDesplegar` (`!= 20`) y `ReglasDeFlotaSinDesplegar` (`!= 24`) — es decir, las otras dos familias **se vigilan cruzadas a propósito, cada una desde el archivo de la otra, exactamente contra este modo de falla**, y el SLA quedó afuera del patrón. Mientras tanto `prometheus_rule_group_rules{...musubi-recording.yml;musubi-sla}` = 11. **Que se instalan a mano se ve en las edades**: `count_over_time(musubi:device_up:norm[30d])` da 143 muestras (11,9 h) y `count_over_time(musubi:service_up:cobertura30d[30d])` da 75 (6,25 h) — **tres momentos de instalación distintos, ninguno el del despliegue**. Y este despliegue es justo el que cambió el archivo (`git diff a243281..b97a81c -- deploy/` da `musubi-recording.yml` con 60 líneas agregadas, por `bb80781` y `4cf9641`). Hoy no hay daño: las 11 expresiones vivas son idénticas al repo, verificado una por una. **El riesgo es el próximo despliegue**: si nadie lo copia, las reglas se quedan viejas o desaparecen, los `avg30d` siguen dando un número plausible, y las dos guardas que existen siguen en `inactive` porque cuentan otros dos archivos. **Segundo problema del mismo archivo, independiente**: publica «92 %» y «69 %» rotulados **a 30 días** calculados sobre el **1,65 % de la ventana** —11,9 h de historia—, y la cobertura que se agregó justamente para poder decirlo no la lee ninguna alerta. Un SLA que se le firma a un cliente no puede ser lo único sin red. **2026-09-05 — LA MITAD DEL DESPLIEGUE, CERRADA (`35b26f2`).** `preparar.sh` ahora lo instala atado a las reglas de flota (van juntas o ninguna: sus once reglas derivan de `musubi_fleet_*` y su guarda vive dentro del archivo de flota); `ReglasDelSlaSinDesplegar` lo cuenta desde `musubi-alerts-flota.yml`, con el conteo custodiado por `TestCadaArchivoDeReglasCustodiaElConteoDelOtro` para que no se pudra; `verificar-despliegue.sh` dejó de ser ciego —filtraba `type == "alerting"` y globeaba `musubi-alerts*.yml`— y ahora distingue las cuatro respuestas en vez de dos, para lo cual el archivo declara por fin su `# despliegue: condicional`; y `verificar-cobertura.sh`, que tenía el mismo defecto un nivel arriba (su lista de ARCHIVOS era a mano, con el argumento contra las listas a mano escrito al lado), destapó una métrica que nadie verificaba: `musubi_fleet_service_declared`. Más la sección del RUNBOOK, cuya falta la denunció el propio `TestCadaRunbookDeUnaAlertaApuntaAUnaSeccionQueExiste`. Tres sabotajes verificados. **LO QUE SIGUE ABIERTO ES LA OTRA MITAD Y ES UNA DECISIÓN**: los `avg30d` se publican rotulados «a 30 días» calculados sobre el 1,65 % de la ventana, y `musubi:service_up:cobertura30d` —la serie que existe justamente para poder decirlo— no la lee ninguna alerta. Las salidas no son equivalentes: (a) alertar cuando la cobertura baja de un umbral, (b) que el número se publique con su cobertura al lado y no desnudo, o (c) no publicar un «30 d» hasta tener 30 d. **2026-09-05 — CERRADO TAMBIÉN ESTO. gio lo dejó a criterio, y lo que se hizo no fue ninguna de las tres tal cual, porque medirlo cambió la pregunta.** Primero: **las cuatro series de `:cobertura30d` YA existían** —device, project, service y project_service— así que «publicar la cobertura al lado» no era el trabajo. El trabajo era que **alguien la consuma**, y no la consumía nada: ni una alerta, ni un panel, ni un guion. Y no hay consumidor del SLA en el repo todavía, así que el riesgo es FUTURO y el mecanismo tiene que sobrevivir a quien construya el reporte. **Ahí está el punto**: quien lo construya va a escribir `avg30d`, porque es el nombre que suena a lo que busca; un número al que hay que acordarse de pedirle su cobertura al lado se publica solo. Así que la defensibilidad viaja EN LA SERIE: **`musubi:*:sla30d`** —device, project y service— es el mismo promedio pero **existe únicamente si la cobertura llega a 0,95, y si no DESAPARECE**. Un panel dibuja un hueco, que es la verdad, en vez de un 92 % que no se sostiene. `avg30d` queda como diagnóstico y así está rotulado. **El umbral es 0,95 y el motivo está escrito**: con `interval: 5m`, 30 días son 8.640 muestras, así que el 5 % son 36 h sin medir — y un SLA de uptime se escribe contra 99,9 %, que son 43 minutos al mes, o sea que un mes con día y medio DESCONOCIDO no puede sostener una afirmación de ese orden. **Y NO SE PUSO LA ALERTA DE «cobertura baja», que era la opción (c), porque medirla la descartó**: no se puede apagar arreglando algo —la única salida es que pase el tiempo— y una alarma que no se apaga actuando es cómo se enseña a ignorar un canal, que este repo ya pagó con los trece `MaquinaCaida` de A79. Lo accionable es la CAÍDA: mientras el TSDB acumula historia la cobertura sólo sube, así que un `delta` negativo significa que algo se rompió —TSDB que perdió datos, scrape cortado, reglas recargadas con otra definición—, y eso lo vigila `CoberturaDelSlaSeCayo`. Tres guardas nuevas: que toda familia con promedio tenga su `:sla30d`, que ese `:sla30d` esté ACOTADO por la cobertura de su familia (un `:sla30d` que fuera copia del promedio es peor que no tenerlo, porque su nombre promete lo que no cumple), y que los tres umbrales sean el MISMO número — en YAML no hay constantes y tres números iguales escritos a mano es cómo uno se queda viejo. Tres sabotajes verificados; borrar un `:sla30d` pone en rojo tres guardas a la vez.

**A98 CERRADO (2026-09-05) — El actualizador de Windows elegía la instalación por «el primer proceso musubi que aparezca», y en una máquina con dos eligió la equivocada — pero lo grave es que el cambiador obedece a una tarea GLOBAL desde cualquier carpeta**

`deploy/actualizar-agente-windows.sh` resolvía la carpeta de instalación con `Get-Process musubi \| Select-Object -First 1`. En `davantis-1` conviven **doce** procesos `musubi.exe` en **dos** carpetas: tres del agente de flota en `AppData\Local\Musubi` y nueve de la app de escritorio en `AppData\Local\Programs\musubi` —cinco `daemon` y cuatro `cerebro`, la app corriendo su propio cerebro local—. **Medido el 2026-09-05**: el `-First 1` eligió la de la app, y la actualización entera fue ahí: bajó el binario, refrescó el cambiador y lo lanzó. El agente de flota nunca se tocó y siguió reportando la versión vieja, así que **el despliegue se veía fallido por el motivo equivocado**. **Lo que hace a este cabo peor que un bug de selección**: `cambiar-agente.cmd` deriva su carpeta de `%~dp0` —bien— pero su paso [1] para la tarea **por nombre global** (`Musubi Agente de Flota`, única por máquina). O sea que una copia del cambiador en CUALQUIER carpeta **para al agente real** y después cambia un binario que no es el suyo. Eso pasó: se detuvo el agente de verdad desde la carpeta de la app. Lo único que evitó el destrozo fue una casualidad —esa carpeta no tiene `device.token`, así que el paso [4] no pudo probar el binario nuevo y el rollback lo devolvió—. Una casualidad no es una defensa. **Y la cautela ya estaba escrita a dos archivos de distancia**: el propio `cambiar-agente.cmd` nombra explícitamente la carpeta de la app de escritorio para no matarla con un `taskkill /IM`. Sabía que había dos instalaciones, y el guion de al lado elegía a ciegas. Es la forma dominante de defecto de este código: entre esta sesión y musubi-00 se encontraron **diez casos en un día**, en subsistemas que no se tocan entre sí. La pregunta que rinde no es «¿esto está mal?» sino **«¿quién más hace esto, y tiene la misma cautela?»** — y su corolario práctico: **si el que lo hace bien lo dice en un comentario, el hermano que no lo hace está a un `grep` de distancia**. **2026-09-05 — ARREGLADO, con revisión adversaria de por medio.** (a) El guion elige por `device.token` —la credencial del dispositivo, el mismo archivo que usa el paso [4] del cambiador— y **se para ante 0 o ante 2**; además cada paso IMPRIME la carpeta elegida, que antes no se sabía hasta el paso 5, cuando ya había escrito dos archivos adentro. (b) `cambiar-agente.cmd` gana su propio candado: si su carpeta no tiene `device.token`, **no toca la tarea ni ningún binario** (`exit /b 5`). Es la defensa que faltaba, y va en el archivo que ejecuta la acción, no sólo en el que la decide. (c) De la revisión salieron cuatro defectos PREEXISTENTES que se arreglaron de paso: el extractor de la bitácora tiraba `stderr` y `error`, así que cualquier falla imprevista salía como un «exit=1» mudo —y el timeout del ejecutor, que escribe `error` y deja `exit_code` en None, salía como «exit=None» sin motivo—; la espera local estaba clavada en 180 s mientras el paso 3 pedía 300, así que el guion se rendía **con el comando todavía corriendo** y el `trap` le bajaba el servidor HTTP debajo a un `fetch` a medio bajar; el paso 5 pegaba la ruta dentro de una cadena sin comillas para `cmd`; y el recorte del mensaje a 200 caracteres cortaba justo donde estaba la segunda carpeta. **Los dos que quedaban abiertos se cerraron el mismo día.** (d) El guion tiene ahora sus guardas en `internal/mcp/despliegue_actualizador_agente_test.go`: **cinco pruebas, cada una con su sabotaje nombrado y verificado en rojo** — que no vuelva el `-First 1` (mirando línea por línea y salteando comentarios: la primera versión se disparaba con el comentario que explica el cabo, o sea que la documentación del arreglo rompía su propia guarda), que el resolver se pare ante 0 y ante 2, que el candado del cambiador esté **antes** del `schtasks /end` —el orden es la propiedad, no la presencia—, que el cambiador siga siendo ASCII puro, y que el guion no cante victoria sin confirmar. (e) **El verde final ya no lo da la versión sola**: cuando la fila reporta la versión nueva, el guion pide una confirmación EN LA MÁQUINA de que hay un proceso vivo corriendo desde la carpeta del agente, y si no lo hay falla diciendo que la versión la escribió el latido de prueba y que la tarea no arrancó. **Y la prueba se rompió a sí misma antes de servir, de la forma más instructiva posible**: el archivo se llamó primero `despliegue_agente_windows_test.go`, y Go lo IGNORÓ ENTERO — un archivo terminado en `_windows_test.go` lleva restricción de build implícita por GOOS y sólo compila en Windows. `go test` contestó `ok` y `go vet` pasó; las cuatro pruebas no existían. Es el defecto que este archivo custodia, una vuelta más adentro.

**A101 CERRADO (2026-09-05) — «¿Quién más hace esto?» sobre A89: eran SIETE lugares, no dos — y los dos que ya lo hacían bien discrepan entre sí sobre cuál gana**

A89 arregló `NewSyncClient` y `nuevoEmpujadorOTLP`. Aplicada la pregunta que rinde, el barrido dio **cinco más**, uno de ellos en el MISMO `internal/mcp/http.go` que se había tocado: `resolveServiceAuth` —el token del cerebro central—, `internal/provision/provision.go`, el canal `musubi cerebro`, el dashboard, la shell interactiva y `internal/cognition/factory.go`. Todos leían con `os.Getenv` a secas la variable NOMBRADA por un campo `auth_token_env`/`TokenEnv`, así que para esos caminos el archivo `<VAR>_FILE` sencillamente no existía: quien siguiera la recomendación del repo quedaba sin credencial y sin aviso. **Los siete usan ahora `config.SecretoDeEnv`**, y la pregunta quedó hecha guarda permanente en `TestNadieLeeUnTokenNombradoSinElRespaldoDelArchivo` — con control de «miró algo», que se ganó el sueldo en el acto: la primera versión revisó **cero** archivos porque la raíz se pasa como `../..` y su `Name()` empieza con punto, así que el filtro de ocultos se saltaba el repo entero. Sin ese contador habría quedado verde sobre la nada, que es el defecto que la guarda persigue. **LO QUE QUEDA ABIERTO ES UNA DECISIÓN, Y TIENE EVIDENCIA DE HOY.** Los dos lugares que YA hacían bien esto discrepan en la precedencia. `deploy/musubi-tool.sh:38` —y `SecretoDeEnv`, que lo copió— hacen ganar a la **variable**: «una variable puesta a mano es una decisión más explícita». `cmd/musubi/agent_token.go` hace ganar al **archivo**, y su argumento es mejor: «es el único de los dos que puede rotar; si están las dos puestas, la que manda es la que puede sobrevivir a la rotación». **El 2026-09-05 se pagó exactamente eso**: la terminal de gio tenía un `MUSUBI_TOKEN` revocado que le ganó en silencio a un `MUSUBI_TOKEN_FILE` correcto, y el arreglo terminó siendo derivar la variable del archivo (`export MUSUBI_TOKEN="$(cat ~/.musubi/token)"`) — o sea, rodear la precedencia en vez de usarla. Con archivo-gana, ese rodeo sobra. El costo de cambiarla también es real: hoy se puede pisar un archivo con una variable para probar con otro principal, y eso dejaría de funcionar. Las dos salidas son legítimas; lo que NO puede seguir es que el mismo repo enseñe las dos. **Y LA DISCREPANCIA TIENE UNA SEGUNDA DIMENSIÓN, que apareció el mismo día arreglando otra cosa**: no es sólo QUÉ GANA, es TAMBIÉN QUÉ SIGNIFICA UN ARCHIVO DE VARIAS LÍNEAS. Para `cmd/musubi/agent_token.go` (`tokensDeArchivo`) varias líneas son una **lista de tokens, el más VIEJO primero** (se apendea; ver la corrección al pie de A101), y existe justamente para que una rotación tenga fallback. Para `SecretoDeEnv` varias líneas son un **error** — y con buen motivo, medido: el `TrimSpace` saca el `\n` del final y nada más, así que dos líneas devuelven un secreto con un salto ADENTRO, y eso no da 401 sino que `net/http` **se niega a mandar el pedido** con «invalid header field value», un error que apunta a la biblioteca HTTP, o sea al único lugar donde la causa no está. Rechazar es lo correcto acá —inventar el formato multi-token para un camino que no lo tiene sería elegir en silencio entre dos credenciales, que es el defecto de todo este día— pero deja al sufijo `_FILE` significando **dos cosas distintas según quién lo lea**. La decisión de gio tiene que cubrir las dos dimensiones a la vez: qué gana, y qué es un archivo de varias líneas. **2026-09-05 — DECIDIDO Y CERRADO. Gio eligió: GANA EL ARCHIVO en los tres, y un archivo de varias líneas es un ERROR salvo en el del agente.** La razón que decide es la del agente y es de mecanismo, no de gusto: el archivo es el único de los dos que puede rotar. Con eso el rodeo `export MUSUBI_TOKEN="$(cat ~/.musubi/token)"` sobra. **Y apareció un tercer camino que nadie había contado**: `musubi-tool.sh` YA mandaba la PRIMERA LÍNEA de un archivo multi-línea en silencio —medido: viajó un `Bearer` con `exit=0`—, así que las semánticas de `_FILE` eran TRES y no dos. Peor: la primera línea es el token MÁS VIEJO, porque el formato de lista se apenda (ver A109), o sea que elegía justo el que la rotación estaba retirando, y ese camino no tiene reintento. **Lo que se tocó**: `SecretoDeEnv` invertido —y si el archivo está nombrado DECIDE, incluso para fallar: caer a la variable convertiría una configuración rota en una credencial silenciosa distinta de la que se pidió, que es A89 por la puerta de al lado—; `musubi-tool.sh` invertido y rechazando multi-línea; y **los cuatro lugares que TIRABAN el error del archivo** (`dashboard.go` ×2, `shell.go` ×2), que era la parte peligrosa: sin eso el riel decía «falta $MUSUBI_TOKEN» con la variable PUESTA — una causa falsa que manda a arreglar lo que no está roto. **Verificado corriendo, no leyendo**: `deploy/pruebas/precedencia-del-token.sh` levanta un oidor, corre el guion de verdad y mira QUÉ BEARER LLEGÓ. Con las dos puestas viaja `Bearer tok-DEL-ARCHIVO`. Es de comportamiento a propósito, por la lección de A107: una guarda de grep sobre ese guion se dejaría satisfacer por cualquiera de las decenas de veces que dice «archivo» sin elegir nada. Sus dos sabotajes corridos —volver a que gane la variable, y volver a mandar la primera línea— cada uno cazado por la aserción que le toca.

**A103 CERRADO (2026-09-05) — La confirmación del actualizador no podía ver a la mitad de los zombis que decía custodiar: miraba UNA ruta y filtraba por nombre de proceso**

La confirmación de A98 enumeraba con `Get-Process musubi` y se quedaba con los procesos cuyo `Path` fuera `<carpeta>\musubi.exe`. Las dos mitades de esa línea la dejaban ciega justo donde importa. **(1)** El paso [2] de `cambiar-agente.cmd` RENOMBRA el binario en uso, así que el agente anterior sobrevive corriendo `musubi.exe.viejo` — el propio cambiador lo tiene escrito y medido desde el 2026-09-02 y por eso mata las DOS rutas; la confirmación miraba una. **(2)** El nombre de proceso de Windows es el del archivo sin su última extensión, así que ese proceso se llama `musubi.exe` y NO `musubi`: `Get-Process musubi` no lo devuelve nunca. O sea que el chequeo escrito el mismo día para cazar zombis no podía ver al zombi típico. Otra vez la forma dominante: la cautela completa estaba en `cambiar-agente.cmd` y llegó a medias a su hermano. **CERRADO el 2026-09-05.** Los bloques de PowerShell compartidos —`RESOLVER` y `CLASIFICAR`— se mudaron a `deploy/lib-agente-windows.sh`, que cargan por `source` el actualizador y el matador nuevo: tener dos copias es programar la próxima divergencia. `CLASIFICAR` enumera por RUTA, las dos rutas, e imprime las horas en ISO (la salida del incidente decía «09/05/2026 01:14:38» y no había forma de saber si era el 9 de mayo o el 5 de septiembre: el formato sale de la config regional de la máquina remota). Guardas `TestLaClasificacionVeTambienAlZombiDelBinarioRenombrado` y `TestLosDosGuionesDeWindowsCarganElMismoLibYNoUnaCopia`, con sus ocho sabotajes corridos en un worktree limpio — porque medirlos en el árbol compartido dio tres falsos «rojo»: musubi-00 estaba escribiendo un `_test.go` que no compilaba, y con el build roto TODO da distinto de cero. **MEDIDO DESPUÉS por musubi-00 en `davantis-1`, y corrige un matiz**: `Get-Process musubi` da **14** (3 agentes + 11 de la app de escritorio), `Get-Process 'musubi.exe'` da **0**, `Win32_Process` con `Name='musubi.exe'` da **14** y el filtro por `ExecutablePath` da **3**. O sea que `Get-Process musubi` SÍ devuelve el caso normal —importa decirlo, porque el error opuesto sería creer que no sirve para nada—; lo que ninguno de los dos métodos habituales devuelve es el proceso de `musubi.exe.viejo`, cuyo `ProcessName` es `musubi.exe` y cuyo `Name` de WMI es `musubi.exe.viejo`. **Los dos métodos que uno elige primero pierden exactamente el caso central.** Con eso apareció un agujero más, arriba de donde estaba mirando: el RESOLVER enumeraba con `Get-Process musubi`, así que si los únicos procesos del agente fueran zombis de `.viejo` —cambiador renombró, instaló, y `schtasks /run` falló— no encontraba la carpeta y `matar-zombis-agente.sh` quedaba ciego JUSTO en el estado para el que existe. Ahora enumera por la hoja de la ruta. Y la guarda de eso falló dos veces antes de servir: primero la satisfacía la CADENA DE UN MENSAJE DE ERROR que nombra `musubi.exe.viejo`, después se quedaba con la última línea que menciona `$cands` —ese mismo mensaje— y daba rojo por el motivo equivocado. Lo mostró el control sin sabotaje, que es la única línea que distingue «mi guarda funciona» de «acá todo falla».

**A104 CERRADO (2026-09-05) — El mensaje de abajo re-diagnosticaba lo que el de arriba ya había medido, y su remedio empeoraba el caso**

Cuando la confirmación en la máquina falla, el PowerShell devuelve la razón exacta —una de cuatro— y `actualizar-agente-windows.sh` la imprimía… y acto seguido escribía encima: «la versión llegó pero NO quedó ningún agente corriendo: el cambiador probó el binario y la tarea no arrancó», más un `schtasks /run`. Eso es UNA de las cuatro razones, fija. **El 2026-09-05 la máquina estaba en otra**: quedaban zombis, el PowerShell dijo «lo que falta es matar esos procesos» y la línea de abajo lo tapó con una causa falsa y un remedio que EMPEORA el caso — `schtasks /run` agrega un proceso más y deja al zombi vivo latiendo con la versión vieja. Un mensaje genérico encima de uno preciso es peor que ninguno: el preciso costó una medición y el genérico la borra. **CERRADO el 2026-09-05.** El guion ya no re-diagnostica: dice que la razón es la línea de arriba y ofrece el remedio de cada una de las cuatro. Y el remedio de la que apareció existe ahora como herramienta, `deploy/matar-zombis-agente.sh`: mata sólo los procesos arrancados ANTES del binario instalado, **se niega si no queda ninguno del binario nuevo** (matarlos dejaría la máquina sin agente y sin canal para arreglarla) y mata DESPEGADO con ocho segundos de retraso, porque el zombi puede ser el mismo agente que levantó el comando del cerebro y un `Stop-Process` que se lleva a su propio ejecutor no devuelve respuesta — y desde acá «no contestó» se lee igual que «el cerebro se cayó».

**A105 CERRADO (2026-09-05) — La guarda que existe para que ningún scrape quede sin vigilar no ve un `job_name` entre comillas simples — y con eso premia el defecto y castiga el arreglo**

`TestLosJobsVigiladosSonLosQueElRepoDeclara` es la guarda de A73: cruza los `job_name` de `prometheus.yml` contra los que vigila la alerta `ScrapeQueElRepoDeclaraYNoExiste`, porque un job que no llega a desplegarse deja ciego todo lo que dependa de sus métricas y se ve en verde. Su patrón aceptaba `"?`, o sea comillas dobles o nada. **`- job_name: 'altura-db'` es YAML válido y es el estilo de media documentación de Prometheus**, y era invisible. **Medido el 2026-09-05 en las dos direcciones, y la segunda es la que lo hace dañino**: (1) declarar un job con comillas simples y NO sumarlo a la alerta deja la guarda en VERDE — el agujero de A73 exacto, entrando por la puerta que la guarda no mira; (2) declararlo con comillas simples Y sumarlo a la alerta —lo correcto— la hace FALLAR con «la alerta vigila el job "altura-db", que prometheus.yml no declara: nunca va a existir y la alerta queda encendida para siempre», o sea que manda a SACAR la línea buena. Premia el defecto y castiga el arreglo: peor que no mirar. Salió de auditar con `deploy/pruebas/sabotaje.sh` los sabotajes declarados de la familia de guardas que leen archivos NO-Go —46 en 15 archivos, la familia con el peor historial del día— porque el selector mecánico de musubi-00 sólo alcanza literales que viven en un `.go` de producción. **CERRADO el 2026-09-05.** El patrón lee el valor ENTERO hasta el fin de línea y después le saca las comillas, en vez de describir con una clase de caracteres qué puede tener un nombre: una clase se queda corta en silencio, y `"un.job.raro@2"` se leía cortado en el `@`. Y se le agregó el control de «los vi a todos» —contar las líneas `job_name:` sin interpretar el valor—, porque el modo de falla no era una aserción equivocada sino un recorrido que no llegaba. Los dos sabotajes corridos en rojo, con el control sin sabotaje en verde.

**A106 CERRADO (2026-09-05) — La guarda del redespliegue decía en su propio comentario haber cerrado este agujero, y no lo había cerrado: los inodos podían estar sólo en el MENSAJE**

`TestSeVerificaElINODOYNoElIsActive` custodia la lección más cara de `redesplegar-cerebro.sh`, escrita en la cabecera del guion: «`systemctl is-active` decía `active` y el proceso corría un binario BORRADO». Por eso el redespliegue compara el inodo de `/proc/$PID/exe` con el del binario en disco y llama a `volver_atras` si difieren. **La guarda ya había fallado una vez y su comentario lo cuenta**: buscaba los nombres de las variables en todo el archivo y pasaba con el `is-active` de vuelta, porque los nombres seguían apareciendo. El arreglo de entonces fue exigir los tres tokens EN LA MISMA LÍNEA. **No alcanzaba, y el sabotaje declarado lo mostró el 2026-09-05**: cambiar `[[ "$INODO_PROC" == "$INODO_DISCO" ]] \|\| volver_atras "…(inodo $INODO_PROC vs $INODO_DISCO)…"` por `systemctl is-active --quiet musubi-brain \|\| volver_atras "…el mismo mensaje…"` deja una línea que TODAVÍA tiene los tres tokens —el mensaje de diagnóstico nombra las dos variables— y la guarda quedaba en VERDE sobre un redespliegue que vuelve a decidir por `is-active`. O sea: el arreglo de un defecto de «la cadena está en el archivo» se convirtió en el mismo defecto acotado a una línea, y **la explicación del arreglo describe exactamente el mecanismo que lo deja pasar**. **CERRADO el 2026-09-05.** La propiedad es POSICIONAL: los inodos van en lo que se EVALÚA, o sea antes del `volver_atras`; lo que viene después es texto para el humano y no decide nada. Verificado en las dos direcciones con el quinto argumento de `sabotaje.sh`: el sabotaje la pone en rojo, y un cambio correcto —reescribir la comparación con `test` en vez de `[[ ]]`— la deja en verde.

**A107 CERRADO (2026-09-05) — La auditoría de las 46 guardas que leen archivos NO-Go: cuatro más que su propio sabotaje declarado no rompía, y las cuatro por la misma razón**

Salió de correr `deploy/pruebas/sabotaje.sh` sobre la familia de guardas de `internal/mcp` cuyo archivo de producción es un `.sh`, un `.yml`, un `.cmd` o un `.py` — 46 sabotajes declarados en 15 archivos, que el selector mecánico de musubi-00 no alcanza porque sólo mira literales que viven en un `.go` de producción, y que es la familia con el peor historial del día. **Las cuatro tienen la misma forma: la guarda pregunta si un TEXTO está, y el texto está en un lugar que no decide nada.** (1) `TestElRedespliegueNoTipeaLaVersionDeEsquema` exigía `version --esquema` en cualquier parte del guion, y esa cadena aparece TRES veces: en el comentario que explica el arreglo, en el mensaje de aviso, y —la única que ejecuta— en la asignación de `$ESPERADO`. Cambiar esa asignación por `ESPERADO=46`, que es el defecto exacto que la prueba prohíbe, dejaba las otras dos en pie y la guarda en verde. Ahora exige una sustitución `$(… version --esquema …)` en código, no en texto. (2) `TestConstruirNoIntentaCorrerUnBinarioDeOtraPlataforma` exigía `GOHOSTOS`, que aparece en la línea que calcula el valor por defecto de `DESTINO_OS`: reemplazar el `if` entero por `if true; then` la dejaba en verde, y con eso una compilación cruzada vuelve a intentar correr un binario de Windows en Linux. Ahora exige una CONDICIÓN que compare las dos dimensiones contra las del host. (3) `TestElColectorDelRelayCuentaLasSondasIntentadasYNoLasQueContestaron` miraba un PREFIJO: `"atendidas": len(PUERTOS)` se satisface con `"atendidas": len(PUERTOS) - len(caidos)`, que es literalmente el defecto —contar sólo los puertos que contestaron, con lo que `fallidas` supera a `atendidas` y el cerebro descarta el reporte entero—. Un prefijo no identifica un valor: hay infinitas expresiones que empiezan igual y significan lo contrario. Ahora exige la expresión completa. (4) `TestPrepararGuardaLaIdentidadDelRelayYNoMienteSobreLoQueProtege` buscaba «NO protege» en todo el archivo, y esa cadena exacta —con las dos mayúsculas— vive en UN SOLO lugar: el comentario. El LEEME dice «No protege» y la línea de salida «no protege». Borrar las DOS salvedades que el operador ve dejaba la guarda en verde. Ahora mira sin comentarios y sin distinguir mayúsculas: lo que importa es que el guion se lo DIGA a alguien. **CERRADO el 2026-09-05**, las cuatro verificadas en las DOS direcciones con el quinto argumento de `sabotaje.sh` —el sabotaje las pone en rojo Y un cambio correcto las deja en verde—. **Números del lote**: 37 sabotajes corridos, 6 cabos reales (A105, A106 y estos cuatro), y **la mitad de los «verdes» eran anclas mías mal puestas**, no defectos: le pegué a un comentario que la guarda descarta a propósito, a un archivo que la guarda no lee, y a un `poner` que tenía un segundo escritor más abajo. Ningún verde se registró sin leerlo.

**A108 CERRADO (2026-09-05) — El drop que evita que cada alerta de flota salga duplicada se daba por presente con un COMENTARIO — y sus tres piezas podían vivir en scrapes distintos**

`TestElScrapeYElEmpujeNoTraenLoMismo` custodia la receta que hace que el scrape y el empuje OTLP no traigan lo mismo: sin ella cada máquina manda dos series y **cada alerta de flota sale duplicada** —pasó el 2026-08-28 al encender el empuje—. La guarda pedía tres cadenas: `metric_relabel_configs`, el `regex` de las dos familias empujadas, y `action: drop`. **Las pedía sueltas, en todo el archivo.** Dos fallas, la primera medida con el sabotaje declarado: (a) `metric_relabel_configs` aparece en un COMENTARIO de `prometheus.yml` que explica la receta, así que renombrar los DOS bloques reales dejaba la guarda en verde — séptimo caso del día de un texto que nombra la cautela y la satisface; (b) aun sin ese comentario, las tres cadenas podrían vivir en scrapes DISTINTOS: el `action: drop` de un job y el `regex` de otro darían por buena una receta que no existe en ninguno. **CERRADO el 2026-09-05.** Se mira el BLOQUE: desde cada `metric_relabel_configs:` de código hasta el próximo `- job_name:`, y las dos piezas tienen que estar adentro del mismo. Verificado en las dos direcciones — el sabotaje la pone en rojo y reordenar las claves adentro del bloque, que es un cambio legítimo, la deja en verde. **Con esto la auditoría de las 46 queda cerrada**: 46 sabotajes corridos, **7 cabos reales** (A105, A106, los cuatro de A107 y éste), y la mitad de los verdes intermedios fueron anclas mías mal puestas. El patrón único detrás de los siete: **la guarda pregunta si un TEXTO está, y el texto está donde no decide nada** — un comentario, un mensaje de error, un prefijo de la expresión, o un vecino.

**A109 CERRADO (2026-09-05) — Tres afirmaciones del repo que fueron verdad y dejaron de serlo: el orden del archivo de tokens, la causa que da el aviso de arranque, y «`service:` es el último bloque»**

Salieron de una investigación en paralelo sobre los cabos pendientes, y las tres tienen la misma forma — un texto que describe un estado ANTERIOR y se lee como el actual. **(1) EL ARCHIVO DE TOKENS NO TRAE «EL MÁS NUEVO PRIMERO».** Cinco lugares lo afirmaban (`secreto_env.go`, su prueba, y tres filas de este registro). `apendarToken` AGREGA AL FINAL —y su comentario explica por qué: un reemplazo atómico crea una entrada de directorio que no es durable sin fsync del directorio—, así que **la primera línea es el token MÁS VIEJO**. Lo fija `TestElAgenteGuardaElTokenNuevoYLoEstrena` (`cmd/musubi/agent_token_test.go`), que exige `[viejo nuevo]` **—corregido el 2026-09-10: esta fila y el comentario de `secreto_env_test.go` nombraban `TestElTokenNuevoSePersisteAntesDeEstrenarse`, una función que NO existe en el repo—**, y `Usar()` devuelve `viejo`. No es un detalle de redacción: quien lea eso y decida «me quedo con la primera línea» —conclusión razonable, y la que `set /p` de Windows toma sola— elige justo el token que la rotación está retirando, y ninguno de esos caminos reintenta: 401 permanente en vez de un error legible. Corregido en los cinco, con guarda `TestNadieDiceQueElArchivoDeTokensTraeElMasNuevoPrimero` que recorre el repo. La guarda se disparó con su PROPIA documentación en la primera corrida —la trampa de A107 al revés: allá el comentario satisfacía la guarda, acá la rompía—, así que la frase prohibida se arma por partes y no vive escrita en el archivo. **(2) EL AVISO DE ARRANQUE NOMBRABA LA CAUSA EQUIVOCADA.** `avisarQueConfigGobierna` cerraba con «manda el del directorio de trabajo», y en esta máquina es FALSO: los daemons corren sin `MUSUBI_HOME` pero con `CLAUDE_PROJECT_DIR`, así que la raíz sale de la variable y el cwd sólo coincide — medido en `/proc/<pid>/environ`. **El propio A96 lo creía al revés y lo dejó escrito.** Un diagnóstico que nombra la causa equivocada manda a mirar el cwd, que se puede cambiar, en vez de la variable, que decide — y esa línea existe justamente para matar la hipótesis equivocada que costó tres horas. Ahora `workspaceDirConOrigen` devuelve la raíz Y por qué es ésa, y el aviso lo dice. Guarda `TestElOrigenDeLaRaizSeDiceYNoSeAdivina`, con el caso de que los tres orígenes tienen que ser DISTINTOS entre sí: una cadena genérica pasaría las otras aserciones sin informar nada. **(3) EL INSTALADOR DEL CEREBRO SE COMÍA EL BLOQUE `sync`.** `install-musubi-brain.sh` reescribía el bloque `service:` con `sed -i '/^service:/,$d'`, justificado en «'service:' es el último bloque del config generado por 'musubi init'». Hoy `Default().Marshal()` pone `service:` en la línea 119 y `sync:` en la 123: medido generando el archivo, el `sed` lo dejaba en 118 líneas y **el bloque `sync` desaparecía**. O sea que re-correr el instalador borraba la configuración de sync del cerebro en silencio, y el `.bak` de al lado se pisa en cada corrida. Ahora un `awk` borra sólo hasta la próxima clave de primer nivel, y se escribe con `cat >` y no con `mv` para conservar inodo y etiqueta (A82 y la lección de SELinux). Guarda `TestElInstaladorDelCerebroNoSeComeLoQueVieneDespuesDeService`, con la contraparte de que el bloque `service` se sigue escribiendo — si no, «arreglarlo» borrándolo entero pasaría la guarda dejando al cerebro sin daemon. Las tres con su sabotaje corrido y su control en verde.

**A110 CERRADO (2026-09-05) — La guarda del verificador de blindaje fuera de Linux la satisfacía un COMENTARIO, y el sabotaje que su propio doc nombraba no podía compilar**

`TestElVerificadorSeCallaFueraDeLinux` custodia que `musubi agent --revisar-blindaje` se CALLE fuera de Linux: `ReadWritePaths` es una directiva de systemd, y en un Windows —dos de las tres máquinas de esta flota— emitiría `ReadWritePaths=C:\Users\gio\...`, que es un consejo equivocado dicho con confianza, el modo de falla exacto que A54 existe para cerrar. La prueba leía `blindaje.go`, cortaba el cuerpo de `revisarBlindajeDelAgente` y pedía que contuviera el literal `runtime.GOOS != "linux"`. **Medido el 2026-09-05 con el sabotaje más plausible que hay** —comentar la guarda para probar el otro camino, que es un debug que cualquiera commitea sin querer: `// if runtime.GOOS != "linux" {` seguido de `if false {`. El paquete COMPILA, el binario vuelve a escupir directivas de systemd sobre rutas con backslash, y la prueba quedaba en **VERDE**, porque el literal seguía ahí — en el comentario. Es la falla 2 de `deploy/pruebas/sabotaje.sh`. **Y el sabotaje que el propio doc nombraba («quitar la guarda») NO PUEDE COMPILAR**: los dos únicos usos de `runtime` en el archivo viven dentro de ese bloque, así que borrarlo deja el import huérfano — un rojo por build roto se lee igual que un rojo por guarda que funciona (falla 5), o sea que el doc mandaba a comprobar la red con un gesto que nunca la comprueba. **CERRADO el 2026-09-05 (66f24ae).** El arreglo no es endurecer el grep: el sistema operativo sale a un seam (`sistemaDelAgente`), como ya se hizo con el reloj del agente, y la prueba EJECUTA la función con `windows` y `darwin` y mide lo que hace. Eso cierra las dos direcciones a la vez — un comentario ya no la engaña, y una reescritura legítima (un `switch`, un helper `esLinux()`, un build tag) tampoco la pone en rojo. Cuatro sabotajes corridos, **dos defectos distintos**: `if false` / la versión comentada / la condición invertida dan rojo en tres aserciones (código ≠ 0, habló de `ReadWritePaths`, no dijo de qué sistema hablaba) y **dos de esas tres no dependen de que esta máquina esté sin blindar**; y `if true` —callarse en TODAS las plataformas, el defecto que los dos primeros brazos no ven— da rojo con un mensaje distinto, y por eso existe el tercer brazo. La otra dirección probada en las cuatro corridas. **Y hay un modo de no-compilar que es ESTRUCTURAL y explica los 12 sabotajes que habían quedado sin veredicto**: cuando la guarda es el ÚNICO lector de una variable o el único uso de un import, borrarla deja `declared and not used` / `imported and not used`, así que el sabotaje literal no puede compilar nunca. La salida es no tocar la guarda: apagarle la ENTRADA o INVERTIRLA. Corregido también el doc de `cmd/musubi/servicios_test.go`, que decía «sacar el `if !vivo`» — **y el hermano `internal/memory/latido_test.go` ofrece la misma frase y ahí SÍ compila**, porque en ese archivo `vivo` se vuelve a leer más abajo: la misma oración, verdadera en un lado y falsa en el otro, y la diferencia no es la guarda sino cuántos lectores tiene la variable. **Y una falla 8 nueva en la herramienta (a3af5ea): una guarda NEGATIVA no se sabotea sacando lo prohibido, se sabotea agregándolo** — sacar lo prohibido es un no-op y su verde dice «no la toqué», no «no cubre». Dos de los «hallazgos» de la auditoría del día eran esto y su hermana (apuntarle a la prueba que ejercita el MÉTODO cuando el sabotaje toca al LLAMADOR), así que el veredicto VERDE del guion ahora ofrece esas dos explicaciones —con el grep para encontrar la hermana— antes de acusar a la guarda.

**A111 CERRADO (2026-09-05) — El script de redespliegue que CORRE en el cerebro está a la mitad del del repo, y su verificación de la migración está muerta desde el esquema 38 — pasó así en los seis redespliegues del 4 y del 5**

Medido el 2026-09-05 preparando el esquema 46 → 47. `/home/musubi/redesplegar-cerebro.sh` tiene **9690 bytes contra 19469** del repo, y dos defectos reales. **(1) La verificación de la migración no verifica nada**: dice `[[ "$ESQUEMA" -ge 37 ]] \|\| volver_atras "la migración no corrió: el esquema quedó en $ESQUEMA y se esperaba 37"`. Con esquema 46 y con 47 esa condición es **vacuamente cierta**, así que pasa aunque la migración no haya corrido — y pasó así en los seis redespliegues que dejaron `musubi.antes-de-*` el 4 y el 5. Es la forma exacta de A107 (una comprobación que se satisface sin comprobar) y encima el arreglo YA ESTÁ EN EL REPO —le pregunta `version --esquema` al binario y compara por IGUALDAD, con el tercer estado «este binario no sabe decirlo»— y no llegó a la máquina, que es A102 otra vez con otro archivo. **(2) La receta de vuelta atrás a mano restaura la base SIEMPRE** (`cp -a $RESPALDO $BASE`), así que tirar de ella cuesta todo lo que el cerebro escribió desde el respaldo incluso cuando el esquema no cambió; el repo ya distingue los dos casos (74f4239). Lo que allá SÍ está y es sólido: la verificación por INODO de `/proc/$PID/exe` contra el archivo en disco llama a `volver_atras` y no es un aviso, y el `/healthz` exige 200 y no «cualquier respuesta». **LA DERIVA NO ES GENERAL, Y ESO ES LO QUE LA HACE INVISIBLE**: de los dos scripts derivados que viven en el server, `/usr/local/bin/musubi-backup` coincide **byte a byte** con `deploy/musubi-backup.sh` (`631b9bd…`, 8318 bytes) y el del redespliegue no. Uno se mantiene al día y el otro no, y **nada los compara**: `deploy/verificar-despliegue.sh` nació de A73 justamente para cruzar lo que el repo dice contra lo que el servidor corre, y cruza las reglas de alerta y no los scripts —el hermano sin la guarda, otra vez—. Su contrato explica por qué (le pregunta a las APIs y no a los archivos, porque un archivo correcto que Prometheus no releyó se ve igual que uno bueno), y para un script de shell esa razón no aplica: no hay daemon que lo relea, el archivo ES lo que corre en el momento en que se lo invoca. **Y EL REDESPLIEGUE NO PUEDE SALIR POR EL CANAL DEL AGENTE**, que es el otro hallazgo de la misma medición: `musubi_fleet_exec` en `musubi-server` corre como `uid=1000(musubi)` y `sudo -n true` contesta «the "no new privileges" flag is set, which prevents sudo from running as root», así que la unidad lo cierra por diseño y `redesplegar-cerebro.sh` exige EUID 0. El server tampoco tiene repo ni Go (sí `curl`, `wget`, `scp`, `rsync`, `base64`, `python3`), así que hay que cruzarle los ~34 MB desde afuera. El binario del esquema 47 está armado y verificado: `0.131.0-flota.d6f623d`, sha256 `05ab835050e9e1f3ba9b57836f40c7257d8c722c21d6f40ce09b8f144ba62fe2`, reproducible (dos builds del árbol limpio dieron el mismo sha). **CORRECCIÓN DE LA MISMA FECHA:** acá seguía «y sin el sufijo `-sucio` que `construir.sh` le pone solo cuando el árbol tiene cambios sin commitear, que es la prueba de que no se lleva trabajo de nadie», **y eso era FALSO** — ver A114. El chequeo usaba `git diff`, que NO ve los archivos sin trackear, y `go build` los compila igual: un `.go` que no está en ningún commit viajaba adentro del binario con la versión declarando que el árbol estaba limpio. La conclusión resultó cierta porque el árbol estaba limpio de verdad, no por el razonamiento — que es la peor forma de tener razón, y la frase ya estaba escrita acá y en un mensaje a gio sobre un binario que se iba a desplegar. La prueba que sí vale es el `vcs.modified=false` que Go embute en el binario y se lee con `go version -m`. **Y un sha nunca viaja solo: viaja con su commit.** `d6f623d` quedó viejo en horas —entre dos commits de ese día el único cambio fue el RUNBOOK y el sha cambió igual, porque la versión embute el commit— y reproduce desde un clon o checkout completo en ESE commit, **no** desde un git worktree: ahí Go no estampa `vcs` en absoluto y el módulo queda en `(devel)`, así que la respuesta no es «distinta», es «ninguna». **2026-09-05 — LA MITAD DE REPO, CERRADA. Y el arreglo no fue la deriva, fue por qué derivó.** De los dos guiones derivados, el que coincidía byte a byte tenía compuerta de sha256 en `install-musubi-brain.sh` y el que derivó llegó a mano y no lo instalaba nadie: **la diferencia no fue el cuidado de nadie, fue el mecanismo**, así que arreglar sólo la copia vieja habría dejado el mismo agujero para el próximo redespliegue. Tres piezas. **(1) `verificar-despliegue.sh` ya cruza los guiones**, que es el hermano sin la guarda que este registro viene repitiendo: nació de A73 para cruzar repo contra producción y cruzaba las reglas de alerta y no los scripts. Su contrato daba la razón —le pregunta a las APIs y no a los archivos, porque un archivo que Prometheus no releyó se ve igual que uno bueno— **y esa razón vale para una configuración que lee un daemon y no para un guion de shell**: no hay quien lo relea, el archivo ES lo que corre cuando se lo invoca. Corregido el ítem del contrato, no agregado como excepción al final. Y son **cuatro respuestas y no dos**: `corre_alla` se traga los errores con `\|\| true`, así que `sha_alla` devuelve el sha, o `AUSENTE`, o `ILEGIBLE`, o vacío, y el vacío sale por `dudoso` (exit 2) — si no, la sección entera se pondría verde el día que no haya ssh. **(2) El redespliegue tiene instalador**, igual que el backup, con `REDESPLIEGUE_SHA256` pineado y su guarda de frescura `TestElPinDelGuionDeRedespliegueEsElVerdadero`, hermana de la del backup — que ya se pudrió una vez, el mismo día que se creó y por el mismo autor que había avisado por escrito. **(3) Y va a `/usr/local/sbin`, no al home de `musubi`, que es un hallazgo aparte y más serio que la deriva.** El guion se corre con `sudo` y vivía en `/home/musubi/`: un archivo que ejecuta root en un directorio que escribe el uid 1000 — **el mismo uid con el que corre `musubi_fleet_exec` en ese servidor**, medido en esta misma fila. Quien alcance el canal del agente podía dejar código escrito ahí y esperar al próximo redespliegue. **Medido después, y la medición cambió la guarda**: ninguna unidad de systemd ni entrada de cron cruza el límite hoy (las dos unidades que apuntan a `/home` corren con `User=musubi`, y no hay nada en `/etc/cron.d`, `/etc/crontab` ni el cron de root), así que la exposición es exactamente ésta y no hay una segunda puerta; `/home/musubi` es `drwx------`, o sea que no es «cualquier usuario» sino específicamente el uid del agente; **pero son DIECISÉIS archivos ejecutables con la misma forma** en ese directorio —`arreglar-principals.py`, `backup-secrets.sh`, `b1-instalar-timer.sh`, `b1-adjudicar.sh` entre otros—, así que la propiedad que vale custodiar no es «el redespliegue está en `/usr/local/sbin`» sino **«ningún destino de instalación cuelga de `/home`»**, y eso es lo que exige la guarda. La copia vieja NO se borra sola —la puso una persona— pero el instalador avisa y el verificador la marca en rojo aunque su contenido esté al día: el problema no es qué dice, es quién puede reescribirla. **Y una guarda de un piso más arriba**, porque agregar una fila a mano dejaba el mismo agujero para el TERCER guion, que es A93 otra vez: `TestCadaGuionQueSeInstalaEnElServidorSeCompara` **deriva la lista** del instalador —todo `install -m … "$DESTINO"` **de código**, resuelto a su ruta— y exige que esté fuera de `/home` y presente en la tabla. El binario del cerebro es la única excepción y está nombrada con su razón (el repo no lo trae, se compila; su deriva la mira `musubi version`). **Ocho sabotajes corridos, ocho rojos, tres con su otra dirección**, y uno era mío: escribí la membresía como `strings.Contains` sobre TODO el verificador, así que **un comentario que nombrara la ruta la habría satisfecho** — el defecto dominante del día, en la guarda que existe justo para cerrarlo. Lo cazó saboteando con la ruta mudada a un comentario. Se agregó además `deploy/pruebas/guiones-derivados.sh`, que **extrae `sha_alla` del archivo de producción y la corre**: un grep quedaría satisfecho porque la función menciona las tres cadenas y el `case` las lista, y saboteando el escape del `$1` del awk se ve que devuelve vacío —o sea «no pude preguntar» sobre archivos que están—. **LO QUE QUEDA ES LA MÁQUINA Y ES DE gio.** Ojo: el binario `0.131.0-flota.d6f623d` de más arriba quedó viejo (HEAD se movió), hay que rearmarlo del árbol limpio. Y el verificador va a dar DOS rojos ciertos hasta que se cruce: `/usr/local/sbin/redesplegar-cerebro.sh` todavía no existe y el legado sigue en `/home/musubi/` con sus 9690 bytes del 2 de septiembre. **2026-09-05 — CERRADO TAMBIÉN EN LA MÁQUINA.** gio corrió el redespliegue (el `sudo` del servidor pide clave, así que no podía salir por acá ni por el canal del agente). Verificado desde el repo con `MUSUBI_SSH=musubi-server ./deploy/verificar-despliegue.sh`, **salida 0**: `/usr/local/sbin/redesplegar-cerebro.sh` coincide byte a byte con el del repo, no quedó copia en el home de `musubi`, y el cerebro corre `0.131.0-flota.d532172`. **Y la verificación de la migración —lo que esta fila denunciaba como muerto— por fin verificó de verdad**: esquema en disco **47** contra el **47** que declara el binario, comparado por IGUALDAD y no con el `-ge 37` vacuamente cierto que pasó en los seis redespliegues anteriores. `healthz=200`, punto de retorno en `pre-redespliegue-20260905-180913.db` y `musubi.antes-de-20260905-180913`, las cuatro máquinas de la flota reportando después del reinicio, y `musubi_fleet_device_token_rotable` —la capacidad que trae el esquema 47— ya expone su serie.

**A112 CERRADO (2026-09-05) — La mitad reciente de ESTE registro no se renderizaba como tabla, y la guarda que existe para eso contaba 47 filas y decía que estaba todo bien**

Encontrado el 2026-09-05 corriendo el sabotaje sobre `TestTodaFilaDeAbiertoTieneLasCeldasDeSuEncabezado` antes de creerle a su verde — le saqué una celda a una fila recién agregada y **la guarda no se movió**. Dos defectos encadenados. **(1) LA TABLA ESTABA PARTIDA EN A92.** Su fila había quedado sin la barra de cierre ni la celda de dueño, y detrás venía —adentro de la tabla— una línea en blanco, un blockquote de corrección (`> **CORRECCIÓN DE UNA MEDICIÓN MÍA…**`, sobre el `49b26b2` que medí mal) y una línea de prosa suelta. En Markdown **un blanco TERMINA la tabla**, así que de A93 en adelante las filas se dibujaban como texto suelto con barras verticales: **dieciséis filas —A93 … A111, o sea todo lo del 4 y el 5 de septiembre— dejaron de leerse como registro** justo en lo más reciente, que es lo que alguien abre primero. Es el daño de B20 (una celda que el renderizador se come) multiplicado por dieciséis. **(2) LA GUARDA NO PODÍA VERLO, POR DOS RAZONES DISTINTAS.** Su `esFila` exige que la línea empiece Y termine en barra vertical, así que la fila de A92 no era «fila» sino «una línea cualquiera» y caía en el `case esperadas > 0: esperadas = 0` — «se terminó la tabla», callado. Y su chequeo de la línea en blanco mira sólo la PRÓXIMA línea no vacía para decidir si la tabla sigue: acá esa línea era el blockquote, así que concluyó que la tabla había terminado de verdad. **Y su control de «miró algo» lo aprobaba**: pedía al menos 3 tablas y 30 filas, y había 4 tablas y 47 filas contadas — un umbral fijo que el archivo ya había superado hace tiempo, así que el control decía «revisé bastante» mientras un tercio de la tabla 1 no se revisaba. **CERRADO el 2026-09-05.** La corrección quedó PLEGADA adentro de la celda de A92 —una corrección va en la celda de su fila, nunca como bloque suelto entre filas— y A92 recuperó su barra de cierre y su dueño. Y la guarda tiene un control nuevo que no envejece: **toda fila con número de registro del archivo tiene que estar entre las visitadas**, y si falta se la NOMBRA — con eso «algo no se revisó» pasa a ser «buscá la fila anterior a A93». Se cuenta contra lo que el archivo TIENE, no contra un número escrito a mano. Verificado con tres sabotajes: sacarle la barra de cierre a A92 pone en rojo el control nuevo y **sólo** ese —17 filas nombradas, A92 incluida—, sacarle una celda pone en rojo la cuenta de celdas, y meter un blanco entre dos filas pone en rojo el chequeo del blanco; los tres con mensajes distintos, así que son tres defectos y no uno contado tres veces. La otra dirección también: agregar una fila nueva bien formada la deja en verde, o sea que el crecimiento normal del registro no la molesta. **Y esta fila se rompió a sí misma dos veces mientras la escribía**, las dos cazadas por la guarda nueva: primero por llevar barras verticales crudas adentro de backticks —que NO protegen, que es el defecto original de B20— y después porque mi propio re-pegado se comió un separador. **La lección de método es la que más vale**: esta guarda ya había fallado DOS veces por dejar de mirar, las dos veces se arregló, y las dos veces el arreglo se dio por bueno sin sabotearlo. Un umbral fijo como control de cobertura es un falso verde con fecha de vencimiento: el día que el archivo lo supera, el control sigue contestando «sí» y deja de significar nada.

**A113 CERRADO (2026-09-05) — `musubi-alerts-flota.yml` está desplegado A MEDIAS —faltan 3 de 27— y una de las tres que faltan es justamente la alerta que avisaría de esto**

Medido el 2026-09-05 con `MUSUBI_SSH=musubi-server ./deploy/verificar-despliegue.sh`, la primera corrida contra el servidor después de A111. **Lo medido, sin interpretar:** Prometheus tiene 44 reglas cargadas y el repo declara 52; las 20 que se despliegan «siempre» están todas; de `musubi-alerts-flota.yml` hay **24 de 27** y faltan `CoberturaDelSlaSeCayo`, `PoliticaFrenadaPorConsentimiento` y `ReglasDelSlaSinDesplegar`; y las recording rules del SLA son **11 contra las 14** que declara `musubi-recording.yml`. Las tres que faltan entraron con `62d4b65` (A93) y `ad2ee9b` (A91): **cerradas en el repo y nunca llegadas a la máquina**, que es A102 y A111 otra vez con otros archivos. **LA FORMA ES LA DE A73 Y ESTA VEZ SE PUEDE VER EL DAÑO.** `ReglasDelSlaSinDesplegar` existe para decir exactamente una cosa —«las recording rules del SLA que corren no son las que declara el repo»— y el hueco que vigila **está abierto ahora mismo**: 11 contra 14. No es que la alerta esté sin desplegar y no pase nada; está sin desplegar Y el defecto que reportaría es real y lleva horas ahí sin que nadie se entere. Una alarma que no está desplegada no es una alarma que falta: es un hueco que se ve en verde. **Y LA PARTE QUE TODAVÍA ES INFERENCIA, DICHA COMO TAL.** El repo cruza los conteos a propósito desde A93 —`ReglasDeFlotaSinDesplegar` vive en `musubi-alerts.yml` y hoy espera **27**, y `ReglasDelSlaSinDesplegar` vive en el archivo de flota y espera 14—, y `TestCadaArchivoDeReglasCustodiaElConteoDelOtro` mantiene esos números frescos EN EL REPO. En el servidor hay 24 cargadas, así que si el `musubi-alerts.yml` desplegado esperara 27 **`ReglasDeFlotaSinDesplegar` tendría que estar disparada — y no lo está**: las que disparan son `DiscoPorLlenarse`, `MusubiSiempreViva` y `ServicioCaido`. De ahí la inferencia: el archivo desplegado sigue esperando el conteo viejo, o sea que **un archivo viejo certifica su propio despliegue incompleto**, y el cruce entre familias no protege contra eso porque el número viaja adentro del mismo archivo que quedó atrás. Lo que la deja como inferencia y no como medición: el `for:` de esas alertas es de 30 min y el verificador sólo lista las que YA disparan, no las pendientes. **Lo resuelve una línea**, y hace falta ssh: `ssh musubi-server 'grep -A2 ReglasDeFlotaSinDesplegar /etc/prometheus/*.yml \| grep -oE "!= *[0-9]+"'` — si dice 24, la inferencia se confirma y esto deja de ser «faltan reglas» para ser «la guarda cruzada tiene un punto ciego estructural». **Por qué no se vio antes**: `verificar-despliegue.sh` SÍ compara las reglas —para eso nació en A73— y funciona; lo que no hay es quién lo corra sin que alguien lo pida. La corrida de hoy salió de A111 y por casualidad. **CONFIRMADO el 2026-09-05, y es peor que «faltan tres reglas».** El conteo desplegado da **`!= 24`**, así que la inferencia se sostiene. Pero al mirarlo entero aparecieron **TRES archivos viejos, no uno**, y sus fechas cuentan la historia: `musubi-alerts.yml` del 4-09 01:15 (20 alertas, y su cruce espera 24 para flota cuando el repo declara 27), `musubi-alerts-flota.yml` del 4-09 14:09 (24 alertas contra 27, y `ReglasDelSlaSinDesplegar` **no está en el archivo en absoluto** —grep da 0—), y `musubi-recording.yml` del 4-09 23:13 (11 recording del SLA contra 14). **EL PUNTO CIEGO ES ESTRUCTURAL Y NO ES UN OLVIDO.** El cruce de A93 —cada archivo custodia el conteo del otro— protege contra que se quede viejo UN archivo. **Pero el despliegue los copia juntos**, así que se quedan viejos JUNTOS: el `musubi-alerts.yml` viejo espera 24 y el `musubi-alerts-flota.yml` viejo tiene exactamente 24, con lo cual **`ReglasDeFlotaSinDesplegar` está en verde precisamente porque el despliegue entero está atrasado**. La guarda no falla por estar mal escrita; falla porque las dos mitades que se vigilan viajan en el mismo paquete, y un número que viaja con lo que vigila no lo vigila. La defensa que sí funcionó es la que NO viaja: `verificar-despliegue.sh` compara contra el repo, que es la única punta que no se copia — y por eso lo vio en la primera corrida. **Y hay una medición que lo dice mejor que cualquier explicación: el `musubi-alerts.yml` desplegado difiere del repo en UN SOLO RENGLÓN, y ese renglón es el número de la guarda** —`!= 24` contra `!= 27`—. Los dos archivos pesan los mismos 23912 bytes, porque «24» y «27» ocupan dos caracteres. O sea que el archivo que vigila el despliegue del otro estaba **al día en todo salvo en la única cifra con la que vigila**, y guardaba justamente el valor que hace pasar el despliegue incompleto por correcto. No se quedó viejo el archivo: se quedó vieja la expectativa, que es lo único que ese archivo aporta a esta pregunta. **Lo que se necesita para arreglarlo NO es root**: el directorio es `/home/musubi/musubi-prometheus/rules/` y es del usuario `musubi` (`rw-r--r-- musubi:musubi`), así que cruzar los tres archivos y recargar Prometheus se hace sin sudo. **Ojo con SELinux al cruzarlos**: hay que escribirlos con `cat > archivo` y no con `scp`/`install`/`mv` encima, porque un archivo nuevo estrena etiqueta y el contenedor deja de leerlo — y un reload con archivos ilegibles contesta 200 y carga CERO reglas, que es el modo de falla que `preparar.sh` documenta. Después hay que comprobar el conteo cargado por la API, no que el reload haya dicho OK. **Nota de alcance**: el otro rojo de la misma corrida —el guion del redespliegue— es A111 y está pendiente de gio; éste es independiente y **no lo arregla el redespliegue del cerebro**: son archivos de reglas de Prometheus, no el binario. **2026-09-05 — CERRADO.** Los tres archivos cruzados con `cat >` —conservando inodo y etiqueta `container_file_t`, con respaldo `*.bak-20260905-180959` de los tres antes de tocarlos— y Prometheus recargado. **Comprobado por la API y no por el reload**, que es la parte que importa: un reload sobre archivos ilegibles contesta 200 y carga CERO reglas. Cargadas ahora: `musubi-alerts-flota.yml` **27** (eran 24), `musubi-alerts.yml` 20, `musubi-recording.yml` **14** (eran 11); las cuatro alertas presentes, incluidas las tres que faltaban y `ReglasDeFlotaSinDesplegar`, que ahora espera 27 y tiene 27. `verificar-despliegue.sh` sale **0**. Antes de cruzarlo se verificó que ninguna de las tres alertas nuevas dependiera de métricas del esquema 47 —cero referencias a `token_rotable`/`token_fuente` en los tres archivos— y que `musubi_fleet_policy_actions_total` ya existiera en el cerebro viejo, así que este arreglo era de verdad independiente del redespliegue.

**A114 CERRADO (2026-09-05) — `construir.sh` declaraba limpio un binario que se llevaba código sin commitear — y la frase «sin sufijo `-sucio`» ya estaba escrita en este registro y en un mensaje a gio, sobre un binario que se iba a desplegar**

La cabecera de `construir.sh` dice que un build sucio «no se prohíbe —a veces hace falta probar algo rápido— se DECLARA», porque un binario con cambios sin commitear no se puede reconstruir: el commit que anuncia NO es el código que corre. Ese contrato se apoyaba en `git diff --quiet && git diff --cached --quiet`, y **`git diff` no ve los archivos SIN TRACKEAR** mientras `go build` compila todo el `.go` del directorio, rastreado o no. Así que el caso PEOR —código que se despliega y no está en NINGÚN commit, el único que no se puede auditar ni reproducir después— era exactamente el que el guion daba por limpio. Medido el 2026-09-05 en un clon aislado con un `cmd/musubi/zz_chivato.go` sin trackear: `git status --short` lo muestra, el chequeo viejo dice LIMPIO, la versión sale `0.131.0-flota.ac75fec` **sin** `-sucio`, el **sha256 CAMBIA** (o sea que el archivo viajó adentro) y el `vcs.modified` que Go embute solo dice **`true`** — lo cazaba, y nadie lo miraba. **DÓNDE DOLIÓ:** la frase «sin el sufijo `-sucio`, que es la prueba de que no se lleva trabajo de nadie» estaba escrita en la fila de A111 y en un mensaje a gio sobre el binario del esquema 47. Los dos binarios en cuestión estaban limpios de verdad, así que la conclusión sobrevivió — **por suerte y no por el razonamiento, que es la peor forma de tener razón.** **CERRADO el 2026-09-05.** El arreglo no fue calcular mejor: fue dejar de calcular dos veces. Había DOS respuestas a «¿estaba limpio el árbol?» —el sufijo que decide el guion y el sello que pone Go— y no se comparaban; dos fuentes de verdad sobre la misma pregunta es cómo se podre una herramienta. Ahora el sufijo arranca como CONJETURA (con `git status --porcelain`, que además es la misma semántica de Go) y después se **CORRIGE contra el sello**: si discrepan, manda el sello —es lo que viajó adentro— y el guion relinkea, avisa que su propio chequeo tiene un defecto, y recién entonces imprime la versión y el sha. Relinkear cuesta ~3 s medidos, porque la compilación está en caché y sólo se paga el link. **Con eso el sufijo deja de ser una afirmación sobre el árbol y pasa a ser una lectura de lo que de verdad se compiló**, y una conjetura rota en cualquiera de las dos direcciones queda curada sola. Se agregó también que el guion DIGA qué lo ensucia (la lista, no sólo el sufijo: con los sin trackear adentro del chequeo, la causa más probable es un archivo que alguien olvidó) y que el sha se imprima UNA sola vez, la del artefacto final — antes salía el sha equivocado y después el corregido, y quien copiara el primero pinearía un archivo que ya no existe. **Y UN HALLAZGO HERMANO, MÁS PELIGROSO QUE EL DEL SUFIJO:** el sha256 **no es una propiedad del commit**, es una propiedad de (commit, tipo de checkout). Medido con el MISMO commit: checkout normal y clon fresco dan `fd4bedb202ad7ffa…` los dos, y un **git worktree** da `81d5e74a2ce0c6ef…`. La causa la dice `go version -m`: en un worktree el módulo es `(devel)` y NO aparece ninguno de los sellos `vcs=git` / `vcs.revision` / `vcs.time` / `vcs.modified`; en un checkout de verdad están los cuatro, y van adentro del binario. Dos consecuencias que hay que decir donde vive el pin: **(a)** un verificador honesto que compile en un worktree ve un mismatch y lo lee como binario adulterado —casi le dije al par que su sha estaba mal por esto—; y **(b)** en un worktree el sello no dice «limpio», **no dice nada**, así que preguntar «¿`modified` es `false`?» y leer la ausencia como un sí es la falla 5 de `deploy/pruebas/sabotaje.sh` con otro traje. El guion ahora distingue los tres estados y avisa fuerte en el vacío. **LA GUARDA NO ES UN GREP, A PROPÓSITO.** Un grep de `--porcelain` lo satisfacería un comentario —el defecto dominante del día, nueve veces— y encima habría puesto en ROJO el arreglo bueno, porque la palabra «untracked» ya no aparece en el guion: la falla 7. Se mide conducta con un arnés, `deploy/pruebas/sufijo-sucio.sh`, cableado por `TestUnBinarioConCodigoSinCommitearLoDiceEnSuVersion`. Corre en un CLON —nunca en un worktree, por lo de arriba—, comprueba las dos direcciones **y la premisa**: que el archivo sin trackear de verdad cambie el binario, comparando los dos sha. Sin esa tercera aserción el arnés mediría la etiqueta y no el peligro. Sabotajes: con la conjetura vieja Y el relink anulado sale en 1 con «UN BINARIO QUE SE LLEVA CÓDIGO SIN COMMITEAR NO SALIÓ MARCADO»; la reescritura legítima de la conjetura (`git status -s`) lo deja en verde. Y uno que NO lo pone en rojo, dejado escrito adentro del arnés para que ese verde no se lea como hueco: marcar SIEMPRE queda curado por el relink, porque el sufijo es una lectura y no una afirmación. **Dos errores míos durante el arreglo, los dos del catálogo de este repo:** el mensaje de error del arnés llevaba backticks adentro de un `echo` con comillas dobles, **así que bash los EJECUTÓ** y la salida traía el resultado de `git diff` incrustado; y medí el código de salida con `$?` después de un pipe, o sea el del `tail`. Las dos son señales con la forma de la respuesta contestando otra pregunta, adentro de la herramienta escrita para cazar eso.

**A115 CERRADO (2026-09-05) — La única defensa que caza «arreglado en el repo, nunca llegado a la máquina» corre sólo cuando un humano se acuerda — y todas las que corren solas tienen su expectativa adentro del paquete que vigilan**

**La medición, del 2026-09-05.** `verificar-despliegue.sh` encontró A111 y A113 en su primera corrida contra el servidor en días, y esa corrida salió de casualidad —yo estaba cerrando otra cosa—. Lo que había estado pasando mientras tanto: las reglas de flota llevaban **desde el 4-09 desplegadas a medias** (24 de 27) con la alarma correspondiente en VERDE, y el guion del redespliegue llevaba **desde el 2-09** a la mitad del del repo con su verificación de la migración muerta, que pasó así en seis redespliegues. **Y no lo corre nada**: cero menciones en `.github/`, cero timers de usuario y cero entradas de cron, en las DOS máquinas — comprobado, no supuesto. No es por falta de dónde: el servidor tiene 19 timers y cinco entradas de cron andando. **POR QUÉ LAS GUARDAS QUE SÍ CORREN SOLAS NO ALCANZAN, QUE ES EL CONTENIDO DE ESTA FILA.** Hay tres desatendidas y las tres tienen la expectativa adentro de lo que vigilan. (1) `ReglasDeFlotaSinDesplegar` y `ReglasDelCerebroSinDesplegar` se custodian cruzadas —cada archivo guarda el conteo del otro, que fue la respuesta correcta de A93— **pero el despliegue las copia juntas, así que envejecen juntas y sus dos números se siguen dando la razón**. Medido: el `musubi-alerts.yml` desplegado difería del repo en UN renglón, `!= 24` contra `!= 27`, con los dos archivos pesando los mismos 23912 bytes. La guarda estaba en verde precisamente porque el despliegue entero estaba atrasado. (2) La comparación de versión del cerebro mira `VERSION`, que lo bumpea una persona (dicho ya en A93 y en A109). (3) `musubi_fleet_device_agent_stale` mira los agentes, no lo que se despliega en el servidor. **La propiedad que falta es una sola: la expectativa tiene que viajar en un paquete DISTINTO del que vigila.** Lo único que hoy la cumple es el repo, porque el repo no se copia a la máquina — y por eso lo vio el verificador y no lo vio ninguna alerta. **LAS SALIDAS, QUE SON UNA DECISIÓN Y NO UN TRÁMITE.** (a) **Un timer en la máquina de gio**, que es la que tiene repo y ssh. Barato y funciona hoy. Su defecto es real y ya lo pagamos en B13: es una estación de trabajo, no un servidor —se la midió fuera de línea el mismo 5-09— y un programador que no está siempre encendido convierte «sin novedad» en «todo bien». (b) **Un job de CI programado**, que no depende de ninguna máquina de la casa, pero necesita que el runner entre al tailnet. (c) **Que la expectativa viaje en el BINARIO**: el cerebro embute en compilación los conteos —o los sha— que el repo declara y los expone como métrica, y la alerta compara `prometheus_rule_group_rules` contra eso. Rompe exactamente el acoplamiento que falló hoy, porque el binario y las reglas se despliegan con mecanismos distintos (`redesplegar-cerebro.sh` contra `preparar.sh`); no cubre desplegar todo junto desde un commit viejo, que es un problema menor y lo mira la comparación de versión. **(d) Y ésta hace falta con cualquiera de las tres**: que quien corra la comparación empuje un latido, y que una alerta suene cuando ese latido se pone viejo. Sin eso, elegir (a), (b) o (c) sólo mueve el silencio de lugar — «nadie lo corrió» seguiría siendo invisible, que es la forma exacta de lo que esta fila describe. El latido lo evalúa Prometheus, que vive en el servidor, así que para (a) es un verdadero dead-man externo y no el vigilante adentro del cajón. **2026-09-05 — CONSTRUIDO (a) + (d), y desplegado.** `deploy/comparar-y-latir.sh` corre el verificador y empuja dos gauges por OTLP con su código de salida; las tres alertas nuevas viven en `musubi-alerts.yml` (20 → 23, con el conteo cruzado del archivo de flota actualizado a 23) y ya están cargadas en producción. El timestamp viaja explícito adentro del valor y no se calcula con `timestamp()`, porque esa función devuelve la hora de EVALUACIÓN y una serie muerta contesta «hace 0 min» — un dead-man construido sobre eso no dispara nunca. Units de usuario cada 6 h con `Persistent=true` (una estación de trabajo se apaga, y sin eso las corridas perdidas no ocurren y nada lo dice) y `SuccessExitStatus=1`, porque «producción diverge» es un resultado válido de una corrida que anduvo perfecto: sin esa línea la unidad quedaría `failed` en cada drift y alguien terminaría deshabilitando la única comparación que existe. **Y EL DEFECTO QUE SÓLO SE VIO CORRIENDO, que es la parte que vale la pena contar.** Las tres alertas se escribieron con la métrica pelada y las tres estaban rotas **en las dos direcciones a la vez**: una serie EMPUJADA se pone rancia ~5 min después de cada empuje y el latido es cada 6 h. Medido en cuatro instantes contra el servidor: a los 58 s presente; a los 308, 508 y 608 s **ausente** del vector instantáneo, mientras `last_over_time(...[7d])` la encontraba en los cuatro. Con eso, `ComparacionRepoServidorSinCorrer` disparaba a los cinco minutos de CADA latido —un falso positivo cada 6 h para siempre, que es cómo se enseña a ignorar un canal (A79)— y las otras dos **no podían dispararse nunca**, porque su `for` de 30 min no llega a cumplirse antes de que la serie se evapore, y `max()` sobre una serie rancia no da 0: no da NADA, y una expresión sin resultado no alerta. **Ninguna lectura del YAML lo mostraba**: las tres expresiones se leen perfectamente bien. Apareció porque se corrió y la alerta estaba disparando con el latido recién empujado. Comprobado el arreglo evaluando las dos expresiones en el mismo instante, con un latido de 344 s: la vieja daba `absent(...) = 1` —habría disparado— y la nueva, vacío. Lo custodia `TestLasAlertasDelLatidoLeenLaSerieConLastOverTime` (sabotaje: volver a la métrica pelada → rojo; cambiar la ventana de 7d a 30d, que es legítimo → verde), y el acoplamiento entre el guion y las alertas lo custodia `TestElLatidoQueEmpujaElVerificadorEsElQueMiranLasAlertas` con tres sabotajes, incluido el del nombre viejo sobreviviendo en un comentario. **2026-09-05 — CERRADO. El timer está habilitado y corriendo en la máquina de gio**, con `Linger=yes` ya puesto. Y no se dio por bueno con «enable»: se disparó la unidad a mano una vez y se miró el resultado —`Result=success`, `ExecMainStatus=0`, la comparación entera en el journal y `✔ latido empujado`— porque «habilitado» y «funciona» son dos cosas y enterarse en la primera corrida programada es tarde. Quedan como mejoras y no como pendientes: (b) un job de CI que no dependa de ninguna máquina de la casa, y (c) que la expectativa de los conteos viaje en el binario.

**A120 CERRADO (2026-09-09) — Las guardas de texto quedan VERDES sobre el defecto que dicen prohibir — DOS CERRADAS el 2026-09-09, quedan las de `despliegue_esquema_test.go`**

Leídas el 2026-09-08, las tres fallan. `TestConstruirNoIntentaCorrerUnBinarioDeOtraPlataforma` queda verde con `"$SALIDA" version` AFUERA del `if` — o sea con A119(2) puesto; A107 endureció su 1ª aserción y dejó a las hermanas de abajo con el defecto que acababa de declarar insuficiente. `TestNingunaSerieDelCerebroCaeEnElDescarteDelScrape` **se desarma entera con un `t.Skip`** ante un reordenamiento de dos claves YAML que Prometheus interpreta igual — y custodia el modo de falla peor que hay (una serie que cae en el descarte no llega, la alerta usa `and on(...)` y NO dispara; ya pasó con `musubi_fleet_device_net_up`). `TestElScriptDeCambioDeAgenteNoDependeDeQuienLoEjecuta` queda verde con un `REM` y se pone roja sobre un guion correcto que documente la decisión: A107 arregló 1 de sus 5 aserciones. **El número que ordena esto**: el árbol declara **383 sabotajes en 88 archivos** y A107 le dio veredicto a 37 — **9,7 %**, y no existe lista ejecutable de los otros 346. **CERRADAS EL 2026-09-09 (`7da94d5`) LAS DOS DEL `.ps1`, y la causa no era la que decía esta fila.** No era sólo que leyeran texto crudo: `TestElAgenteDeWindowsNoCorreComoSystemSinQueAlguienLoPida` anclaba con `strings.Index(ps, "if ($AlArranque)")`, o sea en la PRIMERA ocurrencia. Mientras hubo una sola, la pregunta coincidía con la propiedad; `5183c26` (2026-09-01) insertó otra 162 líneas más arriba y desde entonces el ancla apunta a la línea 143 mientras el `if` que protege está en la 305 — medido: ancla en offset 8394, `-UserId "SYSTEM"` en 17604. **Ocho días con la ÚNICA guarda del repo sobre el registro como SYSTEM apuntando al bloque equivocado**, y comentar el `if` de la 305 —hacer el registro INCONDICIONAL— la dejaba en verde. Se reemplazó por `bloqueQueEncierra`, que camina llaves y pregunta pertenencia; la prueba de que hacía falta la ESTRUCTURA es que las dos guardas anclan a bloques distintos (la del token al `if` de la 143, la de SYSTEM al de la 305). Y se sacó el `t.Skip` que un NO-OP apagaba: sacarle el espacio a `SetAccessRuleProtection($true, $false)` desarmaba la guarda entera con la regla de SYSTEM puesta. Seis sabotajes, uno de ellos verde a propósito (el NO-OP). **`TestElInstaladorDeWindowsEsAsciiConBOM` NO se tocó y es decisión**: lee bytes crudos porque valida codificación, que aplica igual a los comentarios. **LO QUE QUEDA ABIERTO ACÁ** es `TestNingunaSerieDelCerebroCaeEnElDescarteDelScrape` (`despliegue_esquema_test.go:190`), cuyo `t.Skip` es el gemelo del que se sacó y que la sesión `musubi-89` está mirando.
...
sí existe —lista de tokens, el más VIEJO primero, porque se apendea— es del token
...
ComandoTimeoutMax + MargenDeReporte`. Nunca puede matar a uno vivo. **Corregido el 2026-09-26 (A131·T10, #682): sí podía.** La cuenta suponía que todo lo que atiende el agente está acotado por el timeout de un comando, y `musubi:shell` no lo está: el agente atiende la shell bloqueando la sesión entera, así que su fila y lo que viajó detrás esperan hasta `ShellVidaMax` (2 h). Entre los 102 y los 120 minutos de una shell viva, la bitácora y la cronología dibujaban `perdido` la fila del canal y el comando de atrás. La cota ahora es la peor tanda real —una shell y nueve comandos, más las esperas del agente—: 3 h 32 min 43 s.
...
1. Al cerrar un slice, **borrar** su línea de la tabla 1. Su texto no se pierde: baja entero a la
   sección 3, con su número y la palabra `CERRADO`, que es lo que permite seguirle la pista.
   **Una fila de la tabla 1 no puede declarar en su celda de estado que el cabo está cerrado**: la
   tabla contesta «¿qué falta?», y una fila cerrada adentro es una respuesta falsa que además
   inutiliza el conteo. El 2026-09-10 eran **21 de 48**, y dos de ellas —A99 y A102— estaban
   ABIERTAS escondidas detrás de la palabra («cerrado; falta el redespliegue»), así que un filtro
   por «cerrado» las borraba y un filtro por «sin cerrado» no las veía. Lo custodia
   `TestNingunaFilaDeLaTabla1SeDeclaraCerrada`, que mira la **cabeza del veredicto** de la columna
   de estado y no la palabra suelta: A31 dice «`VerificarFirma` **falla cerrado**» —el modo de
   fallo de una verificación de firma, no el estado de un cabo— y es el falso positivo que esa
   guarda tiene clavado como control.
...
3. La tabla 2 no es un cementerio: cada línea dice **bajo qué condición se revisa**.
   Lo custodia `TestNingunaFilaDeLaTabla2EsUnCementerio`, que pide la forma canónica —**Se
   revisa si …** / **cuando …** / **el día que …**— a propósito: una paráfrasis nueva falla en
   vez de pasar, que es lo contrario de lo que hace una lista de palabras. La misma guarda
   cubre la hermana de la regla 1 acá: una fila de la tabla 2 tampoco puede declararse cerrada.
   **Hasta el 2026-09-10 la regla no tenía guarda y seis de veintiuna filas no la cumplían.**
4. **Este archivo cubre el track «Control de flota», no el repo entero — y ahora los vecinos
   tienen el suyo.** El 2026-09-11 gio decidió el alcance: **un registro por track**, no uno solo
   para todo. Se descartó el archivo único porque éste ya pasa las 3900 líneas y sumarle los
   tracks vecinos lo vuelve ilegible, que es otra forma de que un cabo se pierda. Los que existen
   hoy son `specs/riel-local/ABIERTO.md` (prefijo `RL`) y
   `specs/profundidad-derivada-del-diff/ABIERTO.md` (prefijo `PD`); **los números llevan prefijo
   del track** para que una cita no sea ambigua entre archivos. Quien custodia que no falte
   ninguno es `TestLosCabosDeLasCarpetasVecinasEstanContados`, que le pide a CADA carpeta con
   cabos declarados su propio `ABIERTO.md` con al menos tantas filas como cabos — un archivo que
   existe no alcanza: tiene que NOMBRARLOS.
   Leer este archivo como «todo lo abierto de Musubi» sigue siendo el mismo error que dio origen a
   la tabla: creer cubierto lo que nadie miró.
   **Hasta el 2026-09-10 esta regla decía que `specs/riel-local/` «es EL que hoy tiene ítems vivos
   sin registro propio», y era falso en las dos mitades.** Los tres ítems de riel-local declaran su
   razón en el lugar —uno dice «despachado»—, y los ítems vivos sin registro estaban **adentro de
   este mismo track**: ocho, en las dos secciones `Lo que queda fuera` que viven en un `spec.md` y
   no en un `tasks.md` (`flota-alertas-y-politicas`, `flota-shell-interactiva`). Eran invisibles
   porque los DOS agujeros de `TestNingunCaboDeFlotaSeQuedaSinRegistro` se tapaban entre sí: el
   glob miraba sólo `tasks.md` **y** el regex del encabezado rechazaba `## 2 · Lo que queda fuera`.
   Uno de esos ocho declaraba faltante el cooldown persistente, que **A24 cerró con la migración
   33**. La regla señalaba afuera mientras el agujero estaba adentro, que es la forma exacta que
