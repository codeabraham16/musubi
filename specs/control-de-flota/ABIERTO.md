...
>
> Última revisión: **2026-08-27** (tras S6c, S7b, las dos auditorías, el DESPLIEGUE REAL en `musubi-server`,
> y el **despliegue del relay de pantalla + el arreglo de la ruta de RustDesk**).
>
...
| A34 | **El agente de `musubi-server` sigue con el binario viejo** | Las dos Windows corren el binario con el arreglo de la ruta de RustDesk; el servidor no, porque su agente es `musubi-agente.service` (systemd del SISTEMA) y reemplazarlo pide root — `sudo` en esa máquina pide contraseña, que no tengo ni debería tener. **Impacto real: ninguno hoy** — `musubi-server` no tiene la capacidad `screen`, así que el arreglo no le cambia nada. Es una DIVERGENCIA, no una falla: tres máquinas y dos binarios distintos. | **acción del operador** (①) |
...
| A37 | **La clave del relay no está en ningún respaldo** | `~/musubi-rustdesk/data/` tiene la clave privada del relay y la base de IDs. Si ese disco se pierde, el relay vuelve con OTRA clave y **todos los clientes de la flota dejan de conectar hasta que alguien los reconfigure uno por uno**. El backup del cerebro (`musubi backup`) no lo mira, y además ese backup **sigue siendo local-only**, sobre el mismo disco, por decisión de gio del 2026-08-27. | **acción del operador** (③) |
