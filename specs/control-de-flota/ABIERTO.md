...
| A122 | **Trabajo pago sin mergear que vive en un solo disco, y cinco worktrees donde un `git commit -a` de rutina commitea un revert** | Medido el 2026-09-08. **(1)** `d7fe6b3` (`ola0/techos-export`) aporta la perilla `services_per_project_export` con su `EffectiveServicesPerProjectExport` y el caso «negativo = sin techo», más `fleet_export_techos_test.go` (283 líneas). **Verificado: ninguna de las dos cosas existe en `feat/control-de-flota`**, y la rama está en 0 remotos. Su base es `d32c0d6`, a 183 commits, y toca `fleet_prometheus.go`, que A116 reescribió: **no se mergea sin rebase**. **(2)** Cinco worktrees de `.claude/worktrees/` tienen su árbol en un estado MÁS VIEJO que su propio HEAD con el rollback ya en el ÍNDICE — `f26-3` (−1349), `f26-6` (−1225), `f26-4` (−797), `f26-5` (−290), `f26-2` (+103 sin índice). Varios archivos figuran `MM`. **Un `git commit -a` ahí revierte el aporte y deja la rama con cara de estar al día.** Mitigación del 2026-09-08: los 9 blobs que no existían en `.git` están en `.musubi/backups/rescate-worktrees-20260908/` con manifiesto de hashes, los 4 commits no contenidos están en `ramas-no-mergeadas-20260908.bundle` y en etiquetas `rescate/*`, y se podaron los 5 registros fantasma de `/tmp`. **Falta la decisión: rebasar `d7fe6b3` o descartarlo, y qué hacer con los 33 worktrees (607 MB).** | sin asignar |
