...
| A123 | **Los sabotajes mecanizados no mienten, y la mayoría de las anclas sigue siendo prosa que nadie puede correr** | **2026-09-16 · DOCE LOTES MECANIZADOS: 139 → 203, COBERTURA 16,3 % → 23.8 %.** El censo (`go run ./deploy/cmd/arnes -validar`) daba ese día **854 anclas en 196 archivos**: **203 mecanizadas**, **3 exentas** y **648 en prosa y nada más**. El trinquete `TestLaDeudaDeSabotajesNoCreceYElCorpusNoSePodre` sigue sosteniendo las dos direcciones y falla con directivas rotas, con directivas que dejaron de apuntar y con anclas sin ubicar. **EL VEREDICTO EN LOTE NO CAMBIÓ**: de las 38 auditadas a mano en su momento **6 mentían (15,8 %)**; de las mecanizadas de hoy, **0 %**. Además se verificaron de punta a punta, con `-correr`, los paquetes `internal/fleet` (20/20 ROJO), `internal/mcp` (104/104), `cmd/musubi` (37/37) e `internal/memory` (12/12) — eso mide **las directivas tal como quedan escritas**, que es un camino distinto del guion con el que se prueban los cortes. **DOS COSAS QUE APARECIERON MECANIZANDO Y NO ESTABAN EN EL CABO.** (1) Una guarda de `cmd/musubi/flota_test.go` NO PODÍA FALLAR por el defecto que su prosa nombraba: pedía `Contains(flota.html, "inerte")` sobre el archivo entero y `inerte` aparece 4 veces, así que se podía romper el marcado en la función que lo dibuja y el test seguía verde; se acotó al cuerpo de `automatico()` (arreglado en #530). Barrido posterior: de 59 archivos que leen un archivo y comprueban con `Contains`, sólo 3 usan el idioma comparable y **los otros dos ya acotaban** — era un caso, no un patrón. (2) `gofmt` reescribe comentarios de documentación y convierte `''` en `”` DENTRO de una directiva, sin que nada lo detecte: el censo valida el `de` y **nunca el `a`**. Regla: ningún `de`/`a` puede contener `''` ni dobles backticks, y se mira `gofmt -d` antes de `-w`. **DÓNDE ESTÁ LO QUE QUEDA** (medido el 2026-09-16): 15 en `internal/mcp/fleet_cronologia_test.go`, 15 en `internal/mcp/fleet_otlp_test.go`, 14 en `cmd/musubi/agent_test.go`, 14 en `internal/mcp/servicios_test.go`, 12 en `internal/fleet/exposicion_test.go`, 12 en `internal/mcp/fleet_cuatro_ojos_test.go` — y una **cola de 566 anclas en 152 archivos más**, que es donde está el grueso y donde el trabajo deja de ser por archivo. **LA CUENTA NO SE ESCRIBE A MANO: SE PIDE.** `go run ./deploy/cmd/arnes -validar` la imprime entera, y cualquier número de esta fila que no lleve fecha al lado ya está viejo — el título de la fila llevaba 203 y 648 hasta el 2026-09-19. **Lectura del 2026-09-19: 868 anclas en 203 archivos — 247 mecanizadas, 4 exentas, 617 en prosa, cobertura 28,5 %.** **OJO CON EL TECHO**: es un derivado del ÁRBOL ENTERO clavado en una constante, así que dos ramas que mecanizan un lote cada una pasan verdes por separado y la segunda en mergear rompe; los doce lotes fueron secuenciales por eso. | sin asignar |
| A129 | **Faltan dos agentes Windows para cerrar TLS, y el paso que los sigue no se puede adelantar sin dejarlos incomunicados** | **2026-09-19 — LO QUE YA ESTABA HECHO Y NADIE HABÍA MIRADO.** El TLS del cerebro no hay que montarlo: `tailscale serve` ya sirve HTTPS en el **10000** sólo al tailnet y reenvía a `http://127.0.0.1:7717`, con el certificado del tailnet que **se renueva solo**. Verificado de punta a punta CON control negativo: por nombre da 200 con certificado válido; discando la IP y forzando el nombre —que es exactamente lo que hace el agente con `MUSUBI_BRAIN_TLS_NAME`— también; y discar la IP **sin** declarar el nombre **falla**, porque el certificado de Let's Encrypt lleva sólo el nombre como SAN y ninguna IP. `POST /fleet/heartbeat` por ese camino contesta 401 con un token falso, o sea que llega y autentica. **POR QUÉ NADIE LO VIO**: el mensaje del verificador mandaba a poner `tls_cert_file` adentro del cerebro, así que el instrumento que MIDE la postura también RECOMENDABA la salida y su recomendación heredaba su punto ciego — el chequeo lee el config del cerebro y por diseño no puede ver un TLS terminado por delante. Corregido en el mismo día: hoy el bloque recolecta también `tailscale serve status --json` y la dirección de escucha, y `TestLaPerillaDeTLSDecideCorriendo` pasó de cinco casos a ocho. **EL AGENTE DEL PROPIO SERVIDOR NO HAY QUE MIGRARLO**: apunta a `http://127.0.0.1:7717` y su unidad lo dice — «El cerebro es LOCAL: el agente no sale a la red». Su bearer nunca toca la red. **LO QUE FALTA SON DOS VARIABLES POR MÁQUINA**, en `gio` y en `davantis-1`: `MUSUBI_BRAIN_URL=https://100.79.126.62:10000` y `MUSUBI_BRAIN_TLS_NAME=musubi-server.tail89e295.ts.net`. El nombre es OBLIGATORIO y no decorativo: se disca la IP porque con NordVPN el MagicDNS no resuelve. **EL ORDEN NO SE PUEDE INVERTIR, Y ES LA MITAD DEL CABO.** Atar el cerebro a `127.0.0.1` antes de migrar los agentes los deja incomunicados a los dos en el mismo instante — son Windows, sin ssh, y `davantis-1` ya estuvo dos horas sin latir el 2026-09-19. El orden es: migrar los dos agentes y verificar el latido de cada uno, DESPUÉS atar el cerebro a loopback, y RECIÉN AHÍ prender `MUSUBI_EXIGIR_TLS=1`. Prenderla antes da rojo sobre un despliegue correcto, y eso es lo que apaga un canal. **NO HAY BLOQUEO DE CREDENCIAL, Y LO PRIMERO QUE ESCRIBÍ ACÁ ERA FALSO.** Durante unas horas esta fila dijo que la máquina de desarrollo no tenía ninguna credencial con `fleet: exec` y que `~/.musubi/mando.token` era una credencial muerta. Las dos cosas eran mentira, y salieron del mismo error de medición: el listado de principales se pidió con un `head -30` y el recorte se leyó como ausencia. Medido de nuevo entero: `~/.musubi/token` y `~/.musubi/mando.token` son **el mismo valor**, escritos en el mismo segundo, y corresponden al principal **`davantis-2`**, que tiene `fleet:` con `exec`, `metrics`, `screen` y `shell` sobre `'*'`. Verificado con UNA sola llamada de lectura —no se prueba una credencial mandándola a ver si rebota: cinco 401 bloquean por IP también a la buena—: `GET /metrics` por HTTPS al 10000, discando la IP con el nombre forzado desde la máquina de desarrollo, contesta 200 con certificado válido. O sea que el camino TLS ya está probado también desde AFUERA del servidor, con una credencial real. **Higiene**: hay dos copias byte a byte de esa credencial en `~/.musubi/`, así que rotar una deja la otra. **2026-09-20 · MEDIDO CONTRA EL CÓDIGO, Y FALTAN TRES PASOS QUE ESTE CABO NO LISTABA.** (1) **El hermano sin ServerName**: `clienteLatido` —el único cliente que lleva `MUSUBI_BRAIN_TLS_NAME`— cubre el latido (`cmd/musubi/agent.go` (el latido)) y el reporte de resultados (`cmd/musubi/ejecutor.go` (el POST del resultado)), pero el canal de shell arma **su propio** `http.Client` sin ServerName en `cmd/musubi/shell_agente.go`, usado por `bajarTeclas` y `subirSalida`. No bloquea a `gio` ni a `davantis-1` —ninguna tiene la capacidad `shell`— pero rompería a la máquina de desarrollo apenas adopte la URL https. Los tres clientes tienen que salir de `clienteParaElCerebro`, que ya existe y sólo le faltan llamadores. (2) **`deploy/cambiar-agente.cmd` lleva la URL del cerebro HARDCODEADA** en `http://100.79.126.62:7717`, así que atar el cerebro a loopback rompe el mecanismo de actualización de los agentes Windows, y su fallo dispara rollback con un mensaje que acusa al binario. (3) **`deploy/agente-windows.ps1` no escribe `MUSUBI_BRAIN_TLS_NAME`** en el `agente.cmd` que genera: cualquier reinstalación revierte la migración **en silencio**. Necesita un parámetro `-TlsName` y su línea en el heredoc. **Y EL «SONDEAR ANTES» NO SE PUEDE HACER, medido en la máquina el 2026-09-20**: en `davantis-1` NordVPN autoriza **por archivo** y el único ejecutable que alcanza el tailnet es `musubi.exe` (está documentado en `deploy/actualizar-agente-windows.sh`) — una sonda TLS por PowerShell falló con `WSAEACCES` **igual en el caso bueno que en el control negativo**, que es justo lo que prueba que el socket nunca se abrió y que el error no habla del cerebro. Tampoco sirve `musubi fetch` (no fuerza ServerName) ni `musubi_fleet_probe` (saltea los Tier A por diseño). O sea: **se verifica DESPUÉS, no antes**. **EL MODO DE FALLA ES INDISTINGUIBLE DE `MaquinaCaida`**: con la URL mal, el agente NO muere — reintenta con backoff hasta 2 min para siempre (el `default` del bucle, con `esperaMaxima` de 2 min) — y como el despacho es **polling puro** del agente (ver `internal/fleet/protocolo.go`), sin latido no hay exec, ni screen, ni shell. En una Windows sin ssh eso se arregla **yendo a la máquina**. **CÓMO SE HACE, ENTONCES**: en Windows el agente es una **Tarea Programada** (no un servicio) que corre un `agente.cmd` con `set MUSUBI_BRAIN_URL=`; `setx` no sirve. Se respalda el `.cmd`, se escribe el nuevo **al lado** y se hace `Move-Item` de una (no editar en su lugar), se reinicia la tarea y se mira `musubi_fleet_list` a los 90 s. Conviene abrir ventana de mantenimiento: si rompe, `AgenteCaidoConMaquinaViva` tarda 5 min y se mezcla con lo que ya suena. **Verificado desde la máquina de desarrollo (linux) el 2026-09-20**: `https://100.79.126.62:10000` con `--resolve` al nombre del tailnet da HTTP 200 y `ssl_verify=0`; sin declarar el nombre, falla. El camino está sano; lo que falta es poder probarlo DESDE las Windows. **2026-09-20 · LOS DOS AGENTES MIGRADOS, Y EL INVENTARIO DE CLIENTES ERA MÁS GRANDE DE LO QUE ESTA FILA DECÍA.** `davantis-1` y `gio` laten por TLS. `gio` quedó sin agente durante una hora por un `schtasks /end` que se llevó al propio guion de reparación —ahí la tarea corre `agente.cmd` DIRECTO como SYSTEM, así que el proceso que la migraba estaba adentro del árbol que `/end` mata, y `RestartCount` no revive una parada deliberada—; se recuperó YENDO A LA MÁQUINA con `schtasks /run` desde una PowerShell de administrador. Su `migracion-tls.log` corta exactamente en la línea anterior al `/end`, que es la evidencia limpia del diagnóstico. **EL PUERTO EN CLARO NO LO USAN SÓLO LOS AGENTES, Y ÉSE ES EL HALLAZGO.** Contado en el servidor con `ss -tn state established "( sport = :7717 )"`: ONCE conexiones desde el tailnet que no eran ningún agente. Hay **TRES** tipos de cliente del cerebro, no uno: (a) `sync.central_url` del `.musubi/config.yaml`, que lee el daemon por `syncclient.go` (`NewSyncClient`); (b) `MUSUBI_CENTRAL_URL`, que leen `cerebro.go` y `dashboard.go`; y (c) la entrada `musubi-cerebro` del `.mcp.json`, que **la lee el EDITOR y no musubi** — es la que escribe `mcpwire.go` (`wireMCPJSON`). Razonar sobre «los agentes» dejaba afuera a las otras dos. **#579 CERRÓ EL LADO DEL CÓDIGO**: paquete `internal/cerebro` con el constructor (`Cliente`) y el único lector del nombre TLS (`NombreTLS`); `syncclient.go` y CUATRO sitios de `provision` (`Reach`, `Auth`, `wireMCPJSON` y `ensureSyncConfig`) salen de ahí. **El cuarto se escapó de la primera pasada por buscar POR FORMA** —un grep del literal `"http://"`— y ése lo tiene como `http://%s` adentro de un formato más grande; peor, normalizar los otros tres lo volvió ALCANZABLE, porque antes el dial del string crudo moría con «too many colons in address» y el flujo cortaba antes de escribir nada. La guarda pasó de `cmd/musubi/` al repo entero (356 archivos, 14 clientes marcados) y se le taparon cuatro agujeros que encontró una revisión adversaria: una excepción por NOMBRE sin anclar a ningún archivo, otra que dejaba pasar un MÉTODO homónimo, el chequeo del único lector ciego a `SelectorExpr` —que es la única forma que puede escribir un paquete de afuera— y `http.DefaultClient`, que escapaba entero y tenía tres usos reales. **UNA CONFIG TRACKEADA NO SE PUEDE MIGRAR POR MÁQUINA.** De seis `config.yaml` migrados revirtieron exactamente DOS: los de `Altura-erp`, uno en cada máquina, después de un `pull --rebase --autostash`. La causa es que ahí ese archivo estaba VERSIONADO; el control que lo cierra es que en musubi está en `.gitignore` y ahí el cambio sobrevivió intacto. Resuelto gitignoreándolo y versionando un `config.example.yaml` con la URL https ya adentro. **Y SACAR UN ARCHIVO DEL ÍNDICE LO BORRA DEL DISCO EN CADA CLON** en el primer `pull`: pasó en las dos máquinas, y en su lugar se regenera un default con `central_url: ""` que deja el sync APAGADO sin que nada lo diga. **EL MISMO PROYECTO TIENE DOS UPSTREAMS**: `davantis` empuja a GitHub y `davantis-1` tira de Forgejo (`:3000` del server). Comparten historia y alguien los sincroniza, pero un push a uno no alcanza al otro. **LO QUE FALTA, EN ORDEN**: que el conteo en claro desde el tailnet se mantenga en CERO con todas las sesiones levantadas —hoy da cero, pero medido con sesiones recién reiniciadas—, después atar el cerebro a `127.0.0.1` y reiniciar `musubi-brain`, y recién ahí `MUSUBI_EXIGIR_TLS=1`, que vive en la unidad `musubi-comparar` y es un trinquete: la alerta `ExigenciaDeTLSApagada` sólo puede sonar si alguien la apaga después de prenderla.| **sin asignar** — falta confirmar el conteo de conexiones en claro con la flota despierta, atar el cerebro a `127.0.0.1` y prender `MUSUBI_EXIGIR_TLS=1` |
