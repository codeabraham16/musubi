...
| A125 | **Siete cabos declarados fuera de alcance en dos carpetas vecinas no tienen NINGÚN registro** | Medido el 2026-09-11 por `TestLosCabosDeLasCarpetasVecinasEstanContados`: `specs/riel-local/tasks.md` (3 ítems) y `specs/profundidad-derivada-del-diff/tasks.md` (4). No están acá —la **regla 4** dice que este archivo cubre el track «Control de flota» y no el repo— ni en un registro propio, que no existe. O sea que son exactamente lo que la primera línea de este archivo prohíbe: algo abierto sin dueño. **Esta fila NO los adopta**: los NOMBRA, que es lo que faltaba para que la decisión se tome sobre algo contado. La decisión es de alcance —un `ABIERTO.md` por track, o uno solo para todo— y es la número 8 del plan de cierre. Mientras tanto la prueba los lista en cada corrida y **falla si aparece una tercera carpeta**, porque eso cambia la pregunta: de «dos pendientes de una decisión» a «esto se acumula». | **gio** — decisión de alcance |
...
4. **Este archivo cubre el track «Control de flota», no el repo entero.** Otros tracks tienen sus
   propios `## Lo que queda fuera` y NO están acá; hoy el único es `specs/riel-local/tasks.md`.
   Leer este archivo como «todo lo abierto de Musubi» sería el mismo error que dio origen a la
   tabla: creer cubierto lo que nadie miró.
