...
| A123 | **Los 139 sabotajes EJECUTABLES ya tienen veredicto y ninguno miente; lo que queda son 712 promesas que nadie puede correr** | **2026-09-15 · SE CORRIÓ EL BARRIDO COMPLETO, Y LA FILA ESTABA RANCIA EN TRES FRENTES.** Los números de arriba son del 2026-09-10 sobre `f9be15a` y ya no describen el árbol: hoy el censo da **854 anclas en 196 archivos** (no 384 en 89), de las cuales **139 mecanizadas**, **3 exentas** y **712 en prosa y nada más** — cobertura ejecutable **16,3 %**. Y las otras dos afirmaciones dejaron de ser ciertas: **«no hay lista ejecutable»** — `go run ./deploy/cmd/arnes -json` la emite entera, con archivo, línea, prosa y nombre de prueba; **«nada lleva la cuenta»** — `TestLaDeudaDeSabotajesNoCreceYElCorpusNoSePodre` (`internal/mcp/arnes_censo_test.go`) corre EN CI y sostiene un trinquete de DOS direcciones: prohíbe que la deuda suba y también que el techo se afloje más de 30, para que mecanizar un lote obligue a bajar el número. Además falla con directivas rotas, con directivas que dejaron de apuntar y con anclas sin ubicar. **LO ÚNICO QUE DE VERDAD FALTABA ERA EL VEREDICTO EN LOTE, Y YA ESTÁ: 139 de 139, CERO HUECOS.** Corrida completa el 2026-09-15 (`go run ./deploy/cmd/arnes -correr`, **27 min 3 s**, ~10 s por sabotaje — no «horas», que era la estimación sobre 774 anclas de un árbol casi sin mecanizar): **138 en ROJO (sanas), 0 en VERDE, 0 motivos repetidos, 0 rojos sospechosos**, y el árbol verificado sin un solo sabotaje puesto. La única que se abstuvo —`TestLaTokenizacionDelCharsmapEstaFijada`— no es un hueco: su directiva DECLARA `env="MUSUBI_SPM_TESTDATA"` y el arnés se negó a medir sin ella en vez de contarla como verde (CI sí la setea, en los dos jobs de `.github/workflows/ci.yml` que corren embedding); corrida con el fixture puesto, sale **ROJA (sana)**. **Contra la tasa histórica esto es el dato que el cabo pedía**: de las 38 auditadas a mano en su momento, 6 mentían (**15,8 %**); de las 139 mecanizadas de hoy, **0 %**. Tiene explicación —éstas nacieron con el arnés exigiendo control positivo, rojo NOMBRADO y la línea del fallo— pero ahora está medido y no supuesto. **POR QUÉ NO SE CIERRA IGUAL**: quedan **712 anclas en prosa** sin directiva, o sea promesas que nadie puede correr, y cerrar acá borraría ese hecho. **Lo que cierra el cabo es escribirles la directiva**, y el trabajo está acotado y ordenado: están concentradas —27 en `internal/mcp/fleet_otlp_test.go`, 21 en `cmd/musubi/flota_test.go`, 19 en `fleet_cuatro_ojos_test.go` y en `servicios_test.go`, 17 en `cronologia_test.go` y en `fleet_politicas_test.go`— y `-detalle` las lista por archivo. Ojo con el techo: es un derivado del ÁRBOL ENTERO clavado en una constante, así que dos ramas que mecanizan un lote cada una pasan verdes por separado y la segunda en mergear rompe. | sin asignar |
