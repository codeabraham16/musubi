...
| A117 | **El respaldo de la memoria de este proyecto NUNCA corrió en esta máquina, y las cuatro capas que deberían decirlo están mudas** | Medido el 2026-09-08. `.musubi/backups/` tenía **un** archivo, `memory.db.20260726-004141`, de hace **44 días** — y no lo dejó un backup programado sino el auto-heal (`backupDB`, `internal/memory/doctor.go-378`). Encima hay cuatro capas y ninguna emite señal: `checkOffhostBackup` devuelve `Status:"ok", Unmeasured:true` (`doctor.go`) y delega el ruido a «el fallo-cerrado del propio script, visible en `systemctl status musubi-backup`» — **unidad que en esta PC no existe**; `deploy/musubi-alerts-backup-offhost.yml` está parkeado a propósito (`despliegue_custodia_test.go-223`); y el doctor lo muestra en VERDE. La base tiene 30 MB y 2139 observaciones. **Es el único nodo que guarda la única copia de todo, y es el único sin vigilancia.** Mitigación del 2026-09-08: se dejó `memory.db.20260908-133955` a mano — que es exactamente el paso manual que este cabo existe para eliminar. | sin asignar |
