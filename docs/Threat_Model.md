...
| DNS-rebinding (sin credencial) | Chequeo de Host loopback + Origin local en **todas** las puertas de persona, por el envoltorio `puertaDePersona`. Cuelga de que NO haya credencial, **no del bind**: con bearer o registro de principals no corre, porque el navegador de la víctima no puede fabricar un bearer — y así un proxy TLS propio, que preserva el `Host`, puede pasar. Medido el 2026-09-20: antes vivía en 1 de 6 puertas y `/api/stream` servía el feed en vivo y `/api/flota` aceptaba escrituras a un `Host` ajeno. Lo custodian `TestNingunaPuertaDePersonaContestaAUnHostAjenoSinCredencial` (comportamiento) y `TestNingunaPuertaNaceSinDecirDeQuienEs` (estructural: una puerta nueva nace con la compuerta o declara que es de dispositivo) |
