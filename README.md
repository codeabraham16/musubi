...
flowchart LR
    ...
    subgraph M["Musubi · daemon Go"]
        direction TB
        RPC["JSON-RPC 2.0 / stdio<br/>52 herramientas MCP"]
...
El servidor expone **52 herramientas**, agrupadas por dominio:
...
| **Skills** | `musubi_detect_stack` · `musubi_search_skills` · `musubi_list_skills` · `musubi_save_skill` · `musubi_resolve_skills` · `musubi_log_skill_decision` · `musubi_discover_skills` (marketplace) · `musubi_author_skill` |
...
internal/
  ...
  mcp/             # servidor JSON-RPC 2.0 + las 52 herramientas MCP
