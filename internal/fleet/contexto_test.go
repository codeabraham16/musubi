...
func TestElNombreDeLaMaquinaSiempreEsUnTermino(t *testing.T) {
	...
	ts := TerminosDeContexto("altura-db", muchos)
...
func TestElSufijoDeUnidadNoLlegaALaBusqueda(t *testing.T) {
	ts := TerminosDeContexto("pc", []string{"nginx.service", "backup.timer", "api.altura"})
...
func TestLosTerminosNoSeRepiten(t *testing.T) {
	ts := TerminosDeContexto("nginx", []string{"nginx", "NGINX.service", "Nginx"})
	if len(ts) != 1 {
...
func TestUnTerminoDemasiadoCortoNoEntra(t *testing.T) {
	ts := TerminosDeContexto("ab", []string{"x", "ok"})
	if len(ts) != 0 {
	...
	if ts := TerminosDeContexto("ño", nil); len(ts) != 0 {
	...
	if ts := TerminosDeContexto("año", nil); len(ts) != 1 {
