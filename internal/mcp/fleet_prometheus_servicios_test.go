...
func TestLosServiciosDeUnaMaquinaQueNoVeoNoSeExportan(t *testing.T) {
	...
	renderFlota(&b, s.engine, bob, time.Now(), s.sondaIntervalo, versionDePrueba, s.techoServiciosPorProyecto)
	salida := b.String()
...
func TestElScrapeYElEmpujeExportanLosMismosServicios(t *testing.T) {
	...
	cuerpo, _, _, err := armarPayloadOTLP(s.engine, p, ahora, s.sondaIntervalo, versionDePrueba, s.techoServiciosPorProyecto)
	if err != nil {
		t.Fatal(err)
	}
