...
func (s *McpServer) toolFleetLog(ctx context.Context, raw json.RawMessage) (interface{}, *RpcError) {
	...
	if nombre := strings.TrimSpace(args.Device); nombre != "" {
		...
		if filtroID == "" {
			return nil, rpcErrorf(codeUnauthorized,
				"no hay bitácora de %q: o no existe en el proyecto %q, o tu credencial no tiene la capacidad `exec` sobre esa máquina", nombre, proyecto)
		}
	}

	crudos, err := s.engine.BitacoraDeComandos(proyecto, filtroID, tope*4)
	if err != nil {
		return nil, rpcErrorf(codeInternalError, "%v", err)
	}
