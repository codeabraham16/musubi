...
// Latido es TODO lo que un latido escribe sobre el registro, junto, para que entre en UNA sola
// transacción.
//
// ────────────────────────────────────────────────────────────────────────────────────────────
// POR QUÉ UN STRUCT Y UNA TRANSACCIÓN, Y NO CUATRO MÉTODOS COMO ANTES
//
// Un latido hacía como PISO cuatro escrituras en cuatro transacciones separadas: el autorreporte
// (versión y dirección), la capacidad de preguntar, la señal de vida y el vencimiento de la cola
// de comandos. Cada una con su commit, y cada commit con su fsync (WAL sin `synchronous`
// explícito es FULL). Con cuatro máquinas no se nota. Con dos mil latiendo cada 30 s son ~67
// latidos por segundo, o sea ~270 fsyncs por segundo para decir «sigo viva» dos mil veces —
// sobre un disco que además tiene que atender la memoria.
//
// Lo que un latido escribe es UN hecho —«a esta hora esta máquina estaba así»— y merece una sola
// confirmación. Los campos son opcionales con la MISMA regla en todos: lo que no viene no toca la
// columna. Es la asimetría de siempre del latido («vacío no borra»), ahora sostenida en un solo
// lugar en vez de repetida en cuatro.
type Latido struct {
	DeviceID string
	Ahora    time.Time
	// Muestra es el JSON de la telemetría del host (S4). VACÍA NO BORRA LA ANTERIOR: un agente
	// en un OS sin colector, o con el colector roto, late igual (está vivo, que es lo que el
	// latido afirma) y no tiene por qué hacer desaparecer la última medición buena que sí se
	// tomó. Estar viva y saber medirse son cosas distintas.
	Muestra string
	// Version y Direccion son el autorreporte: lo que la máquina sabe de SÍ MISMA y el cerebro
	// no puede averiguar solo. Vacíos no pisan lo que había: un agente viejo que no reporta
	// versión no debe borrar la que quedó registrada.
	Version   string
	Direccion string
	// PuedePreguntar es la capacidad MEDIDA por el agente (A57). Es un puntero porque «no
	// opinó» y «midió y no puede» son hechos distintos, y sólo el segundo se escribe: escribir
	// `false` por un agente viejo que no manda el campo endurecería un `pide` a `prohibido` sin
	// que nadie lo haya medido.
	PuedePreguntar *bool
	// TopeComandos es cuántos comandos pendientes se entregan en este mismo latido (cero: no
	// se toca la cola). Vencer lo viejo y marcar lo entregado van en la MISMA transacción que la
	// señal de vida: es la ventana exacta en la que importa, y así no cuesta un commit aparte.
	TopeComandos int
}

// Latir es el latido completo en UNA transacción: estampa la señal de vida, guarda lo que vino
// del autorreporte, vence los comandos viejos y entrega los pendientes. Devuelve si la máquina
// sigue activa y, si lo está, los comandos que le tocan.
//
...
// agente que se dé por revocado. Y a una máquina que ya no está NO se le entrega nada: la
// transacción se deshace entera, con la cola sin tocar.
//
// Las columnas del autorreporte y la telemetría van en el MISMO UPDATE que last_seen, así que
// ninguna agrega una escritura. Cada una con su CASE-por-parámetro: cuando el valor viene vacío
// (o nulo, para el puntero) la columna se reasigna a sí misma. Una sola sentencia para todos los
// casos, en vez de caminos que se pueden desincronizar.
func (e *DbEngine) Latir(l Latido) (bool, []fleet.Comando, error) {
	id := strings.TrimSpace(l.DeviceID)
	if id == "" {
		return false, nil, nil
	}
	version, direccion := strings.TrimSpace(l.Version), strings.TrimSpace(l.Direccion)
	var puede sql.NullInt64
	if l.PuedePreguntar != nil {
		puede.Valid = true
		if *l.PuedePreguntar {
			puede.Int64 = 1
		}
	...
	res, err := tx.Exec(
		`UPDATE devices
		    SET last_seen       = ?,
		        last_sample     = CASE WHEN ? = ''    THEN last_sample     ELSE ? END,
		        agent_version   = CASE WHEN ? = ''    THEN agent_version   ELSE ? END,
		        address         = CASE WHEN ? = ''    THEN address         ELSE ? END,
		        puede_preguntar = CASE WHEN ? IS NULL THEN puede_preguntar ELSE ? END
		  WHERE id = ? AND revoked = 0`,
		l.Ahora.UTC().Format(time.RFC3339),
		l.Muestra, l.Muestra, version, version, direccion, direccion, puede, puede, id,
	)
	if err != nil {
		return false, nil, fmt.Errorf("error al registrar el latido de %q: %w", id, err)
	}
	n, err := res.RowsAffected()
	if err != nil {
		return false, nil, fmt.Errorf("error al leer el resultado del latido de %q: %w", id, err)
	}
	if n == 0 {
		return false, nil, nil // el Rollback diferido deshace lo (nada) que se hizo
	}

	var comandos []fleet.Comando
	if l.TopeComandos > 0 {
		if comandos, err = tomarComandosEn(tx, id, l.Ahora, l.TopeComandos); err != nil {
	...
	return true, comandos, nil
}

// LatirDevice estampa la última señal de vida y, si viene, la última muestra. Devuelve si
// actualizó. Es el latido SIN cola ni autorreporte —lo que usan la sonda de un Tier B y el exec
// por SSH, que prueban vida sin ser un agente— y por eso es Latir con el resto vacío: la regla
// «vacío no borra» y el A10 viven allá, una sola vez.
func (e *DbEngine) LatirDevice(id string, ahora time.Time, muestra string) (bool, error) {
	ok, _, err := e.Latir(Latido{DeviceID: id, Ahora: ahora, Muestra: muestra})
	return ok, err
}
