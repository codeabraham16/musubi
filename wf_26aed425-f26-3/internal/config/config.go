...
type FleetConfig struct {
	...
	// ServicesExportPerProject es el techo de servicios que salen a métricas POR PROYECTO en
	// cada export (scrape y empuje). 0 ⇒ default (2000). NEGATIVO ⇒ sin techo.
	//
	// POR PROYECTO Y NO EN TOTAL, y la diferencia es una flota a ciegas: el techo anterior era de
	// 2000 en toda la flota por scrape, y a ~35 máquinas con 60 servicios cada una se pasaba. Las
	// que quedaban afuera dejaban de tener `musubi_fleet_service_up`, `ServicioCaido` no tenía
	// nada que matchear, y ninguna alerta lo decía. Un tenant que crece no puede dejar sin
	// cobertura a los demás; y cuando un proyecto se pasa de su techo, el export lo DICE en
	// `musubi_fleet_export_truncated{kind="services"}` y ExportacionTruncada dispara.
	ServicesExportPerProject int `yaml:"services_export_per_project,omitempty"`
...
// ServiciosExportadosPorProyectoDefault es el techo de servicios por proyecto y por export cuando
// nadie configuró otro. 2000 son ~33 máquinas con 60 servicios cada una en UN tenant: bastante
// más que el proyecto más cargado de hoy, y todavía cómodo para Prometheus.
const ServiciosExportadosPorProyectoDefault = 2000

// EffectiveServicesExportPerProject devuelve el techo de servicios por proyecto. 0 ⇒ sin techo.
//
// Mismo vocabulario que ProbeMinutes: no escribir nada da el default, y sólo un negativo lo
// apaga. Tratar todo <= 0 como «sin techo» dejaría sin protección justamente al despliegue que
// no configuró nada, que es al revés de lo que un default tiene que hacer.
func (f FleetConfig) EffectiveServicesExportPerProject() int {
	if f.ServicesExportPerProject < 0 {
		return 0
	}
	if f.ServicesExportPerProject == 0 {
		return ServiciosExportadosPorProyectoDefault
	}
	return f.ServicesExportPerProject
}
