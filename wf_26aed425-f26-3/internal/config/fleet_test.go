...
// El techo de servicios exportados por proyecto sigue el mismo vocabulario que los otros dos, y
// por el mismo motivo: 0 (no lo escribí) tiene que dar el default de 2000; negativo (lo apagué)
// tiene que dar «sin techo». Un default de «sin techo» dejaría a Prometheus sin protección
// justamente en el despliegue que no configuró nada, que es la mayoría.
//
// Sabotaje que la hace fallar: tratar cualquier valor <= 0 como "sin techo" en
// EffectiveServicesExportPerProject, o cambiar el default sin tocar la constante.
func TestElTechoDeServiciosExportadosDistingueElDefaultDelApagado(t *testing.T) {
	casos := []struct {
		nombre  string
		cfg     FleetConfig
		esperar int
	}{
		{"sin escribir nada ⇒ default", FleetConfig{}, ServiciosExportadosPorProyectoDefault},
		{"explícito", FleetConfig{ServicesExportPerProject: 500}, 500},
		{"negativo ⇒ sin techo", FleetConfig{ServicesExportPerProject: -1}, 0},
	}
	if ServiciosExportadosPorProyectoDefault != 2000 {
		t.Errorf("el default es %d y la documentación (config.example.yaml, RUNBOOK) dice 2000", ServiciosExportadosPorProyectoDefault)
	}
	for _, c := range casos {
		if got := c.cfg.EffectiveServicesExportPerProject(); got != c.esperar {
