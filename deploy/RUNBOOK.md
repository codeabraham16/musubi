...
apaga sola. Si en vez de corregir restaurás un respaldo que conserva el mtime del archivo que el
cerebro tiene cargado (tomado y devuelto con `cp -p`, `cp -a` o `rsync -a`), también vuelve a 0 en
≤10 s, pero sin relectura: el cerebro lo reconoce por el mtime como el archivo que ya autentica, así
que en ese caso no aparece la línea «recargado en caliente» en el journal.
